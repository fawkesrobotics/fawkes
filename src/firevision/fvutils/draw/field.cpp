...
Field::Field(FieldLines *lines, bool destroy_on_delete)
{
  ...
  set_color_line_points(YUV_t::magenta());
...
Field::draw_line_points(SharedMemoryImageBuffer *target, bool draw_landscape, float scale) const
{
  ...
  for (fld_line_points::const_iterator it = __points.begin(); it != __points.end(); ++it)
  {
    ...
    draw_cross(target, x, y, __c_line_points);
  }
}

/**
 * Draws a simple cross (to mark a point)
 * 
 * @param target the image buffer
 * @param x the x position in the image
 * @param y the y position in the image
 * @param color the color
 * @param bar_length the length of a bar (needs to be mod 2)
 */
inline void
Field::draw_cross(SharedMemoryImageBuffer *target, unsigned int x, unsigned int y, YUV_t color, unsigned int bar_length) const
{
  bar_length += (bar_length % 2); 
  unsigned int delta_u = target->width() * target->height();
  unsigned int delta_v = delta_u + delta_u / 2;
  unsigned char *buffer = target->buffer();

  if (y < 0 || y >= target->height() || x < 0 || x >= target->width()) return; //Point is outside the field...

  for (unsigned int cx = std::max(0, static_cast<int>(x - bar_length)); cx < std::min(target->width(), x + bar_length + 1); ++cx) {
    unsigned int addr = y * target->width() + cx;
    
    buffer[addr] = color.Y;
    buffer[addr / 2 + delta_u] = color.U;
    buffer[addr / 2 + delta_v] = color.V;
  }

  for (unsigned int cy = std::max(0, static_cast<int>(y - bar_length)); cy < std::min(target->height(), y + bar_length + 1); ++cy) {
    unsigned int addr = cy * target->width() + x;
