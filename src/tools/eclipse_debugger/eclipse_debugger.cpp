 ...
 */
EclipseDebugger::EclipseDebugger(BaseObjectType *cobject, const Glib::RefPtr<Gtk::Builder> &builder)
{
	bb = NULL;

	connection_dispatcher.signal_connected().connect(
	  sigc::mem_fun(*this, &EclipseDebugger::on_connect));
	connection_dispatcher.signal_disconnected().connect(
	  sigc::mem_fun(*this, &EclipseDebugger::on_disconnect));
...
EclipseDebugger::on_connect()
{
	try {
		if (!bb) {
			...
			EclipseDebuggerInterface::ConnectionMessage *cm =
			  new EclipseDebuggerInterface::ConnectionMessage();
