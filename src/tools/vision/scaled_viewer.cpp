...
timeout_handler()
{
	...
	convert(cam->colorspace(), RGB, cam->buffer(), rgb_buffer, orig_width, orig_height);
	tt.ping_end(ttc_convert);

	tt.ping_start(ttc_draw);
	Glib::RefPtr<Gdk::Pixbuf> image = Gdk::Pixbuf::create_from_data(
	  rgb_buffer, Gdk::COLORSPACE_RGB, false, 8, orig_width, orig_height, 3 * orig_width);

	int                       width  = img_image->get_width();
	int                       height = img_image->get_height();
	Glib::RefPtr<Gdk::Pixbuf> scaled = image->scale_simple(width, height, Gdk::INTERP_NEAREST);
...
main(int argc, char **argv)
{
	...
	builder = Gtk::Builder::create_from_file(RESDIR "/guis/scale_viewer/scale_viewer.ui");
