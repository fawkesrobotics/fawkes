...
private:
	...
	public:
		FuseColumnRecord()
		{
			...
			add(m_name);
			add(m_service_name);
			add(m_service_domain);
			add(m_service_hostname);
			add(m_service_port);
			add(m_image_id);
			add(m_image_width), add(m_image_height);
	...
	typedef enum { SRC_NONE, SRC_FILE, SRC_SHM, SRC_FUSE } ImageSource;

	typedef enum { MODE_VIEWER, MODE_COLOR_TRAIN, MODE_MIRROR_CALIB, MODE_MIRROR_CALIB_EVAL } OpMode;
