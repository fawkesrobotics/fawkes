...
ColormapViewerWidget::set_layer_selector(Gtk::Scale *scl)
{
	...
	m_scl_layer_selector->signal_change_value().connect(
	  sigc::mem_fun(*this, &ColormapViewerWidget::on_layer_selected));
}
...
ColormapViewerWidget::draw(unsigned int layer)
{
	...
	if (layer >= m_cm->deepness()) {
		if (!m_scl_layer_selector)
			...
			layer = (unsigned int)rint(m_scl_layer_selector->get_value());
	}

	unsigned int cm_layer = (layer * m_cm->depth()) / m_cm->deepness();

	unsigned char *colormap_buffer = (unsigned char *)malloc(
	  colorspace_buffer_size(YUV422_PLANAR, m_cm->image_width(), m_cm->image_height()));
	...
	unsigned char *scaled_colormap_buffer =
	...
	Glib::RefPtr<Gdk::Pixbuf> colormap_image = Gdk::Pixbuf::create_from_data(
	  m_colormap_img_buf, Gdk::COLORSPACE_RGB, false, 8, img_width, img_height, 3 * img_width);
