...
FuseTransferWidget::FuseTransferWidget()
{
	...
	m_signal_update_local_lut_list.connect(
	  sigc::mem_fun(*this, &FuseTransferWidget::update_local_lut_list));
	m_signal_update_remote_lut_list.connect(
	  sigc::mem_fun(*this, &FuseTransferWidget::update_remote_lut_list));
 ...
 */
void
FuseTransferWidget::add_fountain_service(const char *name, const char *host_name, uint16_t port)
{
	ClientData data;
	data.client       = 0;
...
FuseTransferWidget::set_local_lut_list_trv(Gtk::TreeView *trv)
{
	...
	m_trv_local_lut_list->signal_cursor_changed().connect(
	  sigc::mem_fun(*this, &FuseTransferWidget::local_lut_selected));
}
...
FuseTransferWidget::set_remote_lut_list_trv(Gtk::TreeView *trv)
{
	...
	m_trv_remote_lut_list->signal_cursor_changed().connect(
	  sigc::mem_fun(*this, &FuseTransferWidget::remote_lut_selected));
}
...
FuseTransferWidget::remote_lut_selected()
{
	...
	try {
		...
		FUSE_lutdesc_message_t *lut_desc =
		  (FUSE_lutdesc_message_t *)malloc(sizeof(FUSE_lutdesc_message_t));
...
FuseTransferWidget::fuse_inbound_received(FuseNetworkMessage *m) throw()
{
	...
	case FUSE_MT_LUT:
		try {
			...
			if (lut_content->width() != 256 || lut_content->height() != 256) {
				m_signal_delete_client();
				break;
			}
	...
	case FUSE_MT_SET_LUT_FAILED: printf("LUT upload failed\n");
