...
ColorTrainWidget::set_reset_selection_btn(Gtk::Button *btn)
{
	...
	m_btn_reset_selection->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::reset_selection));
}
...
ColorTrainWidget::set_add_to_colormap_btn(Gtk::Button *btn)
{
	...
	m_btn_add_to_colormap->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::add_to_colormap));
}
...
ColorTrainWidget::set_reset_colormap_btn(Gtk::Button *btn)
{
	...
	m_btn_reset_colormap->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::reset_colormap));
}
...
ColorTrainWidget::set_load_histos_btn(Gtk::Button *btn)
{
	...
	m_btn_load_histos->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::load_histograms));
}
...
ColorTrainWidget::set_save_histos_btn(Gtk::Button *btn)
{
	...
	m_btn_save_histos->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::save_histograms));
}
...
ColorTrainWidget::set_load_colormap_btn(Gtk::Button *btn)
{
	...
	m_btn_load_colormap->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::load_colormap));
}
...
ColorTrainWidget::set_save_colormap_btn(Gtk::Button *btn)
{
	...
	m_btn_save_colormap->signal_clicked().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::save_colormap));
}
...
ColorTrainWidget::set_segmentation_img(Gtk::Image *img)
{
	...
	m_img_segmentation->signal_size_allocate().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::resize_seg_image));
}
...
{
	m_scl_threshold = scl;
	m_scl_threshold->signal_change_value().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::set_threshold));
}
...
{
	m_scl_min_prob = scl;
	m_scl_min_prob->signal_change_value().connect(
	  sigc::mem_fun(*this, &ColorTrainWidget::set_min_prob));
}
 ...
 */
void
ColorTrainWidget::set_cm_selector(Gtk::SpinButton *depth,
...
ColorTrainWidget::load_histograms()
{
	...
	case (Gtk::RESPONSE_OK): {
		...
		if (m_spbtn_cm_depth)
			m_spbtn_cm_depth->set_value(log(cur->depth()) / log(2));
		if (m_spbtn_cm_width)
			m_spbtn_cm_width->set_value(log(cur->width()) / log(2));
		if (m_spbtn_cm_height)
			m_spbtn_cm_height->set_value(log(cur->height()) / log(2));
...
{
	Gtk::MessageDialog dialog(*m_wnd_parent,
...
ColorTrainWidget::load_colormap()
{
	...
	case (Gtk::RESPONSE_OK): {
		...
		*current               = *tycm;
		...
		if (m_spbtn_cm_depth)
			m_spbtn_cm_depth->set_value(log(cm_depth) / log(2));
		if (m_spbtn_cm_width)
			m_spbtn_cm_width->set_value(log(cm_width) / log(2));
		if (m_spbtn_cm_height)
			m_spbtn_cm_height->set_value(log(cm_height) / log(2));
...
ColorTrainWidget::draw_segmentation_result()
{
	...
	unsigned char *rgb_buffer = (unsigned char *)malloc(colorspace_buffer_size(RGB, width, height));
	convert(m_img_cs, RGB, scaled_buffer, rgb_buffer, width, height);

	Glib::RefPtr<Gdk::Pixbuf> image =
	  Gdk::Pixbuf::create_from_data(rgb_buffer,
	                                Gdk::COLORSPACE_RGB,
	                                false,
	                                8,
	                                width,
 ...
 */
void
ColorTrainWidget::free_rgb_buffer(const guint8 *rgb_buffer)
{
