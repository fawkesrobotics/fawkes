...
main(int argc, char **argv)
{
	...
	if (argp.has_arg("o")) {
		buf = new SharedMemoryImageBuffer(argp.arg("o"),
		                                  cam->colorspace(),
		                                  cam->pixel_width(),
		                                  cam->pixel_height());
	}
	...
	if (verbose) {
		printf("Camera opened, settings:\n"
		       ...
		       "  Delay:       %i ms\n",
		       cam->colorspace(),
		       colorspace_to_string(cam->colorspace()),
		       cam->pixel_width(),
		       cam->pixel_height(),
		...
		if (SDL_WaitEvent(&event)) {
			...
			case SDL_KEYUP:
				if (event.key.keysym.sym == SDLK_SPACE) {
					...
					if (cam->buffer() != NULL) {
						if (buf)
							memcpy(buf->buffer(), cam->buffer(), cam->buffer_size());
#ifdef HAVE_RECTINFO
						if (rectifying) {
							convert(cam->colorspace(),
							        YUV422_PLANAR,
							        cam->buffer(),
							        unfiltered_buffer,
							        cam->pixel_width(),
...
#		if GTK_VERSION_GE(3, 0)
							Glib::RefPtr<Gtk::FileFilter> filter_rectinfo = Gtk::FileFilter::create();
...
#		endif
							...
							if (result == Gtk::RESPONSE_OK) {
								...
								try {
									...
									for (RectificationInfoFile::RectInfoBlockVector::iterator b = blocks->begin();
									     b != blocks->end();
									     ++b) {
...
#		endif
									...
									blocks->erase(
									  bi); // needs to be erased because otherwise it would be deleted by following delete
									delete blocks;
