...
class FireVisionNetworkTool : public FuseClientHandler, public ServiceBrowseHandler
{
public:
	/** Constructor.
		...
		case FUSE_MT_IMAGE:
			...
			try {
				...
				if (ic->format() == FUSE_IF_RAW) {
					FvRawWriter *w = new FvRawWriter(file_,
					                                 ic->pixel_width(),
					                                 ic->pixel_height(),
					                                 (colorspace_t)ic->colorspace(),
					                                 ic->buffer());
					w->write();
					delete w;
   ...
   */
	...
	{
		FUSE_imagereq_message_t *idm =
		  (FUSE_imagereq_message_t *)malloc(sizeof(FUSE_imagereq_message_t));
   ...
   */
	...
	run()
	{
		if (argp_->has_arg("h")) {
			...
			if (argp_->has_arg("i") || argp_->has_arg("j") || argp_->has_arg("c")
			    || argp_->has_arg("C")) {
