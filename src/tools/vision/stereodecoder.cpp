 ...
 * */
void
interleave_yuv422planar(unsigned char *yuv422_first,
	...
	try {
		...
		for (list<string>::iterator f = files.begin(); f != files.end(); ++f) {
			...
			unsigned char *rgb = (unsigned char *)malloc(
			  colorspace_buffer_size(RGB, fvraw->pixel_width(), fvraw->pixel_height()) * 2);
			unsigned char *deinterlaced =
			  (unsigned char *)malloc((size_t)fvraw->pixel_width() * (size_t)fvraw->pixel_height() * 2);
			unsigned char *yuv             = (unsigned char *)malloc_buffer(YUV422_PLANAR,
                                                          fvraw->pixel_width(),
                                                          fvraw->pixel_height() * 2);
			unsigned char *yuv_interleaved = (unsigned char *)malloc_buffer(YUV422_PLANAR,
			                                                                fvraw->pixel_width(),
			                                                                fvraw->pixel_height() * 2);
			...
			Bumblebee2Camera::deinterlace_stereo(raw16,
			                                     deinterlaced,
			                                     fvraw->pixel_width(),
			                                     fvraw->pixel_height());
			Bumblebee2Camera::decode_bayer(
			  deinterlaced, rgb, fvraw->pixel_width(), fvraw->pixel_height(), BAYER_PATTERN_BGGR);
			/*
      ...
      */
			convert(RGB, YUV422_PLANAR, rgb, yuv, fvraw->pixel_width(), fvraw->pixel_height());
