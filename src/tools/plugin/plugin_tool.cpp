...
PluginTool::list_avail()
{
	...
	FawkesNetworkMessage *msg =
	  new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_LIST_AVAIL);
	c->enqueue(msg);
...
PluginTool::list_loaded()
{
	...
	FawkesNetworkMessage *msg =
	  new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_LIST_LOADED);
	c->enqueue(msg);
...
PluginTool::run()
{
	...
	case M_LIST_AVAIL: list_avail(); break;

	case M_LIST_LOADED: list_loaded(); break;

	case M_WATCH: watch(); break;

	default: print_usage(program_name_);
	}
