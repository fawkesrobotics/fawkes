...
SkillGuiGraphViewport::SkillGuiGraphViewport()
{
	...
	if (w) {
		fcd_ = new Gtk::FileChooserDialog(*w, "Save Graph", Gtk::FILE_CHOOSER_ACTION_SAVE);
		fcd_->set_transient_for(*w);
	} else {
		fcd_ = new Gtk::FileChooserDialog("Save Graph", Gtk::FILE_CHOOSER_ACTION_SAVE);
	}
	...
	signal_size_allocate().connect_notify(
	  sigc::hide(sigc::mem_fun(*this, &SkillGuiGraphViewport::render)));
