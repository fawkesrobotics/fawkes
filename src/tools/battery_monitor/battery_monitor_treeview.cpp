...
BatteryMonitorTreeView::~BatteryMonitorTreeView()
{
	...
	for (biit = m_battery_interfaces.begin(); biit != m_battery_interfaces.end(); ++biit) {
	...
	for (i = m_interface_dispatcher.begin(); i != m_interface_dispatcher.end(); ++i) {
		delete i->second;
	}

	// delete remote blackboards
	for (std::map<string, BlackBoard *>::iterator i = m_remote_bbs.begin(); i != m_remote_bbs.end();
	     ++i) {
		delete i->second;
	}
...
BatteryMonitorTreeView::add_host(const char *h)
{
	...
	// no battery interface opened, yet
	{
		try {
			...
			id->signal_data_changed().connect(
			  sigc::mem_fun(*this, &BatteryMonitorTreeView::on_data_changed));
			id->signal_writer_added().connect(
			  sigc::mem_fun(*this, &BatteryMonitorTreeView::on_writer_added));
			id->signal_writer_removed().connect(
			  sigc::mem_fun(*this, &BatteryMonitorTreeView::on_writer_removed));
