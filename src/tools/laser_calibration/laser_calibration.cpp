...
LaserCalibration::laser_to_pointcloud(const LaserInterface &laser)
{
	...
	for (uint i = 0; i < laser.maxlenof_distances(); i++) {
		cloud->points[i].x = distances[i] * cosf(deg2rad(i) * (360. / laser.maxlenof_distances()));
		cloud->points[i].y = distances[i] * sinf(deg2rad(i) * (360. / laser.maxlenof_distances()));
	}
	return cloud;
}
 ...
 */
void
LaserCalibration::transform_pointcloud(const string &target_frame, PointCloudPtr cloud)
{
	for (auto &point : cloud->points) {
		tf::Stamped<tf::Point> point_in_laser_frame(tf::Point(point.x, point.y, point.z),
		                                            fawkes::Time(0., cloud->header.stamp),
		                                            cloud->header.frame_id);
		tf::Stamped<tf::Point> point_in_base_frame;
		tf_transformer_->transform_point(target_frame, point_in_laser_frame, point_in_base_frame);
...
LaserCalibration::get_mean_z(PointCloudPtr cloud)
{
	if (cloud->points.size() < min_points) {
		stringstream error;
		error << "Not enough laser points in rear cloud, got " << cloud->size() << ", need "
		      << min_points;
 ...
 */
float
LaserCalibration::get_matching_cost(PointCloudPtr cloud1, PointCloudPtr cloud2, float *rot_yaw)
{
	if (cloud1->points.size() < min_points || cloud2->points.size() < min_points) {
		stringstream error;
		error << "Not enough points, got " << cloud1->points.size() << " and " << cloud2->points.size()
		      << " points, need " << min_points;
		throw InsufficientDataException(error.str().c_str());
	}
	...
	if (rot_yaw) {
		pcl::Registration<Point, Point, float>::Matrix4 transformation = icp.getFinalTransformation();
