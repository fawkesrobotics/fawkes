...
: Gtk::Window(cobject)
{
	...
	avahi_dispatcher->signal_service_added().connect(
	  sigc::retype_return<void>(sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_service_added)));
	avahi_dispatcher->signal_service_removed().connect(
	  sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_service_removed));
	...
	tb_connection->signal_clicked().connect(
	  sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_connection_clicked));
...
NetLogGuiGtkWindow::on_connection_clicked()
{
	...
	if (ssd.run()) {
		...
		try {
			...
			NetworkService *service = new NetworkService(name.c_str(),
			                                             "_fawkes._tcp",
			                                             "",
			                                             hostname.c_str(),
			                                             port,
			...
			if (page >= 0) {
				Gtk::ScrolledWindow *scrolled =
				  dynamic_cast<Gtk::ScrolledWindow *>(ntb_logviewers.get_nth_page(page));
...
NetLogGuiGtkWindow::on_service_added(fawkes::NetworkService *service)
{
	...
	button->signal_clicked().connect(
	  sigc::bind(sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_connbut_clicked), image, logview));
	logview->get_connection_dispatcher()->signal_connected().connect(
	  sigc::bind(sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_connected), image));
	logview->get_connection_dispatcher()->signal_disconnected().connect(
	  sigc::bind(sigc::mem_fun(*this, &NetLogGuiGtkWindow::on_disconnected), image));
