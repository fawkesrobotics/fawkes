...
function use_skill(skill_name, iface_name)
    ...
    M.depends_interfaces = { -- {v = iface_name, type = "JacoInterface"},
   ...
   jaco{arm="left" | "right", ...}
    ...
    local function jc_arm_is_final(state)
        iface:read()
        return state.fsm.vars.msgid == iface:msgid() and iface:is_final()
    end
    ...
    local function jc_iface_no_writer(state) return not iface:has_writer() end
    ...
    M.fsm:add_transitions{
        ...
        {"GOTO_RETRACT", "GOTO", cond = true, desc = "params set"}, {
            "GOTO",
            "FAILED",
            precond = "vars.x == nil or vars.y == nil or vars.z == nil",
            desc = "insufficient params"
        ...
        {"PARAMS", "CHECK_FINAL", cond = true, desc = "msg sent"}, {
            "CHECK_FINAL",
            "FINAL",
            precond = "vars.no_wait",
            desc = "skip final checking"
        },
        {
            "CHECK_FINAL",
            "CHECK_ERROR",
            cond = jc_arm_is_final,
            desc = "arm final"
   ...
   --]]
    ...
    function M.GRIPPER.init(self)
        self.error = false
        local f1, f2, f3
        if type(self.fsm.vars.gripper) == "table" and self.fsm.vars.gripper.f1 ~=
            nil and self.fsm.vars.gripper.f2 ~= nil and self.fsm.vars.gripper.f3 ~=
            nil then
