...
fsm:add_transitions{
    {
        ...
        "DECIDE_MODE",
        "TURNONOFF",
        precond = "vars.enable ~= nil",
        desc = "enable parm"
    },
    {
        "DECIDE_MODE",
        "CALIBRATE",
        precond = "vars.calibrate",
        desc = "calib parm"
    }, {
        "DECIDE_MODE",
        "VELOCITY",
        precond = "vars.velocity ~= nil",
        desc = "max velocity"
    }, {
        "DECIDE_MODE",
        "GOTO",
        ...
        "DECIDE_MODE",
        "PLANNERPARAMS",
        precond = "vars.plannerparams ~= nil",
        desc = "planner params"
    }, {"DECIDE_MODE", "STOP", precond = "vars.stop"},
    {"DECIDE_MODE", "PARK", precond = "vars.park"},
    {"DECIDE_MODE", "GRIPPER", precond = "vars.gripper"}, {
        "DECIDE_MODE",
        "MOVE",
        precond = "vars.move and vars.nr and (vars.enc or vars.angle)"
    ...
    {"MOVE", "FAILED", cond = jc_next_msg, desc = "next msg"}, {
        "CHECKERR",
        "FINAL",
        cond = "katanaarm:error_code() == katanaarm.ERROR_NONE",
        desc = "no error"
    }, {"CHECKERR", "FAILED", cond = true, desc = "error"}
}

function CALIBRATE:init()
    self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(
                              katanaarm.CalibrateMessage:new())
end

function TURNONOFF:init()
    katanaarm:msgq_enqueue_copy(katanaarm.SetEnabledMessage:new(self.fsm.vars
                                                                    .enable))
end

function STOP:init() katanaarm:msgq_enqueue_copy(katanaarm.StopMessage:new()) end

function VELOCITY:init()
    katanaarm:msgq_enqueue_copy(katanaarm.SetMaxVelocityMessage:new(self.fsm
                                                                        .vars
                                                                        .velocity))
end
...
function GOTO:init()
    ...
    if math.abs(x) > 5 or math.abs(y) > 5 or math.abs(z) > 5 then
        ...
        local gm = katanaarm.LinearGotoMessage:new(theta_error, offset,
                                                   straight, frame, rot_frame,
                                                   x, y, z, phi, theta, psi)
        self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(gm)
    end
end
...
function MOVE:init()
    if self.fsm.vars.enc then
        if self.fsm.vars.rel then
            self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(
                                      katanaarm.MoveMotorEncoderMessage:new(
                                          self.fsm.vars.nr, self.fsm.vars.enc))
        else
            self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(
                                      katanaarm.SetMotorEncoderMessage:new(
                                          self.fsm.vars.nr, self.fsm.vars.enc))
        end
    else
        if self.fsm.vars.rel then
            self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(
                                      katanaarm.MoveMotorAngleMessage:new(
                                          self.fsm.vars.nr, self.fsm.vars.angle))
        else
            self.fsm.vars.msgid = katanaarm:msgq_enqueue_copy(
                                      katanaarm.SetMotorAngleMessage:new(
                                          self.fsm.vars.nr, self.fsm.vars.angle))
        end
    end
end
