...
StaticTransformsThread::entries_get_from_config()
{
		...
		if ((transforms.find(cfg_name) == transforms.end())
			...
			if (active) {
				try {
					...
					std::string child_frame = config->get_string((cfg_prefix + "child_frame").c_str());

					if (frame[0] == '/') {
						logger->log_warn(name(),
						                 "Transform %s parent frame %s starts with /,"
					...
					if (child_frame[0] == '/') {
						logger->log_warn(name(),
						                 "Transform %s child frame %s starts with /,"
						                 "removing leading slash.",
						                 cfg_name.c_str(),
						                 frame.c_str());
					...
					float rx = 0., ry = 0., rz = 0., rw = 1., ryaw = 0., rpitch = 0., rroll = 0.;
	...
	for (std::list<Entry>::iterator i = entries_.begin(); i != entries_.end(); ++i) {
		...
		tf_publishers[i->transform->child_frame_id]->send_transform(*(i->transform),
		                                                            /* is_static */ true);
	}
}
