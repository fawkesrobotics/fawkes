 ...
 */
BlackBoardLogReplayPlugin::BlackBoardLogReplayPlugin(Configuration *config) : Plugin(config)
{
	...
	if (err != 0) {
		char      buf[1024];
		Exception se("Cannot access logdir %s (%s)", logdir.c_str(), strerror_r(errno, buf, 1024));
		...
		if (logs.find(log_name) == logs.end()) {
			...
			if (hook_str != "") {
				...
				if (hook_str == "pre_loop") {
					...
					throw Exception("Invalid hook '%s' for %s", hook_str.c_str(), i->get_string().c_str());
				}
				...
				BBLogReplayThread *lr_thread = new BBLogReplayThread(
				  i->get_string().c_str(), logdir.c_str(), scenario.c_str(), grace_period, loop_replay);
