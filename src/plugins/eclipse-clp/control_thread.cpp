...
AgentControlThread::init()
{
	...
	try {
		...
		simulation_shutdown_script_ =
		  strdup(config->get_string("/eclipse-clp/gazebo/simulation-shutdown-script").c_str());
	} catch (...) {
		logger->log_error(name(),
		                  "error reading config value: /eclipse-clp/gazebo/simulation-shutdown-script");
		allow_shutdown_ = false;
	}
...
AgentControlThread::loop()
{
	...
	while (!m_test_iface->msgq_empty()) {
		if (m_test_iface->msgq_first_is<TestInterface::CalculateMessage>()) {
			TestInterface::CalculateMessage *msg =
			  m_test_iface->msgq_first<TestInterface::CalculateMessage>();
