...
/** Creates the initial EclExternalBlackBoard object
   ...
   */
void
EclExternalBlackBoard::create_initial_object(BlackBoard *bb, Logger *logger)
{
...
EclExternalBlackBoard::instance()
{
	return m_instance;
}
...
/** Access the BlackBoard instance.
   ...
   */
BlackBoard *
EclExternalBlackBoard::blackboard_instance()
{
	return m_blackboard;
}
...
EclExternalBlackBoard::interfaces()
{
	return m_interfaces;
}
	...
	if (interfaces.find(uid) == interfaces.end()) {
		try {
			...
			if (0 == strcmp("w", mode.name())) {
				iface =
				  EclExternalBlackBoard::instance()->blackboard_instance()->open_for_writing(interface_type,
				                                                                             interface_id);
			} else {
				iface =
				  EclExternalBlackBoard::instance()->blackboard_instance()->open_for_reading(interface_type,
				                                                                             interface_id);
			}
...
{
	for (std::map<std::string, Interface *>::iterator it =
	       EclExternalBlackBoard::instance()->interfaces().begin();
	...
	if (interfaces.find(uid) != interfaces.end()) {
		...
		for (fit = iface->fields(); fit != iface->fields_end(); ++fit) {
			if (0 == strcmp(field, fit.get_name())) {
				...
				default:
					fprintf(stderr,
					        "p_bb_get(): could not find type of interface! Type: %s (%d)",
					        fit.get_typename(),
					        fit.get_type());
					break;
				}
	...
	if (interfaces.find(uid) != interfaces.end()) {
		...
		for (fit = iface->fields(); fit != iface->fields_end(); ++fit) {
			if (0 == strcmp(field, fit.get_name())) {
				...
				case IFT_BYTE:
				case IFT_ENUM: fprintf(stderr, "p_bb_set(): NOT YET IMPLEMENTET\n"); break;
	...
	if (interfaces.find(uid) != interfaces.end()) {
		...
		while (!iface->msgq_empty()) {
			...
			for (InterfaceFieldIterator fit = msg->fields(); fit != msg->fields_end(); ++fit) {
				...
				case IFT_INT8: value = EC_word((long)fit.get_int8()); break;

				case IFT_UINT8: value = EC_word((long)fit.get_uint8()); break;

				case IFT_INT16: value = EC_word((long)fit.get_int16()); break;

				case IFT_UINT16: value = EC_word((long)fit.get_uint16()); break;

				case IFT_INT32: value = EC_word((long)fit.get_int32()); break;

				case IFT_UINT32: value = EC_word((long)fit.get_uint32()); break;

				case IFT_INT64: value = EC_word((long)fit.get_int64()); break;

				case IFT_UINT64: value = EC_word((long)fit.get_uint64()); break;

				case IFT_FLOAT: value = EC_word((double)fit.get_float()); break;

				case IFT_STRING: value = EC_word(fit.get_string()); break;

				case IFT_BYTE:
				case IFT_ENUM: fprintf(stderr, "p_bb_recv_messages(): NOT YET IMPLEMENTED\n"); break;

				default: fprintf(stderr, "p_bb_recv_messages(): unknown field type\n");
				}

				EC_word field = ::list(EC_word(fit.get_name()), ::list(value, nil()));
				args          = ::list(field, args);
			}

			// construct list of messages: [[MsgType, [[Key1, Val1], ...]], ... ]
			msg_list = ::list(::list(EC_word(msg->type()), ::list(args, nil())), msg_list);
...
p_bb_observe_pattern()
{
	...
	if (EC_succeed != EC_arg(1).is_string(&type_pattern)) {
		EclExternalBlackBoard::logger()->log_error(EclExternalBlackBoard::name(),
	...
	if (EC_succeed != EC_arg(2).is_string(&id_pattern)) {
		EclExternalBlackBoard::logger()->log_error(EclExternalBlackBoard::name(),
		                                           "%s: Second argument must be a string.",
		                                           __func__);
		return EC_fail;
	}
...
p_bb_listen_for_change()
{
	...
	if (iface_it == interfaces.end()) {
		EclExternalBlackBoard::logger()->log_error(EclExternalBlackBoard::name(),
		                                           "%s: Interface %s has not been opened.",
		                                           __func__,
		                                           uid.c_str());
		return EC_fail;
	}
...
process_message_args(Message *msg, EC_word arg_list)
{
	...
	for (; EC_succeed == arg_list.is_list(head, tail); arg_list = tail) {
		...
		if (EC_succeed != head.is_list(field, t1) || EC_succeed != t1.is_list(value, t2)) {
		...
		InterfaceFieldIterator fit;
		for (fit = msg->fields(); fit != msg->fields_end(); ++fit) {
			if (0 == strcmp(fit.get_name(), field_name)) {
				...
				case IFT_BYTE:
				case IFT_ENUM: fprintf(stderr, "p_bb_send_message(): NOT YET IMPLEMENTET\n"); break;
