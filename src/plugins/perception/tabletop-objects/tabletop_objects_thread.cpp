...
TabletopObjectsThread::init()
{
	...
	if (pcl_manager->exists_pointcloud<PointType>(cfg_input_pointcloud_.c_str())) {
		...
		fcoloredinput_ = pcl_manager->get_pointcloud<ColorPointType>(cfg_input_pointcloud_.c_str());
...
bool
TabletopObjectsThread::is_polygon_edge_better(PointType &cb_br_p1p,
                                              PointType &cb_br_p2p,
                                              PointType &br_p1p,
                                              PointType &br_p2p)
{
...
TabletopObjectsThread::loop()
{
	...
	*last_pcl_time_ = pcl_time;
	...
	while (!happy_with_plane) {
		...
		if (temp_cloud->points.size() <= 10) {
			logger->log_warn(name(),
			                 "[L %u] no more points for plane detection, skipping loop",
			                 loop_count_);
		...
		if ((double)inliers->indices.size()
		    < (cfg_segm_inlier_quota_ * (double)temp_cloud->points.size())) {
			logger->log_warn(
			  name(),
			  "[L %u] no table in scene, skipping loop (%zu inliers, required %f, voxelized size %zu)",
			  loop_count_,
			  inliers->indices.size(),
			  (cfg_segm_inlier_quota_ * temp_cloud->points.size()),
		...
		try {
			tf::Stamped<tf::Vector3> table_normal(tf::Vector3(coeff->values[0],
			                                                  coeff->values[1],
			                                                  coeff->values[2]),
			                                      fawkes::Time(0, 0),
			...
			if (fabs(z_axis.angle(baserel_normal)) > cfg_max_z_angle_deviation_) {
				...
				logger->log_warn(name(),
				                 ...
				                 baserel_normal.y(),
				                 baserel_normal.z(),
				                 z_axis.angle(baserel_normal),
				                 cfg_max_z_angle_deviation_);
			}
		} catch (Exception &e) {
		...
		if (happy_with_plane) {
			...
			try {
				...
				tf::Stamped<tf::Point> centroid(tf::Point(table_centroid[0],
				                                          table_centroid[1],
				                                          table_centroid[2]),
				                                fawkes::Time(0, 0),
				                                input_->header.frame_id);
				...
				if ((baserel_centroid.z() < cfg_table_min_height_)
					...
					happy_with_plane = false;
					logger->log_warn(name(),
					                 "[L %u] table height %f not in range [%f, %f]",
					                 loop_count_,
					                 baserel_centroid.z(),
					                 cfg_table_min_height_,
					                 cfg_table_max_height_);
				}
	...
	pcl::search::KdTree<PointType>::Ptr kdtree_table(new pcl::search::KdTree<PointType>());
	...
	table_ec.setMinClusterSize(cfg_table_min_cluster_quota_ * cloud_table_voxelized->points.size());
	...
	if (!table_cluster_indices.empty()) {
		...
		pcl::PointIndices::ConstPtr table_cluster_indices_ptr(
		...
		*cloud_proj_ = *cloud_table_extracted;
		...
		logger->log_info(name(),
		                 "[L %u] table plane clustering did not generate any clusters",
		                 loop_count_);
	}
...
#	endif
	...
	*cloud_hull_ = *simplified_polygon;
	...
	try {
		...
		tf_listener->lookup_transform(cfg_base_frame_, input_->header.frame_id, input_time, t);
		...
		for (size_t i = 0; i < psize; ++i) {
			...
			if (!(((left_frustrum_normal.dot(p1) < 0.03) && (left_frustrum_normal.dot(p2) < 0.03))
				...
				if ((lower_frustrum_normal.dot(p1) < 0.01) && (lower_frustrum_normal.dot(p2) < 0.01)) {
					// it's a lower frustrum line, keep just in case we do not
					// find a better one
					if ((lf_pidx1 == std::numeric_limits<size_t>::max())
					    || is_polygon_edge_better(br_p1p,
					                              br_p2p,
					                              baserel_polygon_cloud->points[lf_pidx1],
					                              baserel_polygon_cloud->points[lf_pidx2])) {
		...
		if (cfg_table_model_enable_ && (pidx1 != std::numeric_limits<size_t>::max())
			...
			for (unsigned int i = 0; i < 3; ++i)
				table_centroid[i] = table_center[i];
			table_centroid[3] = 0.;
			...
			Eigen::Affine3f affine =
			  Eigen::Translation3f(table_centroid.x(), table_centroid.y(), table_centroid.z())
			  * Eigen::AngleAxisf(angle, rotaxis);

			Eigen::Vector3f model_p1(-cfg_table_model_width_ * 0.5, cfg_table_model_length_ * 0.5, 0.),
			...
			affine = Eigen::Translation3f(table_centroid.x(), table_centroid.y(), table_centroid.z())
			         * Eigen::AngleAxisf(angle_p1_p2, model_rotaxis) * Eigen::AngleAxisf(angle, rotaxis);

			// Just the rotational part
			Eigen::Quaternionf qt;
			qt = Eigen::AngleAxisf(angle_p1_p2, model_rotaxis) * Eigen::AngleAxisf(angle, rotaxis);
			...
			CloudPtr table_model = generate_table_model(cfg_table_model_length_,
			                                            cfg_table_model_width_,
			                                            cfg_table_model_step_);
	...
	try {
		tf::Stamped<tf::Point> origin(tf::Point(0, 0, 0), fawkes::Time(0, 0), input_->header.frame_id);
		...
		logger->log_warn(name(), "[L %u] could not transform viewpoint to base link", loop_count_);
	}
	...
	pcl::ComparisonOps::CompareOp op =
	  viewpoint_above ? (coeff->values[3] > 0 ? pcl::ComparisonOps::GT : pcl::ComparisonOps::LT)
	                  : (coeff->values[3] < 0 ? pcl::ComparisonOps::GT : pcl::ComparisonOps::LT);
	pcl_utils::PlaneDistanceComparison<PointType>::ConstPtr above_comp(
	  new pcl_utils::PlaneDistanceComparison<PointType>(coeff, op));
	pcl::ConditionAnd<PointType>::Ptr above_cond(new pcl::ConditionAnd<PointType>());
	...
	pcl::ConditionAnd<PointType>::Ptr polygon_cond(new pcl::ConditionAnd<PointType>());

	pcl_utils::PolygonComparison<PointType>::ConstPtr inpoly_comp(
	  new pcl_utils::PolygonComparison<PointType>(
	    (model_cloud_hull_ && !model_cloud_hull_->points.empty()) ? *model_cloud_hull_
	                                                              : *cloud_hull_));
	...
	for (OldCentroidVector::iterator it = old_centroids_.begin(); it != old_centroids_.end(); ++it) {
		it->increment_age();
	}
...
TabletopObjectsThread::extract_object_clusters(CloudConstPtr input)
{
	...
	pcl::search::KdTree<PointType>::Ptr kdtree_cl(new pcl::search::KdTree<PointType>());
                                       ...
                                       std::vector<ColorCloudPtr> &tmp_obj_clusters)
{
	...
	if (num_points > 0) {
		...
		for (it = cluster_indices.begin(); it != cluster_indices.end() && centroid_i < MAX_CENTROIDS;
		     ++it, ++centroid_i) {
			...
			pcl_utils::transform_pointcloud(cfg_base_frame_,
			                                *single_cluster,
			                                *obj_in_base_frame,
			                                *tf_listener);
			...
			if (cfg_cylinder_fitting_) {
				new_centroids[centroid_i] =
				  fit_cylinder(obj_in_base_frame, new_centroids[centroid_i], centroid_i);
			}
		}
...
TabletopObjectsThread::ColorCloudPtr
TabletopObjectsThread::colorize_cluster(CloudConstPtr           input_cloud,
  ...
  std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> &scores)
{
	...
	for (int i = 0; i < NUM_KNOWN_OBJS_; i++) {
		scores[i][0] = compute_similarity(cluster_dim[0], known_obj_dimensions_[i][0]);
		scores[i][1] = compute_similarity(cluster_dim[1], known_obj_dimensions_[i][1]);
		scores[i][2] = compute_similarity(cluster_dim[2], known_obj_dimensions_[i][2]);
	}
	return true;
}
                                    ...
                                    const Eigen::Quaternionf &   attitude)
{
	...
	try {
		tf::Stamped<tf::Pose> spose(
		  tf::Pose(tf::Quaternion(attitude.x(), attitude.y(), attitude.z(), attitude.w()),
...
TabletopObjectsThread::generate_table_model(const float length,
                                            const float width,
	...
	for (unsigned int t = 0; t < num_t; ++t) {
		for (unsigned int w = 0; w < num_w; ++w) {
			for (unsigned int h = 0; h < num_h; ++h) {
				...
				p.x = h * step - width_2;
				if ((h == num_h - 1) && fabs(p.x - width_2) > max_error)
					p.x = width_2;

				p.y = w * step - length_2;
				if ((w == num_w - 1) && fabs(p.y - length_2) > max_error)
					p.y = length_2;

				p.z = t * step - thickness_2;
				if ((t == num_t - 1) && fabs(p.z - thickness_2) > max_error)
					p.z = thickness_2;
			}
		}
	}
...
void
TabletopObjectsThread::delete_old_centroids(OldCentroidVector centroids, unsigned int age)
{
	centroids.erase(std::remove_if(centroids.begin(),
	                               centroids.end(),
...
void
TabletopObjectsThread::delete_near_centroids(CentroidMap       reference,
                                             OldCentroidVector centroids,
                                             float             min_distance)
{
	centroids.erase(std::remove_if(centroids.begin(),
	                               centroids.end(),
	                               [&](const OldCentroid &old) -> bool {
		                               for (CentroidMap::const_iterator it = reference.begin();
		                                    it != reference.end();
		                                    ++it) {
			                               if (pcl::distances::l2(it->second, old.get_centroid())
			                                   < min_distance) {
...
void
TabletopObjectsThread::remove_high_centroids(Eigen::Vector4f table_centroid, CentroidMap centroids)
{
	tf::Stamped<tf::Point> sp_baserel_table;
	tf::Stamped<tf::Point> sp_table(tf::Point(table_centroid[0],
	                                          table_centroid[1],
	                                          table_centroid[2]),
	                                fawkes::Time(0, 0),
	                                input_->header.frame_id);
	try {
		...
		for (CentroidMap::iterator it = centroids.begin(); it != centroids.end();) {
			try {
				tf::Stamped<tf::Point> sp_baserel_centroid(tf::Point(it->second[0],
				                                                     it->second[1],
				                                                     it->second[2]),
				                                           fawkes::Time(0, 0),
				                                           cfg_base_frame_);
                                    ...
                                    uint const &           centroid_i)
{
	...
	logger->log_debug(
	  name(), "Computed object dimensions: %f %f %f", obj_dim[0], obj_dim[1], obj_dim[2]);
	logger->log_debug(name(), "Size similarity to known objects:");
	for (int os = 0; os < NUM_KNOWN_OBJS_; os++) {
		logger->log_debug(name(),
		                  "** Cup %i: %f in x, %f in y, %f in z.",
		                  os,
		                  obj_size_scores[os][0],
		                  obj_size_scores[os][1],
	...
	pcl::search::KdTree<ColorPointType>::Ptr tree_cyl(new pcl::search::KdTree<ColorPointType>());
	...
	if (cloud_cylinder_baserel->points.empty()) {
		...
		obj_shape_confidence_[centroid_i] =
		  (double)(cloud_cylinder_baserel->points.size()) / (obj_in_base_frame->points.size() * 1.0);
		logger->log_debug(name(),
		                  "Cylinder fit confidence = %zu/%zu = %f",
		...
		if (cfg_verbose_cylinder_fitting_) {
			logger->log_debug(name(),
			                  ...
			                  pnt_max.z - pnt_min.z);
			logger->log_debug(name(), "Cylinder height according to bounding box: %f", obj_dim[2]);
			logger->log_debug(name(),
			                  "Cylinder radius according to cylinder fitting: %f",
			                  (*coefficients_cylinder).values[6]);
			logger->log_debug(name(), "Cylinder radius according to bounding box y: %f", obj_dim[1] / 2);
		}
		//Cylinder radius:
	...
	if (cfg_verbose_cylinder_fitting_) {
		logger->log_debug(name(), "Shape similarity = %f", obj_shape_confidence_[centroid_i]);
	}
	for (int os = 0; os < NUM_KNOWN_OBJS_; os++) {
		if (cfg_verbose_cylinder_fitting_) {
			logger->log_debug(name(), "** Similarity to known cup %i:", os);
			logger->log_debug(name(), "Size similarity  = %f", obj_likelihoods_[centroid_i][os]);
			obj_likelihoods_[centroid_i][os] =
			  (0.6 * obj_likelihoods_[centroid_i][os]) + (0.4 * obj_shape_confidence_[centroid_i]);
			logger->log_debug(name(), "Overall similarity = %f", obj_likelihoods_[centroid_i][os]);
		}
  ...
  std::vector<Eigen::Vector4f, Eigen::aligned_allocator<Eigen::Vector4f>> new_centroids)
{
	...
	if (first_run_) {
		...
		for (int row = 0; row < hp.num_rows; row++) { // new centroids
			unsigned int col = 0;
			for (CentroidMap::iterator col_it = centroids_.begin(); col_it != centroids_.end();
		...
		for (int row = 0; row < assignment_size; row++) {
			if (row >= hp.num_rows) { // object has disappeared
				...
				for (OldCentroidVector::iterator it = old_centroids_.begin(); it != old_centroids_.end();
				     ++it) {
