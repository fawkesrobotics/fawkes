...
public:
	...
	{
		float val =
		  (coeff_->values[0] * point.x + coeff_->values[1] * point.y + coeff_->values[2] * point.z
		   + coeff_->values[3])
		  / sqrtf(coeff_->values[0] * coeff_->values[0] + coeff_->values[1] * coeff_->values[1]
...
public:
	...
	: viewer("PCL OpenNI Planar Segmentation Viewer"), device_id_(device_id), new_cloud_(false)
	{
	...
	int
	get_nn(float x, float y, float z) const
	{
		PointType          p(x, y, z);
	...
	get()
	{
		...
		typename PlaneDistanceComparison<PointType>::ConstPtr above_comp(
		...
		if (!vertices_.empty()) {
			...
			typename PolygonComparison<PointType>::ConstPtr inpoly_comp(
		...
		pcl::KdTree<pcl::PointXYZRGB>::Ptr kdtree_cl(new pcl::KdTreeFLANN<pcl::PointXYZRGB>());
		...
		uint8_t colors[5][3] = {{255, 0, 0}, {0, 0, 255}, {255, 255, 0}, {255, 0, 255}, {0, 255, 255}};
...
*/
    ...
    */
		...
		viewer.runOnVisualizationThread(boost::bind(&OpenNIPlanarSegmentation::viz_cb, this, _1),
		                                "viz_cb");
	...
	if (driver.getNumberDevices() > 0) {
		for (unsigned deviceIdx = 0; deviceIdx < driver.getNumberDevices(); ++deviceIdx) {
			cout << "Device: " << deviceIdx + 1 << ", vendor: " << driver.getVendorName(deviceIdx)
			     ...
			     << driver.getSerialNumber(deviceIdx) << "\'" << endl;
			cout << "device_id may be #1, #2, ... for the first second etc device in the list or" << endl
			     << "                 bus@address for the device connected to a specific usb-bus / "
			        "address combination (wotks only in Linux) or"
			     << endl
