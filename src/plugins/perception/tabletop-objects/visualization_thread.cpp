...
TabletopVisualizationThread::init()
{
	...
	try {
		cfg_show_cvxhull_line_highlighting_ =
		  config->get_bool(CFG_PREFIX_VIS "show_convex_hull_line_highlighting");
	} catch (Exception &e) {
	} // ignored, use default
	try {
...
TabletopVisualizationThread::loop()
{
	...
	for (M_Vector4f::iterator it = centroids_.begin(); it != centroids_.end(); ++it) {
       ...
       */
			...
			tf::Stamped<tf::Point> centroid(tf::Point(it->second[0], it->second[1], it->second[2]),
			                                fawkes::Time(0, 0),
			                                cfg_base_frame_);
	...
	if (cfg_show_frustrum_ && !table_model_vertices_.empty()) {
		...
		upper_right_m = Eigen::AngleAxisf(-half_hva, Eigen::Vector3f::UnitZ())
		...
		upper_left_m = Eigen::AngleAxisf(half_hva, Eigen::Vector3f::UnitZ())
		               * Eigen::AngleAxisf(-half_vva, Eigen::Vector3f::UnitY());
		Eigen::Vector3f upper_left = upper_left_m * Eigen::Vector3f(4, 0, 0);

		Eigen::Matrix3f lower_right_m;
		lower_right_m = Eigen::AngleAxisf(-half_hva, Eigen::Vector3f::UnitZ())
		                * Eigen::AngleAxisf(half_vva, Eigen::Vector3f::UnitY());
		Eigen::Vector3f lower_right = lower_right_m * Eigen::Vector3f(2, 0, 0);

		Eigen::Matrix3f lower_left_m;
		lower_left_m = Eigen::AngleAxisf(half_hva, Eigen::Vector3f::UnitZ())
		               * Eigen::AngleAxisf(half_vva, Eigen::Vector3f::UnitY());
		...
		frustrum_triangles.points[0].x   = frustrum_triangles.points[3].x =
		  frustrum_triangles.points[6].x = 0.;
		frustrum_triangles.points[0].y   = frustrum_triangles.points[3].y =
		  frustrum_triangles.points[3].y = 0.;
		frustrum_triangles.points[0].z   = frustrum_triangles.points[3].z =
		  frustrum_triangles.points[3].z = 0.;
...
TabletopVisualizationThread::triangulate_hull()
{
	...
	// Compute convex hull
	int exitcode =
	  qh_new_qhull(dim, table_model_vertices_.size(), points, ismalloc, flags, outfile, errfile);
