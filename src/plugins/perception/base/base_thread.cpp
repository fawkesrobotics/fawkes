...
FvBaseThread::loop()
{
	...
	for (ait_ = aqts_.begin(); ait_ != aqts_.end();) {
		if (ait_->second->vision_threads->empty()
			...
			logger->log_info(name(), "Acquisition thread %s timed out, destroying", ait_->second->name());
	...
	while (stit != started_threads_.end()) {
		logger->log_info(name(),
		                 "Thread %s has been started, %zu",
		                 stit->second->name(),
		                 started_threads_.size());
		...
		if (stit->second->vision_threads->has_cyclic_thread()) {
			...
			if (stit->second->aqtmode() != FvAcquisitionThread::AqtCyclic) {
				logger->log_info(name(),
				                 "Switching acquisition thread %s to cyclic mode",
				                 stit->second->name());
			...
			if (stit->second->aqtmode() != FvAcquisitionThread::AqtContinuous) {
				logger->log_info(name(),
				                 "Switching acquisition thread %s to continuous mode",
				                 stit->second->name());
				stit->second->prepare_finalize();
				stit->second->cancel();
				stit->second->join();
...
Camera *
FvBaseThread::register_for_camera(const char *camera_string, Thread *thread, colorspace_t cspace)
{
	Camera *c = NULL;
	aqts_.lock();
	...
	try {
		...
		if (aqts_.find(id) != aqts_.end()) {
			...
			c = aqt->camera_instance(cspace,
			                         (vision_thread->vision_thread_mode() == VisionAspect::CONTINUOUS));
...
CameraControl *
FvBaseThread::acquire_camctrl(const char *cam_string, const std::type_info &typeinf)
{
	CameraArgumentParser cap(cam_string);
	std::string          id = cap.cam_type() + "." + cap.cam_id();
...
FvBaseThread::release_camctrl(CameraControl *cc)
{
	...
	if ((f = std::find(owned_controls_.begin(), owned_controls_.end(), cc))
	    != owned_controls_.end()) {
		delete *f;
...
FvBaseThread::unregister_thread(Thread *thread)
{
	...
	for (ait_ = aqts_.begin(); ait_ != aqts_.end(); ++ait_) {
		...
		if (ait_->second->vision_threads->empty()) {
			// Make thread stop capturing data
			logger->log_info(name(),
			                 "Disabling capturing on thread %s (no more threads)",
			                 ait_->second->name());
