...
: Thread("PointCloudDBMergeThread", Thread::OPMODE_WAITFORWAKEUP), MongoDBAspect("default")
{
}
...
PointCloudDBMergeThread::init()
{
	...
	pl_xyz_ = new PointCloudDBMergePipeline<pcl::PointXYZ>(
	  mongodb_client, config, logger, tf_listener, output_);

	pl_xyzrgb_ = new PointCloudDBMergePipeline<pcl::PointXYZRGB>(
	  mongodb_client, config, logger, tf_listener, output_);

	try {
		merge_if_ = blackboard->open_for_writing<PclDatabaseMergeInterface>("PCL Database Merge");
...
PointCloudDBMergeThread::loop()
{
	...
	if (PclDatabaseMergeInterface::MergeMessage *msg = merge_if_->msgq_first_safe(msg)) {
		...
		database   = (strcmp(msg->database(), "") != 0) ? msg->database() : cfg_database_;
  ...
  */
	...
	if (times.empty()) {
		logger->log_warn(name(), "Called for merge from %s, but no times given", collection.c_str());
