...
PointCloudDBROSCommThread::init()
{
	...
	merge_if_ = blackboard->open_for_reading<PclDatabaseMergeInterface>("PCL Database Merge");
	...
	store_if_ = blackboard->open_for_reading<PclDatabaseStoreInterface>("PCL Database Store");
	...
	*srv_merge_ =
	  rosnode->advertiseService("/pcl_db/merge", &PointCloudDBROSCommThread::merge_cb, this);

	*srv_retrieve_ =
	  rosnode->advertiseService("/pcl_db/retrieve", &PointCloudDBROSCommThread::retrieve_cb, this);

	*srv_store_ =
	  rosnode->advertiseService("/pcl_db/store", &PointCloudDBROSCommThread::store_cb, this);
}
                                    ...
                                    fawkes_msgs::MergePointClouds::Response &resp)
{
	...
	if (req.timestamps.size() > mm->maxlenof_timestamps()) {
		logger->log_warn(name(),
		                 "Number of requested timestamps (%zu) "
		                 "exceeds maximum number allowed (%zu)",
		                 req.timestamps.size(),
		                 mm->maxlenof_timestamps());
		resp.ok    = false;
	...
	for (size_t i = 0; i < req.timestamps.size(); ++i) {
		timestamps[i] =
		  (int64_t)req.timestamps[i].data.sec * 1000L + (int64_t)req.timestamps[i].data.nsec / 1000000L;
	}
                                       ...
                                       fawkes_msgs::RetrievePointCloud::Response &resp)
{
	...
	int64_t timestamp = (int64_t)req.timestamp.sec * 1000L + (int64_t)req.timestamp.nsec / 1000000L;

	logger->log_info(
	  name(), "Restoring %li from %s.%s", timestamp, req.database.c_str(), req.collection.c_str());
...
{
#if PCL_VERSION_COMPARE(>=, 1, 7, 0)
	PclDatabaseStoreInterface::StoreMessage *mm = new PclDatabaseStoreInterface::StoreMessage();
	...
	RefPtr<pcl::PointCloud<pcl::PointXYZ>>    pcl_xyz(new pcl::PointCloud<pcl::PointXYZ>());
	RefPtr<pcl::PointCloud<pcl::PointXYZRGB>> pcl_xyzrgb(new pcl::PointCloud<pcl::PointXYZRGB>());
