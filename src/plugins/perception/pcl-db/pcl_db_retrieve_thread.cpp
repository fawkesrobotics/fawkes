...
: Thread("PointCloudDBRetrieveThread", Thread::OPMODE_WAITFORWAKEUP), MongoDBAspect("default")
{
}
...
PointCloudDBRetrieveThread::init()
{
	...
	pl_xyz_ = new PointCloudDBRetrievePipeline<pcl::PointXYZ>(
	  mongodb_client, config, logger, tf_listener, original_, output_);

	pl_xyzrgb_ = new PointCloudDBRetrievePipeline<pcl::PointXYZRGB>(
	  mongodb_client, config, logger, tf_listener, original_, output_);
...
PointCloudDBRetrieveThread::loop()
{
	...
	if (retrieve_if_->msgq_empty())
		return;

	if (PclDatabaseRetrieveInterface::RetrieveMessage *msg = retrieve_if_->msgq_first_safe(msg)) {
