...
#ifndef _PLUGINS_PERCEPTION_PCL_DB_PCL_DB_MERGE_PIPELINE_H_
#define _PLUGINS_PERCEPTION_PCL_DB_PCL_DB_MERGE_PIPELINE_H_
...
public:
   ...
   */
	...
	: PointCloudDBPipeline<PointType>(mongodb_client, config, logger, output), tf_(transformer)
	{
		...
		cfg_transform_to_sensor_frame_ = config->get_bool(CFG_PREFIX_MERGE "transform-to-sensor-frame");
		...
		cfg_global_frame_            = config->get_string(CFG_PREFIX_MERGE "global-frame");
		cfg_passthrough_filter_axis_ = config->get_string(CFG_PREFIX_MERGE "passthrough-filter/axis");
		...
		cfg_icp_ransac_iterations_ = config->get_uint(CFG_PREFIX_MERGE "icp/ransac-iterations");
		cfg_icp_max_correspondance_distance_ =
		  config->get_float(CFG_PREFIX_MERGE "icp/max-correspondance-distance");
		cfg_icp_max_iterations_     = config->get_uint(CFG_PREFIX_MERGE "icp/max-iterations");
		cfg_icp_transformation_eps_ = config->get_float(CFG_PREFIX_MERGE "icp/transformation-epsilon");
		cfg_icp_euclidean_fitness_eps_ =
		  config->get_float(CFG_PREFIX_MERGE "icp/euclidean-fitness-epsilon");

		this->logger_->log_info(this->name_,
		                        "Age Tolerance: %li  "
		                        "Limits: [%f, %f]  tf range: [%li, %li]",
		                        this->cfg_pcl_age_tolerance_,
		                        cfg_passthrough_filter_limits_[0],
		                        cfg_passthrough_filter_limits_[1],
		                        this->cfg_transform_range_[0],
   ...
   */
	...
	merge(std::vector<long long> &times, std::string &database, std::string &collection)
	{
		...
		std::vector<typename PointCloudDBPipeline<PointType>::CloudPtr> non_aligned_downsampled(
		  num_clouds);
		std::vector<typename PointCloudDBPipeline<PointType>::CloudPtr> aligned_downsampled(num_clouds);
		std::vector<typename PointCloudDBPipeline<PointType>::CloudPtr> aligned_downsampled_remplane(
		  num_clouds);
		std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> transforms(num_clouds
		                                                                                   - 1);
		...
		pcls =
		  PointCloudDBPipeline<PointType>::retrieve_clouds(times, actual_times, database, collection);
		if (pcls.empty()) {
		...
		for (unsigned int i = 0; i < num_clouds; ++i) {
			// retrieve transforms
			fawkes::tf::MongoDBTransformer transformer(this->mongodb_client_, database);
		...
		if (use_alignment_) {
			...
			pass.setFilterLimits(cfg_passthrough_filter_limits_[0], cfg_passthrough_filter_limits_[1]);
			...
			typename PointCloudDBPipeline<PointType>::CloudPtr filtered_z(
			  new typename PointCloudDBPipeline<PointType>::Cloud());
			...
			for (unsigned int i = 0; i < num_clouds; ++i) {
				...
				this->logger_->log_info(this->name_,
				                        "Filtered cloud %u contains %zu points",
				                        i,
				                        non_aligned_downsampled[i]->points.size());
			}
			...
			*aligned_downsampled[0] = *non_aligned_downsampled[0];
			...
			for (unsigned int i = 0; i < num_clouds; ++i) {
				*aligned_downsampled_remplane[i] = *aligned_downsampled[i];
				...
				this->logger_->log_info(this->name_,
				                        "Removed plane from cloud %u, "
				                        "%zu of %zu points remain",
				                        i,
				                        aligned_downsampled_remplane[i]->points.size(),
		...
		now.stamp();
		fawkes::pcl_utils::set_time(this->output_, now);
		usleep(1000000);
...
private: // methods
	void
	remove_plane(typename PointCloudDBPipeline<PointType>::CloudPtr &cloud)
	{
		...
		*cloud = extracted;
		...
		typename PointCloudDBPipeline<PointType>::CloudPtr hull(
		  new typename PointCloudDBPipeline<PointType>::Cloud());
		...
		typename fawkes::pcl_utils::PlaneDistanceComparison<PointType>::ConstPtr above_comp(
		  new fawkes::pcl_utils::PlaneDistanceComparison<PointType>(coeff, op));
		typename pcl::ConditionAnd<PointType>::Ptr above_cond(new pcl::ConditionAnd<PointType>());
		...
		typename PointCloudDBPipeline<PointType>::CloudPtr cloud_above(
		  new typename PointCloudDBPipeline<PointType>::Cloud());
		...
		typename pcl::ConditionAnd<PointType>::Ptr polygon_cond(new pcl::ConditionAnd<PointType>());

		typename fawkes::pcl_utils::PolygonComparison<PointType>::ConstPtr inpoly_comp(
		  new fawkes::pcl_utils::PolygonComparison<PointType>(*hull));
	...
	// untested
	bool
	align_ndt(CloudConstPtr source, CloudConstPtr target, Eigen::Matrix4f &transform)
	{
		Cloud final;
	             ...
	             std::vector<long long> &                                        actual_times)
	{
		...
		if (cfg_transform_to_sensor_frame_) {
			...
			transformer.lookup_transform(cfg_fixed_frame_,
			                             cfg_global_frame_,
			                             source_time,
			                             transform_recorded);
			...
			try {
				...
				this->logger_->log_warn(this->name_, "Failed to transform point cloud, exception follows");
				this->logger_->log_warn(this->name_, e);
			}
		}
	}
