		...
		if ((client_configs_.find(cfg_name) == client_configs_.end())
			...
			try {
				...
				if (conf->is_enabled()) {
					...
					logger->log_info(name(), "Added MongoDB client configuration %s", cfg_name.c_str());
					conf->log(logger, name(), "  ");
				} else {
					logger->log_info(name(),
					                 "Ignoring disabled MongoDB client "
					                 "configuration %s",
					                 cfg_name.c_str());
				...
				logger->log_warn(name(),
				                 "Invalid MongoDB client config %s, ignoring, "
				                 "exception follows.",
				                 cfg_name.c_str());
		...
		if ((instance_configs_.find(cfg_name) == instance_configs_.end())
			...
			try {
				...
				if (conf->is_enabled()) {
					...
					logger->log_info(name(), "Added MongoDB instance configuration %s", cfg_name.c_str());
				} else {
					logger->log_info(name(),
					                 "Ignoring disabled MongoDB instance "
					                 "configuration %s",
					                 cfg_name.c_str());
					ignored_configs.insert(cfg_name);
				}
				...
				logger->log_warn(name(),
				                 "Invalid MongoDB instance config %s, ignoring, "
				                 "exception follows.",
				                 cfg_name.c_str());
				logger->log_warn(name(), e);
				ignored_configs.insert(cfg_name);
		...
		if ((replicaset_configs_.find(cfg_name) == replicaset_configs_.end())
			...
			try {
				...
				if (conf->is_enabled()) {
					...
					logger->log_info(name(),
					                 "Ignoring disabled MongoDB replica set "
					                 "configuration %s",
					                 cfg_name.c_str());
					ignored_configs.insert(cfg_name);
				}
				...
				logger->log_warn(name(),
				                 "Invalid MongoDB replica set config %s, ignoring, "
				                 "exception follows.",
				                 cfg_name.c_str());
				logger->log_warn(name(), e);
				ignored_configs.insert(cfg_name);
...
MongoDBThread::finalize()
{
	for (auto c : instance_configs_) {
		logger->log_info(name(),
		                 "Stopping instance '%s', grace period %u sec",
		                 c.first.c_str(),
		                 c.second->termination_grace_period());
