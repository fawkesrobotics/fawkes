...
LaserMinMergeDataFilter::filter()
{
	...
	if (ignored_.size() != in.size())
		ignored_.resize(in.size(), false);
	...
	for (unsigned int a = 0; a < vecsize; ++a) {
		if (in[a]->frame.empty()) {
			if (!ignored_[a]) {
				logger->log_warn(filter_name.c_str(),
				                 "input buffer %s has no frame, ignoring",
				                 in[a]->name.c_str());
			}
			ignored_[a] = true;
		} else {
			if (ignored_[a]) {
				logger->log_warn(filter_name.c_str(),
				                 "input buffer %s has recovered, frame %s",
				                 in[a]->name.c_str(),
				                 in[a]->frame.c_str());
			}
			ignored_[a] = false;
	...
	for (unsigned int a = first + 1; a < vecsize; ++a) {
		...
		for (unsigned int i = 0; i < (const unsigned int)out_data_size; ++i) {
			if ((outbuf[i] == 0)
			    || ((inbuf[i] != 0)
			        && (!std::isfinite(outbuf[i])
			            || (std::isfinite(inbuf[1]) && (inbuf[i] < outbuf[i]))))) {
				outbuf[i] = inbuf[i];
			}
		}
	}
