...
ProtobufCommPlexilAdapter::initialize()
{
    ...
    {"pb_get_int",
     std::bind(&ProtobufCommPlexilAdapter::pb_get_value, this, p::_1, PLEXIL::INTEGER_TYPE)},
    {"pb_get_real",
     std::bind(&ProtobufCommPlexilAdapter::pb_get_value, this, p::_1, PLEXIL::REAL_TYPE)},
    {"pb_get_bool",
     std::bind(&ProtobufCommPlexilAdapter::pb_get_value, this, p::_1, PLEXIL::BOOLEAN_TYPE)},
    {"pb_get_string",
     std::bind(&ProtobufCommPlexilAdapter::pb_get_value, this, p::_1, PLEXIL::STRING_TYPE)},
...
ProtobufCommPlexilAdapter::proc_get_param_command(PLEXIL::Command *cmd)
{
	...
	case 3:
		m_execInterface.handleCommandReturn(cmd, PLEXIL::Value(msgmeta.time_received.in_sec()));
		break;
	}
...
ProtobufCommPlexilAdapter::release_message(const std::string &msg_id)
{
	...
	if (colon_pos != std::string::npos) {
		...
		q.messages.erase(std::remove(q.messages.begin(), q.messages.end(), msg_id), q.messages.end());
...
{
	std::vector<PLEXIL::Value> const &args = cmd->getArgValues();
	if (!verify_args(args, "ProtobufCommAdapter:pb_create", {{"msg_type", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}
	...
	try {
		std::shared_ptr<google::protobuf::Message> m = message_register_->new_message_for(msg_type);

		message_meta msgmeta{.time_received = fawkes::Time(clock_),
		                     .from_host     = "",
		                     .from_port     = 0,
...
{
	std::vector<PLEXIL::Value> const &args = cmd->getArgValues();
	if (!verify_args(args, "ProtobufCommAdapter:pb_destroy", {{"msg_id", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}
...

{
	...
	for (size_t i = 0; i < field_path.size(); ++i) {
		std::pair<std::string, long int> parsed_field = parse_field_name(field_path[i], func);
		...
		if (field->is_repeated() && partial_index >= 0
		...
		if (partial_index >= 0 && partial_index < std::numeric_limits<long int>::max()
		    && partial_index >= refl->FieldSize(*msg, field)) {
			warn("ProtobufCommAdapter:" << func << ":"
			                            << " Index " << partial_index << " out of bounds for "
			                            << partial_name);
			return false;
		}
	...
	try {
		...
		case FieldDescriptor::TYPE_DOUBLE:
			if (value.valueType() != PLEXIL::REAL_TYPE) {
				warn("ProtobufCommAdapter:pb_set_value:"
				     ...
				     << field_name << ", expects Real, got " << PLEXIL::valueTypeName(value.valueType()));
				m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
				m_execInterface.notifyOfExternalEvent();
				return;
			} else {
		...
		case FieldDescriptor::TYPE_INT64:
			if (value.valueType() != PLEXIL::INTEGER_TYPE) {
				warn("ProtobufCommAdapter:pb_set_value:"
				     "Invalid type for field "
				     << field_name << ", expects Integer, got "
		...
		case FieldDescriptor::TYPE_BOOL:
			if (value.valueType() != PLEXIL::BOOLEAN_TYPE) {
				warn("ProtobufCommAdapter:pb_set_value:"
				     "Invalid type for field "
				     << field_name << ", expects Boolean, got "
				     << PLEXIL::valueTypeName(value.valueType()));
				m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
				m_execInterface.notifyOfExternalEvent();
				return;
		...
		case FieldDescriptor::TYPE_STRING:
			if (value.valueType() != PLEXIL::STRING_TYPE) {
				warn("ProtobufCommAdapter:pb_set_value:"
				     "Invalid type for field "
				     << field_name << ", expects String, got " << PLEXIL::valueTypeName(value.valueType()));
				m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
				m_execInterface.notifyOfExternalEvent();
				return;
			} else {
...
ProtobufCommPlexilAdapter::pb_get_value(PLEXIL::Command *cmd, PLEXIL::ValueType value_type)
{
	...
	if (!verify_args(args,
	                 ...
	                 {{"msg_id", PLEXIL::STRING_TYPE}, {"field", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}
	...
	case FieldDescriptor::TYPE_FLOAT:
		if (value_type != PLEXIL::REAL_TYPE) {
			warn("ProtobufCommAdapter:pb_get_value:" << m->GetTypeName() << ":" << field_name
	...
	if (field->is_repeated()) {
		if (partial_index >= refl->FieldSize(*msg, field)) {
			warn("ProtobufCommAdapter:pb_get_value:" << m->GetTypeName() << "Index " << partial_index
			                                         << " for " << partial_name << " is out of bounds");
			m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
			m_execInterface.notifyOfExternalEvent();
			return;
		}
		...
		switch (field->type()) {
		case FieldDescriptor::TYPE_DOUBLE:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      refl->GetRepeatedDouble(*msg, field, partial_index)));
			break;
		case FieldDescriptor::TYPE_FLOAT:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      refl->GetRepeatedFloat(*msg, field, partial_index)));
			break;
		case FieldDescriptor::TYPE_UINT64:
		case FieldDescriptor::TYPE_FIXED64:
		...
		case FieldDescriptor::TYPE_STRING:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      refl->GetRepeatedString(*msg, field, partial_index)));
			break;
		case FieldDescriptor::TYPE_ENUM:
			m_execInterface.handleCommandReturn(
		...
		case FieldDescriptor::TYPE_SINT64:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      (PLEXIL::Integer)refl->GetInt64(*msg, field)));
			break;
		case FieldDescriptor::TYPE_UINT64:
		case FieldDescriptor::TYPE_FIXED64:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      (PLEXIL::Integer)refl->GetUInt64(*msg, field)));
			break;
		case FieldDescriptor::TYPE_INT32:
		case FieldDescriptor::TYPE_SFIXED32:
		case FieldDescriptor::TYPE_SINT32:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      (PLEXIL::Integer)refl->GetInt32(*msg, field)));
			break;
		case FieldDescriptor::TYPE_UINT32:
		case FieldDescriptor::TYPE_FIXED32:
			m_execInterface.handleCommandReturn(cmd,
			                                    PLEXIL::Value(
			                                      (PLEXIL::Integer)refl->GetUInt32(*msg, field)));
			break;
		case FieldDescriptor::TYPE_BOOL:
		...
		default:
			warn("ProtobufCommAdapter:pb_get_value:" << m->GetTypeName() << " invalid value type for "
			                                         << field_name);
			m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
			m_execInterface.notifyOfExternalEvent();
			return;
...
{
	std::vector<PLEXIL::Value> const &args = cmd->getArgValues();
	if (!verify_args(args, "ProtobufCommAdapter:pb_tostring", {{"msg_id", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}
...
ProtobufCommPlexilAdapter::pb_broadcast(PLEXIL::Command *cmd)
{
	...
	if (!verify_args(args,
	                 ...
	                 {{"peer_id", PLEXIL::INTEGER_TYPE}, {"msg_id", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}
  ...
  const std::vector<PLEXIL::Value> *override_args)
{
	...
	if (!verify_args(args,
	                 "ProtobufCommAdapter:pb_peer_create_local_crypto",
	...
	if (send_port > 0) {
		...
		peer->signal_received().connect(
		  boost::bind(&ProtobufCommPlexilAdapter::handle_peer_msg, this, peer_id, _1, _2, _3, _4));
		peer->signal_recv_error().connect(
		  boost::bind(&ProtobufCommPlexilAdapter::handle_peer_recv_error, this, peer_id, _1, _2));
		peer->signal_send_error().connect(
		  boost::bind(&ProtobufCommPlexilAdapter::handle_peer_send_error, this, peer_id, _1));
...
ProtobufCommPlexilAdapter::pb_peer_create(PLEXIL::Command *cmd)
{
	...
	if (!verify_args(in_args,
	                 "pb_peer_create",
	                 {{"address", PLEXIL::STRING_TYPE}, {"port", PLEXIL::INTEGER_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
		m_execInterface.notifyOfExternalEvent();
		return;
	}

	std::vector<PLEXIL::Value> args{
	  in_args[0], in_args[1], in_args[1], PLEXIL::Value(""), PLEXIL::Value("")};
...
ProtobufCommPlexilAdapter::pb_peer_create_local(PLEXIL::Command *cmd)
{
	...
	std::vector<PLEXIL::Value> args{
	  in_args[0], in_args[1], in_args[2], PLEXIL::Value(""), PLEXIL::Value("")};
...
ProtobufCommPlexilAdapter::pb_peer_destroy(PLEXIL::Command *cmd)
{
	std::vector<PLEXIL::Value> const &args = cmd->getArgValues();
	if (!verify_args(args,
	                 "ProtobufCommAdapter:pb_peer_destroy",
...
ProtobufCommPlexilAdapter::pb_peer_setup_crypto(PLEXIL::Command *cmd)
{
	std::vector<PLEXIL::Value> const &args = cmd->getArgValues();
	if (!verify_args(args,
	                 "ProtobufCommAdapter:pb_peer_setup_crypto",
	                 {{"peer_id", PLEXIL::INTEGER_TYPE},
	                  {"crypto_key", PLEXIL::STRING_TYPE},
	                  {"cipher", PLEXIL::STRING_TYPE}})) {
		m_execInterface.handleCommandAck(cmd, PLEXIL::COMMAND_FAILED);
...
{
	message_meta m{.time_received = fawkes::Time(clock_),
...
{
	if (logger_) {
		logger_->log_warn("PlexilProtobuf", "Failed to send peer message: %s", msg.c_str());
	}
}

extern "C" {
void
initProtobufCommAdapter()
{
