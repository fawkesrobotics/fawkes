...
LoggingPlexilAdapter::executeCommand(PLEXIL::Command * cmd)
{
  std::map<std::string, std::function<void (const std::vector<PLEXIL::Value> &)>> mapping_norv =
	  {
	   {"print",     [this](const std::vector<PLEXIL::Value> &values) {
		                 this->logger_->log_info("Plexil", "%s", v_tostring(values, false).c_str());}},
	   {"pprint",    [this](const std::vector<PLEXIL::Value> &values) {
		                 this->logger_->log_info("Plexil", "%s", v_tostring(values, true).c_str());}},
	   {"log_debug", [this](const std::vector<PLEXIL::Value> &values) {
		                 this->logger_->log_debug("Plexil", "%s", v_tostring(values, true).c_str());}},
	   {"log_info",  [this](const std::vector<PLEXIL::Value> &values) {
		                 this->logger_->log_info("Plexil", "%s", v_tostring(values, true).c_str());}},
	   {"log_warn",  [this](const std::vector<PLEXIL::Value> &values) {
  ...
  std::map<std::string, std::function<PLEXIL::Value (const std::vector<PLEXIL::Value> &)>> mapping_rv =
	  {
	   {"printToString",  [](const std::vector<PLEXIL::Value> &values) {
