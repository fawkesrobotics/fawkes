...
MetricsRequestProcessor::process_request(const fawkes::WebRequest *request)
{
	...
	std::list<io::prometheus::client::MetricFamily> metrics(
	  std::move(metrics_manager_->all_metrics()));
	...
	if (accepted_encoding.find("application/vnd.google.protobuf") != std::string::npos) {
		...
		for (auto &&metric : metrics) {
			std::string result;
			google::protobuf::util::MessageToJsonString(metric,
			                                            &result,
		...
		for (auto &&metric : metrics) {
			if (metric.metric_size() > 0) {
				...
				if (metric.has_type()) {
					...
					switch (metric.type()) {
					case io::prometheus::client::COUNTER: typestr = "counter"; break;
					case io::prometheus::client::GAUGE: typestr = "gauge"; break;
					case io::prometheus::client::UNTYPED: typestr = "untyped"; break;
					case io::prometheus::client::HISTOGRAM: typestr = "histogram"; break;
					case io::prometheus::client::SUMMARY: typestr = "summary"; break;
					}
					if (typestr != NULL) {
