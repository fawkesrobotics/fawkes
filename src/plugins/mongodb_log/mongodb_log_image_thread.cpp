...
: Thread("MongoLogImagesThread", Thread::OPMODE_CONTINUOUS), MongoDBAspect("default")
{
...
MongoLogImagesThread::init()
{
	...
	cfg_storage_interval_ = config->get_float("/plugins/mongodb-log/images/storage-interval");
...
MongoLogImagesThread::loop()
{
	...
	logger->log_debug(name(),
	                  "Stored %u of %zu images in %.1f ms",
	                  num_stored,
	                  imgs_.size(),
	...
	if (!unbacked_images.empty()) {
		...
		for (i = unbacked_images.begin(); i != unbacked_images.end(); ++i) {
			logger->log_info(name(),
			                 "Shutting down MongoLog for no longer available image %s",
			                 i->c_str());
