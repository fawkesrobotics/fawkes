...
MapLaserGenThread::init()
{
	...
	} // ignored, use AMCL map

	logger->log_info(name(), "Using %s map configuration", have_custom_map ? "custom" : "AMCL");
	fawkes::amcl::read_map_config(config,
	                              ...
	                              cfg_free_thresh_,
	                              have_custom_map ? AMCL_CFG_PREFIX "map-lasergen/"
	                                              : AMCL_CFG_PREFIX);
...
MapLaserGenThread::loop()
{
	...
	for (unsigned int i = 0; i < 360; ++i) {
		dists[i] = map_calc_range(
		  map_, laser_pos_x_, laser_pos_y_, normalize_rad(deg2rad(i) + laser_pos_theta_), 100.);
	}
#ifdef HAVE_RANDOM
	if (cfg_add_noise_) {
		for (unsigned int i = 0; i < 360; ++i) {
	...
	if (cfg_send_zero_odom_) {
		tf::Transform tmp_tf(tf::create_quaternion_from_yaw(0), tf::Vector3(0, 0, 0));
		Time          transform_expiration = (Time(clock) + 1.0);
...
MapLaserGenThread::set_laser_pose()
{
	...
	logger->log_debug(name(),
	                  "LaserTF: (%f,%f,%f)",
	                  laser_pose_.getOrigin().x(),
	                  laser_pose_.getOrigin().y(),
