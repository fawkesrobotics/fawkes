...
/***************************************************************************
 *  pf_kdtree.c: KD tree functions
 *
 *  Created: Thu May 24 18:39:48 2012
...
// Destroy a tree
void
pf_kdtree_free(pf_kdtree_t *self)
{
	free(self->nodes);
	free(self);
	return;
...
void
pf_kdtree_clear(pf_kdtree_t *self)
{
	self->root       = NULL;
	self->leaf_count = 0;
	self->node_count = 0;
...
void
pf_kdtree_insert(pf_kdtree_t *self, pf_vector_t pose, double value)
{
	int key[3];
...
double
pf_kdtree_get_prob(pf_kdtree_t *self, pf_vector_t pose)
{
	int               key[3];
	pf_kdtree_node_t *node;
...
int
pf_kdtree_get_cluster(pf_kdtree_t *self, pf_vector_t pose)
{
	int               key[3];
	pf_kdtree_node_t *node;
...
// Compare keys to see if they are equal
int
pf_kdtree_equal(pf_kdtree_t *self, int key_a[], int key_b[])
{
	//double a, b;
...
// Insert a node into the tree
pf_kdtree_node_t *
pf_kdtree_insert_node(pf_kdtree_t      *self,
                      pf_kdtree_node_t *parent,
                      pf_kdtree_node_t *node,
                      int               key[],
                      double            value)
...
// Recursive node search
pf_kdtree_node_t *
pf_kdtree_find_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int key[])
{
	if (node->leaf) {
...
void
pf_kdtree_cluster(pf_kdtree_t *self)
{
	int                i;
...
// Recursively label nodes in this cluster
void
pf_kdtree_cluster_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int depth)
{
	int i;
	int nkey[3];
...
// Draw the tree
void
pf_kdtree_draw(pf_kdtree_t *self, rtk_fig_t *fig)
{
	if (self->root != NULL)
...
// Recursively draw nodes
void
pf_kdtree_draw_node(pf_kdtree_t *self, pf_kdtree_node_t *node, rtk_fig_t *fig)
{
	if (node->leaf) {
