...
#include "fuzzy/potential_field_controller.h"
...
{ 
  ...
  while(!gts_fifo_is_empty(vertexes))
    { 
      ...
      //        std::cerr << "get points " << GTS_POINT (v)->x << ", " << GTS_POINT (v)->y << std::endl;
   
      NPoint *p = new NPoint();
    
      //        std::cerr << "points before" << p->x << ", " << p->y << std::endl;
      //   if(GTS_POINT(v) == GTS_POINT(nearestObstacle()))
  ...
  //    std::cerr << "return points" << std::endl;
   
  return p_list;
}


/** Gets the lines out the surface.
 ...
 */
GtsPoint * Navigator::getTargetPoint()
{
  return pathfinder->getTargetPoint();
}
 ...
 */
double Navigator::bernstein(int i, int n, double t)
{
...
void Navigator::mainLoop()
{
  //while(true)
  {
        ...
        if(nearestObs != NULL)
          {
            ...
            std::cout << "-------distance  "  << 
              gts_point_distance(GTS_POINT (nearestObs), pathfinder->getRobotPoint()) - robot_width/2. - nearestObs->width/2. << std::endl;     
    ...
    //i = 1 weil der roboter_punkt nicht bewegt werden soll
    for(unsigned int i = 0; i < path.size(); i++)
      {
    ...
    pathfinder->setTargetPoint(gts_point_new(gts_point_class(), 
                                             path[path.size() - 1]->x, path[path.size() - 1]->y, 0));
