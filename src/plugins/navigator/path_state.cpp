 ...
 *  by computing the distance between the path points.
 *
...
{              
  ...
  // if current_face == NULL then the target_point is outside the surface
  if (current_face == NULL || gts_point_triangle_distance(target_point, &(current_face->triangle)) < 1) 
    return true;
  else
    return false;
}
...
GtsPoint * PathState::nextPoint(GtsPoint* p1, GtsPoint* p2, double &newCost)
{
  ...
  else if((gts_point_distance(p1, p2) - obstacle1_radius - obstacle2_radius) <= robot_width * 1.5)
    {
      next_point2 = next_point3 = next_point1;
    }
  else //if the edge is narrow then calculate a new point near by the middle
    {
      vector_length = (gts_point_distance(p1, p2) - obstacle1_radius - obstacle2_radius)
        / robot_width;
...
void PathState::addChild(GtsPoint * p1, GtsPoint * p2, GtsEdge * next_edge, std::vector< AStarState * > &children)
{
  ...
  //if the robot fits through p1 and p2   
  if(gts_point_distance(p1, p2) > robot_width + obstacle1_radius + obstacle2_radius + 5)
    {
      double newCost;
...
std::vector< AStarState * > PathState::generateChildren()
{
  ...
  //Hiermit bekommt man die beiden anderen Seiten des Dreiecks
