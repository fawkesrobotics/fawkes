...
bool PathState::isGoal()
{
  ...
  if (current_face == NULL || gts_point_triangle_distance(target_point, &(current_face->triangle)) < 0.001)
    return true;
  else
    return false;
}
...
void PathState::addChild(GtsPoint * p1, GtsPoint * p2, GtsEdge * next_edge, std::vector< AStarState * > &children)
{
  ...
  if(gts_point_distance(p1, p2) > robot_width + obstacle1_radius + obstacle2_radius + 0.05)
    {
      double newCost;
