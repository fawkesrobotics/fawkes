...
/** Constructor. */
OpenRobotinoComThread::OpenRobotinoComThread() : RobotinoComThread("OpenRobotinoComThread")
{
#ifdef HAVE_OPENROBOTINO_API_1
	com_ = this;
#endif
	...
	if (odo_seq != last_seqnum_) {
		...
		if (motors_com_->numMotors() != 3) {
			logger->log_error(name(),
			                  "Invalid number of motors, got %u, expected 3",
		...
		if (digital_inputs_com_->numDigitalInputs() != 8) {
			logger->log_error(name(),
			                  "Invalid number of digital inputs, got %u, expected 8",
		...
		if (analog_inputs_com_->numAnalogInputs() != 8) {
			logger->log_error(name(),
			                  "Invalid number of analog inputs, got %u, expected 8",
		...
		if (distances_com_->numDistanceSensors() != NUM_IR_SENSORS) {
			logger->log_error(name(),
			                  "Invalid number of distance sensors, got %u, expected 9",
...
{
#ifdef HAVE_OPENROBOTINO_API_1
	logger->log_info(name(), "%sabling bumper estop on request", msg->is_enabled() ? "En" : "Dis");
