 ...
 */
OpenPRSKernelManager::OpenPRSKernelManager(const std::string &server_host,
                                    ...
                                    bool                    utils_gdb_delay)
{
	...
	try {
		if (config_->is_list("/openprs/kernels/data-path")) {
			std::vector<std::string> pl = config_->get_strings("/openprs/kernels/data-path");
			std::for_each(pl.begin(), pl.end(), [&data_path](std::string &p) { data_path.push_back(p); });
		} else {
			std::string cfg_data_path = config_->get_string("/openprs/kernels/data-path");
	...
	const char *envp_path_ext[] = {
	  "LD_LIBRARY_PATH", OPENPRS_MOD_DIR, "OPRS_DATA_PATH", oprs_data_path.c_str(), NULL};
	std::vector<std::string> envp_v = envp_copy_expand(environ, envp_path_ext);

	envp_v.push_back(
	  boost::str(boost::format("FAWKES_OPRS_GDB_DELAY=%s") % (utils_gdb_delay ? "true" : "false")));
