...
/***************************************************************************
 *  rx28.h - Class for accessing Robotis RX28 servos
 *
 *  Created: Tue Jun 16 11:09:32 2009 (based on visca.h)
...
#ifndef _PLUGINS_PANTILT_ROBOTIS_RX28_H_
#define _PLUGINS_PANTILT_ROBOTIS_RX28_H_
...
#define RX28_CONTROL_TABLE_LENGTH 0x32
#define RX28_MAX_NUM_SERVOS 254
...
public:
	...
	void write_table_value(unsigned char id,
	                       unsigned char addr,
	                       unsigned int  value,
	                       bool          double_byte = false);
	void write_table_values(unsigned char  id,
...
private:
	unsigned char calc_checksum(const unsigned char  id,
	                            const unsigned char  instruction,
	                            const unsigned char *params,
	                            const unsigned char  plength);
	void          send(const unsigned char  id,
	                   const unsigned char  instruction,
	                   const unsigned char *params,
	                   const unsigned char  plength);
	void          recv(const unsigned char exp_length, unsigned int timeout_ms = 0xFFFFFFFF);
	void          assert_valid_id(unsigned char id);
	unsigned int  merge_twobyte_value(unsigned int id, unsigned char ind_l, unsigned char ind_h);
	unsigned int
	get_value(unsigned int id, bool refresh, unsigned int ind_l, unsigned int ind_h = 0xFFFFFFFF);
