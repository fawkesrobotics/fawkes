...
#ifdef HAVE_TF
  TransformAspect(TransformAspect::ONLY_PUBLISHER, (std::string("PTU ") + ptu_name).c_str()),
#endif
...
PanTiltRX28Thread::init()
{
	...
	if (cfg_publish_transforms_) {
		float pan_trans_x  = config->get_float((ptu_cfg_prefix_ + "pan_trans_x").c_str());
		float pan_trans_y  = config->get_float((ptu_cfg_prefix_ + "pan_trans_y").c_str());
		float pan_trans_z  = config->get_float((ptu_cfg_prefix_ + "pan_trans_z").c_str());
		float tilt_trans_x = config->get_float((ptu_cfg_prefix_ + "tilt_trans_x").c_str());
		float tilt_trans_y = config->get_float((ptu_cfg_prefix_ + "tilt_trans_y").c_str());
		float tilt_trans_z = config->get_float((ptu_cfg_prefix_ + "tilt_trans_z").c_str());

		std::string frame_id_prefix = std::string("") + ptu_name_;
		try {
			frame_id_prefix = config->get_string((ptu_cfg_prefix_ + "frame_id_prefix").c_str());
		} catch (Exception &e) {
		} // ignore, use default
	...
	for (RobotisRX28::DeviceList::iterator i = devl.begin(); i != devl.end(); ++i) {
		if (cfg_pan_servo_id_ == *i) {
			...
			logger->log_warn(name(),
			                 "Servo %u in PTU servo chain, but neither "
			                 "configured as pan nor as tilt servo",
			                 *i);
		}
	}
...
PanTiltRX28Thread::WorkerThread::get_velocities(float &pan_vel, float &tilt_vel)
{
	...
	pan_velticks =
	  (unsigned int)(((float)pan_velticks / (float)RobotisRX28::MAX_SPEED) * max_pan_speed_);
	tilt_velticks =
	  (unsigned int)(((float)tilt_velticks / (float)RobotisRX28::MAX_SPEED) * max_tilt_speed_);
}
...
{
	if (pan_margin > 0.0)
		pan_margin_ = pan_margin;
	if (tilt_margin > 0.0)
		tilt_margin_ = tilt_margin;
 ...
 */
void
PanTiltRX28Thread::WorkerThread::get_pantilt(float &pan, float &tilt, fawkes::Time &time)
{
	get_pantilt(pan, tilt);
	time = pantilt_time_;
}
...
PanTiltRX28Thread::WorkerThread::is_final()
{
  ...
  */
	...
	return ((fabs(pan - target_pan_) <= pan_margin_) && (fabs(tilt - target_tilt_) <= tilt_margin_))
	       || (!rx28_->is_moving(pan_servo_id_) && !rx28_->is_moving(tilt_servo_id_));
}
...
{
	return (rx28_->is_torque_enabled(pan_servo_id_) && rx28_->is_torque_enabled(tilt_servo_id_));
}
...
PanTiltRX28Thread::WorkerThread::loop()
{
	if (enable_) {
		...
		value_rwlock_->unlock();
		ScopedRWLock lock(rx28_rwlock_);
		if (led_enable_ || led_disable_ || velo_pending_ || move_pending_)
			usleep(3000);
	}
	...
	if (led_enable_) {
		...
		if (velo_pending_ || move_pending_)
			usleep(3000);
...
PanTiltRX28Thread::WorkerThread::exec_goto_pantilt(float pan_rad, float tilt_rad)
{
	...
	if ((pan_pos < 0) || ((unsigned int)pan_pos < pan_min) || ((unsigned int)pan_pos > pan_max)) {
		logger_->log_warn(
		  name(), "Pan position out of bounds, min: %u  max: %u  des: %i", pan_min, pan_max, pan_pos);
		return;
	}

	if ((tilt_pos < 0) || ((unsigned int)tilt_pos < tilt_min)
