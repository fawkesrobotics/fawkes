...
ColliActThread::init()
{
	...
	logger->log_debug(name(),
	                  "Default drive_mode: %i (%s)",
	                  cfg_drive_mode_,
	                  if_navi_->tostring_DriveMode(cfg_drive_mode_));
...
ColliActThread::loop()
{
	...
	while (!if_navi_->msgq_empty()) {
		if (if_navi_->msgq_first_is<NavigatorInterface::StopMessage>()) {
			...
			NavigatorInterface::SetMaxVelocityMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetMaxVelocityMessage>();
			...
			NavigatorInterface::SetMaxRotationMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetMaxRotationMessage>();
			...
			NavigatorInterface::SetEscapingMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetEscapingMessage>();
			...
			NavigatorInterface::SetSecurityDistanceMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetSecurityDistanceMessage>();
			...
			NavigatorInterface::SetStopAtTargetMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetStopAtTargetMessage>();
			...
			NavigatorInterface::SetOrientationModeMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetOrientationModeMessage>();

			logger->log_debug(name(),
			                  "setting orient_at_target to %s",
			                  if_navi_->tostring_OrientationMode(msg->orientation_mode()));
			if_navi_->set_orientation_mode(msg->orientation_mode());

		} else if (if_navi_->msgq_first_is<NavigatorInterface::SetDriveModeMessage>()) {
			NavigatorInterface::SetDriveModeMessage *msg =
			  if_navi_->msgq_first<NavigatorInterface::SetDriveModeMessage>();

			logger->log_debug(name(),
			                  "setting drive_mode to %s",
			                  if_navi_->tostring_DriveMode(msg->drive_mode()));
	...
	if (motion_msg) {
		if (motion_msg->is_of_type<NavigatorInterface::StopMessage>()) {
			...
			NavigatorInterface::CartesianGotoMessage *msg =
			  dynamic_cast<NavigatorInterface::CartesianGotoMessage *>(motion_msg);
			logger->log_debug(name(),
			                  "CartesianGotoMessage received, x:%f  y:%f  ori:%f",
			                  msg->x(),
			                  msg->y(),
			                  msg->orientation());
			...
			NavigatorInterface::PolarGotoMessage *msg =
			  dynamic_cast<NavigatorInterface::PolarGotoMessage *>(motion_msg);
			logger->log_debug(name(),
			                  "PolarGotoMessage received, phi:%f  dist:%f",
			                  msg->phi(),
			                  msg->dist());
