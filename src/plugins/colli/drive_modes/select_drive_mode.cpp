...
SelectDriveMode::load_drive_modes_omnidirectional()
{
	...
	if (cfg_escape_mode_ == colli_escape_mode_t::potential_field) {
		...
		logger_->log_error("SelectDriveMode",
		                   "Unknown escape drive mode. "
...
SelectDriveMode::set_laser_data(std::vector<polar_coord_2d_t> &laser_points)
{
	for (unsigned int i = 0; i < drive_modes_.size(); i++) {
		// drive mode checking
		if (drive_modes_[i]->get_drive_mode_name() == NavigatorInterface::ESCAPE) {
			EscapeDriveModule *dm = static_cast<EscapeDriveModule *>(drive_modes_[i]);
...
SelectDriveMode::update(bool escape)
{
	...
	if (escape == true) {
		if (escape_flag_ == 0 && if_motor_->des_vx() != 0.f && if_motor_->des_vy() != 0.f
	...
	for (unsigned int i = 0; i < drive_modes_.size(); i++) {
		// error checking
		if (drive_modes_[i]->get_drive_mode_name() == desired_mode && drive_mode != 0) {
			logger_->log_error("SelectDriveMode",
			                   "Error while selecting drive mode. There is more than one mode with the "
			                   "same name!!! Stopping!");
			drive_mode = 0;
			break;
		}
	...
	if (drive_mode == 0) {
		...
		drive_mode->set_current_robo_speed(if_motor_->vx(), if_motor_->vy(), if_motor_->omega());
		...
		drive_mode->set_current_colli_mode(if_colli_target_->orientation_mode(),
		                                   if_colli_target_->is_stop_at_target());
