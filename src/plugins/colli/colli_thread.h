...
private:
   ...
   */
	...
	fawkes::colli_trans_rot_t
	  proposed_; // the proposed trans-rot that should be realized in MotorInstruct
	...
	bool
	  cfg_visualize_idle_; /**< Defines if visualization should run when robot is idle without a target. */

	float cfg_min_rot_;        /**< The minimum rotation speed. */
	float cfg_min_drive_dist_; /**< The minimum distance to drive straight to target */
	float
	  cfg_min_long_dist_drive_; /**< The minimum distance to drive straight to target in a long distance */
	float
	      cfg_min_long_dist_prepos_; /**< The minimum distance to drive to a pre-positino of a target in long distance */
	float cfg_min_rot_dist_; /**< The minimum rotation distance to rotate, when at target */
	float
	                                    cfg_target_pre_pos_; /**< Distance to target pre-position (only if colli_state_t == DriveToOrientPoint) */
	...
	float
	  cfg_iface_read_timeout_; /**< Maximum age (in seconds) of required data from reading interfaces*/
