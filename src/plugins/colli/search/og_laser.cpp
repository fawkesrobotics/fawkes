  ...
  if_laser_(laser)
{
	...
	obstacle_distance_ =
	  config->get_float((cfg_prefix + "laser_occupancy_grid/distance_account").c_str());
	initial_history_size_ =
	  3 * config->get_int((cfg_prefix + "laser_occupancy_grid/history/initial_size").c_str());
	max_history_length_ =
	  config->get_float((cfg_prefix + "laser_occupancy_grid/history/max_length").c_str());
	min_history_length_ =
	  config->get_float((cfg_prefix + "laser_occupancy_grid/history/min_length").c_str());
	...
	cfg_emergency_stop_beams_used_ =
	  config->get_float((cfg_prefix + "emergency_stopping/beams_used").c_str());

	cfg_delete_invisible_old_obstacles_ = config->get_bool(
	  (cfg_prefix + "laser_occupancy_grid/history/delete_invisible_old_obstacles/enable").c_str());
	cfg_delete_invisible_old_obstacles_angle_min_ = config->get_int(
	  (cfg_prefix + "laser_occupancy_grid/history/delete_invisible_old_obstacles/angle_min").c_str());
	cfg_delete_invisible_old_obstacles_angle_max_ = config->get_int(
	  (cfg_prefix + "laser_occupancy_grid/history/delete_invisible_old_obstacles/angle_max").c_str());
	...
	if (cfg_delete_invisible_old_obstacles_angle_max_
	    > cfg_delete_invisible_old_obstacles_angle_min_) {
		angle_range_ = deg2rad((unsigned int)abs(cfg_delete_invisible_old_obstacles_angle_max_
		                                         - cfg_delete_invisible_old_obstacles_angle_min_));
	} else {
		angle_range_ = deg2rad((360 - cfg_delete_invisible_old_obstacles_angle_min_)
		                       + cfg_delete_invisible_old_obstacles_angle_max_);
	}
	...
	laser_frame_     = config->get_string(
    (cfg_prefix + "frame/laser")
      .c_str()); //TODO change to base_link => search in base_link instead base_laser

	cfg_obstacle_inc_ = config->get_bool((cfg_prefix + "obstacle_increasement").c_str());
	cfg_force_elipse_obstacle_ =
	  config->get_bool((cfg_prefix + "laser_occupancy_grid/force_ellipse_obstacle").c_str());

	if_buffer_size_ = config->get_int((cfg_prefix + "laser_occupancy_grid/buffer_size").c_str());
	if_buffer_size_ = std::max(
	  if_buffer_size_,
	  1); //needs to be >= 1, because the data is always wrote into the buffer (instead of read())

	cell_costs_.occ =
	  config->get_int((cfg_prefix + "laser_occupancy_grid/cell_cost/occupied").c_str());
	...
	offset_base_.y = 0;
	offset_laser_.x =
	  robo_shape_->get_complete_width_x() / 2.f - robo_shape_->get_robot_length_for_deg(0);
	offset_laser_.y =
	  robo_shape_->get_complete_width_y() / 2.f - robo_shape_->get_robot_length_for_deg(90);
	logger->log_debug("LaserOccupancyGrid",
 ...
 */
void
LaserOccupancyGrid::validate_old_laser_points(cart_coord_2d_t pos_robot,
                                              cart_coord_2d_t pos_new_laser_point)
{
	...
	for (std::vector<LaserPoint>::iterator it = old_readings_.begin(); it != old_readings_.end();
	     ++it) {
...
LaserOccupancyGrid::update_occ_grid(int midX, int midY, float inc, float vx, float vy)
{
	...
	try {
		...
		logger_->log_error("LaserOccupancyGrid",
...
void
LaserOccupancyGrid::integrate_old_readings(int                   midX,
                                           int                   midY,
                                           float                 inc,
                                           float                 vel,
                                           tf::StampedTransform &transform)
{
	...
	std::vector<LaserOccupancyGrid::LaserPoint> *pointsTransformed =
	  transform_laser_points(old_readings_, transform);
...
void
LaserOccupancyGrid::integrate_new_readings(int                   midX,
                                           int                   midY,
                                           float                 inc,
                                           float                 vel,
                                           tf::StampedTransform &transform)
{
	std::vector<LaserOccupancyGrid::LaserPoint> *pointsTransformed =
	  transform_laser_points(new_readings_, transform);
...
LaserOccupancyGrid::integrate_obstacle(int x, int y, int width, int height)
{
     ...
     */
		...
		if ((posX > 0) && (posX < height_) && (posY > 0) && (posY < width_)
