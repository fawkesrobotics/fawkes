...
require("skills.nao")
module("skills.nao.intercept", skills.nao.module_init)
...
function intercept_checkstatus(margin)
   return nao.relgoto.relgoto_checkstatus(margin);
end
...
function intercept(...)
   ...
   local ball_x = nao_ball:relative_x();
   local ball_y = nao_ball:relative_y();
