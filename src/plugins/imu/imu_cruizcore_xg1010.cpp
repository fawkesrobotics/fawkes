...
public:
	...
	std::size_t
	operator()(const Error &err, std::size_t bytes_transferred)
	{
...
CruizCoreXG1010AcquisitionThread::init()
{
	...
	if (cfg_baud_rate_ != 115200 && cfg_baud_rate_ != 57600 && cfg_baud_rate_ != 38400
	    && cfg_baud_rate_ != 28800 && cfg_baud_rate_ != 19200 && cfg_baud_rate_ != 9600
	    && cfg_baud_rate_ != 4800) {
		throw Exception("Invalid baud rate");
	}

	if ((cfg_freq_ > 25 && cfg_baud_rate_ < 9600) || (cfg_freq_ > 50 && cfg_baud_rate_ < 19200)) {
	...
	if (serial_.is_open()) {
		...
		try {
			...
			boost::asio::async_read(serial_,
			                        ...
			                        boost::bind(&CruizCoreXG1010AcquisitionThread::handle_read,
			                                    this,
			...
			if (ec_) {
				...
				do {
					...
					if (catch_up) {
						...
						size_t remaining   = CRUIZCORE_XG1010_PACKET_SIZE
						                   - (read_size - full_frames * CRUIZCORE_XG1010_PACKET_SIZE);
						to_read = remaining;
					} else {
	  ...
	  */
			...
			if (e.code() == boost::asio::error::eof) {
				close_device();
				logger->log_warn(name(), "CruizCoreXG1010 connection lost, trying to re-open");
			} else {
...
CruizCoreXG1010AcquisitionThread::check_deadline()
{
	...
	deadline_.async_wait(
	  boost::lambda::bind(&CruizCoreXG1010AcquisitionThread::check_deadline, this));
#else
