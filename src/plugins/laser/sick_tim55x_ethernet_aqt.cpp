	...
	if (socket_.is_open()) {
		try {
...
#else
			                              boost::bind(&SickTiM55xEthernetAcquisitionThread::handle_read,
			                                          this,
			...
			if (e.code() == boost::asio::error::eof) {
				close_device();
				logger->log_warn(name(), "Sick TiM55x/Ethernet connection lost, trying to reconnect");
			} else {
...
SickTiM55xEthernetAcquisitionThread::open_device()
{
	try {
		...
		boost::asio::ip::tcp::resolver::query    query(cfg_host_, cfg_port_);
...
void
SickTiM55xEthernetAcquisitionThread::send_with_reply(const char *request, std::string *reply)
{
...
SickTiM55xEthernetAcquisitionThread::check_deadline()
{
	...
	deadline_.async_wait(
	  boost::lambda::bind(&SickTiM55xEthernetAcquisitionThread::check_deadline, this));
#else
...
#if BOOST_VERSION >= 104800
	soft_deadline_.async_wait(
	  boost::lambda::bind(&SickTiM55xEthernetAcquisitionThread::check_soft_timeout, this));
#else
	soft_deadline_.async_wait(
	  boost::bind(&SickTiM55xEthernetAcquisitionThread::check_soft_timeout, this));
#endif
}
