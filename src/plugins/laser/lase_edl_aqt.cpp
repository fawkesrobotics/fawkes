 ...
 */
LaseEdlAcquisitionThread::LaseEdlAcquisitionThread(std::string &cfg_name, std::string &cfg_prefix)
...
void
LaseEdlAcquisitionThread::pre_init(fawkes::Configuration *config, fawkes::Logger *logger)
{
	if (pre_init_done_)
		return;
	...
	try {
		...
		if (canres == "low") {
			...
			logger->log_error(name(),
			                  "Canonical resolution %s is invalid, must be 'low' "
	...
	try {
		...
		if (cfg_angle_step_ < min_angle_step_) {
			logger->log_warn(name(),
			                 "Configured angle step %u less than required minimum "
			                 "of %u, raising to minimum",
			                 cfg_angle_step_,
			                 min_angle_step_);
		...
		if ((number_of_values_ != 360) && (number_of_values_ != 720)) {
			throw Exception("At the moment only configurations with 360 or 720 "
			                "laser beams are supported, but %u requested",
			                number_of_values_);
		}
	...
	for (unsigned int i = 1; i <= cfg_num_init_tries_; ++i) {
		...
		try {
			...
			if (!cfg_use_default_) {
				...
				SET_CONFIG(CONFIGITEM_GLOBAL,
				           CONFIGDATA_LENGTH_GLOBAL,
				           cfg_sensor_id_,
				           cfg_rotation_freq_,
				           cfg_angle_step_);

				// set functions (sector definition)
				SET_FUNCTION(SECTOR_0,
				             SECTORFUNC_NORMAL_MEASUREMENT,
				             (16 * 360) - cfg_angle_step_,
				             cfg_set_default_ ? FLASH_YES : FLASH_NO);
				SET_FUNCTION(SECTOR_1,
				             SECTORFUNC_NOT_INITIALIZED,
				             0,
				             cfg_set_default_ ? FLASH_YES : FLASH_NO);
			}
			...
			if (i < cfg_num_init_tries_) {
				logger->log_warn("LaseEdlAcquisitionThread",
				                 "Initialization, retrying %d more times",
				                 cfg_num_init_tries_ - i);
				logger->log_warn("LaseEdlAcquisitionThread", e);
			} else {
				logger->log_error("LaseEdlAcquisitionThread",
				                  "Initialization failed, giving up after %u tries",
				                  cfg_num_init_tries_);
				throw;
			}
		}
...
LaseEdlAcquisitionThread::recv(WORD **data, bool allocate)
{
	...
	if (read == 0x0000) { // receiving only one frame
		...
		if (read != cfg_sensor_id_resp_) {
			logger->log_warn("LaseEdlAcquisitionThread",
			                 "Sensor ID is not the expected ID, "
			                 "ignoring message");
			return -1;
		}
		...
		for (WORD f = number_of_incoming_frames - 1; f > 0; --f) {
			...
			if (read != f) {
				logger->log_warn("LaseEdlAcquisitionThread",
				                 "Recv protocol violation, "
				                 "wrong frame number: expected %u, but got %u",
				                 f,
				                 read);
				return -1;
			}
...
{
	WORD *real_response;
	WORD *expected_response = make_word_array(2, respcode(CMD_GET_PROFILE), cfg_profile_format_);
	...
	if (cfg_profile_format_ == PROFILEFORMAT_DISTANCE) {
		...
		for (int i = 3; i < response_size; ++i) {
			...
			if (++dist_index >= (int)number_of_values_)
				dist_index = 0;
		}
		...
		for (int i = 3; i < response_size; ++i) {
			...
			if (++echo_index >= (int)number_of_values_)
				echo_index = 0;
		}
...
void
LaseEdlAcquisitionThread::SET_FUNCTION(WORD sect_num, WORD sect_func, WORD sect_stop, WORD flash)
{
	WORD *command = make_word_array(5, CMD_SET_FUNCTION, sect_num, sect_func, sect_stop, flash);
...
LaseEdlAcquisitionThread::GET_PROFILE(WORD prof_num, WORD prof_format)
{
	...
	send_and_check(command, 3, make_word_array(2, respcode(CMD_GET_PROFILE), prof_format), 2);
}

void
LaseEdlAcquisitionThread::CANCEL_PROFILE()
{
	send_and_check(make_word_array(1, CMD_CANCEL_PROFILE),
	               1,
	               make_word_array(1, respcode(CMD_CANCEL_PROFILE)),
	               1);
}
...
LaseEdlAcquisitionThread::TRANS_IDLE()
{
	...
	send_and_check(
	  command, 1, make_word_array(1, respcode(CMD_TRANS_IDLE)), 1, &real_response, &response_size);

	bool failed = (real_response[response_size - 1] != 0x0001);
	free(real_response);
	if (failed)
		throw Exception("Failed to set trans idle");
}
...
LaseEdlAcquisitionThread::TRANS_ROTATE(WORD frequency)
{
	...
	int   response_size;
	send_and_check(
	  command, 2, make_word_array(1, respcode(CMD_TRANS_ROTATE)), 1, &real_response, &response_size);

	bool failed = (real_response[response_size - 1] != 0x0002);
	free(real_response);
	if (failed)
		throw Exception("Failed to set trans rotate");
}
...
LaseEdlAcquisitionThread::TRANS_MEASURE()
{
	...
	int   response_size;
	send_and_check(
	  command, 1, make_word_array(1, respcode(CMD_TRANS_MEASURE)), 1, &real_response, &response_size);

	bool failed =
	  (real_response[response_size - 2] != 0x0003) || (real_response[response_size - 1] != 0x0000);
	unsigned int error_code = real_response[response_size - 1];
	free(real_response);
	if (failed)
		throw Exception("Failed set trans measure, error code %u", error_code);
}
