...
SickTiM55xUSBAcquisitionThread::init()
{
	...
	} // ignore, if there is only one take that
...
SickTiM55xUSBAcquisitionThread::loop()
{
	...
	if (usb_device_handle_) {
		...
		usb_rv = libusb_bulk_transfer(usb_device_handle_,
		...
		if (usb_rv != 0) {
			if (usb_rv == LIBUSB_ERROR_NO_DEVICE) {
				...
				logger->log_warn(name(),
				                 "Failed to read Sick TiM55x data (%d): %s",
				                 usb_rv,
	...
	for (ssize_t i = 0; i < num_devs; ++i) {
		...
		if (desc.idVendor == USB_VENDOR && desc.idProduct == USB_PRODUCT) {
			...
			if (cfg_serial_ != "") {
				...
				usb_rv = libusb_get_string_descriptor_ascii(usb_device_handle_,
				                                            desc.iSerialNumber,
				                                            serial_desc,
				                                            32);
				...
				if (usb_rv <= 0) {
					logger->log_warn(name(),
					                 "Failed to read serial from Sick TiM55x: %s",
					                 libusb_strerror((libusb_error)usb_rv));
					libusb_close(usb_device_handle_);
					usb_device_handle_ = NULL;
					continue;
				...
				if (cfg_serial_ == serial_desc_s) {
					...
					logger->log_info(name(),
					                 "Ignoring Sick TiM55x with non-matching serial %s"
					                 " (looking for %s)",
					                 serial_desc_s.c_str(),
					                 cfg_serial_.c_str());
					libusb_close(usb_device_handle_);
...
void
SickTiM55xUSBAcquisitionThread::send_with_reply(const char *request, std::string *reply)
{
	MutexLocker lock(usb_mutex_);
	...
	usb_rv = libusb_bulk_transfer(
	  usb_device_handle_, (1 | LIBUSB_ENDPOINT_IN), tmpbuf, 32 * 1024, &actual_length, USB_TIMEOUT);
	if (usb_rv != 0) {
