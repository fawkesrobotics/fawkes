...
: Gtk::Window(cobject)
{
	...
	cmb_pre_filter_type->signal_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_pre_filter_type_changed));
	hsc_pre_filter_size->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_pre_filter_size_changed));
	hsc_pre_filter_cap->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_pre_filter_cap_changed));
	hsc_sad_window_size->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_sad_window_size_changed));
	hsc_min_disparity->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_min_disparity_changed));
	hsc_num_disparities->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_num_disparities_changed));
	hsc_texture_threshold->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_texture_threshold_changed));
	hsc_uniqueness_ratio->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_uniqueness_ratio_changed));
	hsc_speckle_window_size->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_speckle_window_size_changed));
	hsc_speckle_range->signal_value_changed().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_speckle_range_changed));
	cb_try_smaller_windows->signal_toggled().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_try_smaller_windows_toggled));

	tb_connection->signal_clicked().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_connection_clicked));
	tb_exit->signal_clicked().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_exit_clicked));
	;
	connection_dispatcher.signal_connected().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_connect));
	connection_dispatcher.signal_disconnected().connect(
	  sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::on_disconnect));
...
Bumblebee2CalibGtkWindow::on_texture_threshold_changed()
{
	...
	if (params_if_ && params_if_->has_writer() && (params_if_->texture_threshold() != new_value)) {
...
Bumblebee2CalibGtkWindow::on_uniqueness_ratio_changed()
{
	...
	if (params_if_ && params_if_->has_writer() && (params_if_->uniqueness_ratio() != new_value)) {
...
Bumblebee2CalibGtkWindow::on_speckle_window_size_changed()
{
	...
	if (params_if_ && params_if_->has_writer() && (params_if_->speckle_window_size() != new_value)) {
...
Bumblebee2CalibGtkWindow::on_connect()
{
	try {
		...
		params_if_ = bb_->open_for_reading<OpenCVStereoParamsInterface>("bumblebee2");
		...
		ifd_params_->signal_data_changed().connect(
		  sigc::hide(sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::update_param_values)));
		...
		case OpenCVStereoParamsInterface::PFT_XSOBEL: cmb_pre_filter_type->set_active(0); break;
		default: cmb_pre_filter_type->set_active(1); break;
		}
		...
		buffer_rgb_rect_left_ =
		  malloc_buffer(RGB, cam_left_rectified_->pixel_width(), cam_left_rectified_->pixel_height());

		sconn_update_images_ =
		  Glib::signal_timeout().connect(sigc::mem_fun(*this, &Bumblebee2CalibGtkWindow::update_images),
		                                 IMG_UPDATE_INTERVAL);
		...
		if (buffer_rgb_disparity_)
			free(buffer_rgb_disparity_);
		if (buffer_rgb_rect_left_)
			free(buffer_rgb_rect_left_);
		cam_left_rectified_ = cam_disparity_ = NULL;
		buffer_rgb_disparity_ = buffer_rgb_rect_left_ = NULL;
...
Bumblebee2CalibGtkWindow::on_disconnect()
{
	...
	img_writer->set_from_icon_name(Gtk::Stock::NO.id, Gtk::IconSize(Gtk::ICON_SIZE_SMALL_TOOLBAR));
...
Bumblebee2CalibGtkWindow::update_param_values()
{
	...
	switch (params_if_->pre_filter_type()) {
	case OpenCVStereoParamsInterface::PFT_XSOBEL: lab_pre_filter_type->set_text("XSOBEL"); break;
	default: lab_pre_filter_type->set_text("NORM RESP"); break;
	}
...
Bumblebee2CalibGtkWindow::update_images()
{
	if (bb_ && params_if_ && params_if_->has_writer()) {
		...
		convert(cam_left_rectified_->colorspace(),
		        RGB,
		        cam_left_rectified_->buffer(),
		        buffer_rgb_rect_left_,
		        rlwidth,
		        rlheight);
		...
		Glib::RefPtr<Gdk::Pixbuf> image =
		  Gdk::Pixbuf::create_from_data(buffer_rgb_rect_left_,
		                                Gdk::COLORSPACE_RGB,
		                                /* has alpha */ false,
		                                /* bits per color */ 8,
		                                rlwidth,
		                                rlheight,
		                                /* row stride */ 3 * rlwidth,
		...
		convert(cam_disparity_->colorspace(),
		        RGB,
		        cam_disparity_->buffer(),
		        buffer_rgb_disparity_,
		        rlwidth,
		        rlheight);
		...
		Glib::RefPtr<Gdk::Pixbuf> dimage =
		  Gdk::Pixbuf::create_from_data(buffer_rgb_disparity_,
		                                Gdk::COLORSPACE_RGB,
		                                /* has alpha */ false,
		                                /* bits per color */ 8,
		                                rlwidth,
