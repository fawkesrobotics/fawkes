...
LaserClusterThread::init()
{
	...
	if (cfg_line_removal_) {
		...
		cfg_segm_min_inliers_ = config->get_uint(cfg_prefix_ + "line_removal/segmentation_min_inliers");
		cfg_segm_sample_max_dist_ =
		  config->get_float(cfg_prefix_ + "line_removal/segmentation_sample_max_dist");
		cfg_line_min_length_ = config->get_float(cfg_prefix_ + "line_removal/min_length");
	}
	...
	try {
		...
		config_if_ = NULL;
		config_if_ =
		  blackboard->open_for_writing_f<LaserClusterInterface>("/laser-cluster/%s", cfg_name_.c_str());
	...
	if (asprintf(&output_cluster_labeled_name, "/laser-cluster/%s-labeled", cfg_name_.c_str())
	    != -1) {
                                 ...
                                 const Eigen::Quaternionf &   attitude)
{
	...
	if (is_visible) {
		...
		Eigen::Vector4f last_centroid(iface->translation(0) - cfg_offset_x_,
		                              ...
		                              iface->translation(2) - cfg_offset_z_,
		                              0.);
...
float
LaserClusterThread::calc_line_length(CloudPtr                    cloud,
