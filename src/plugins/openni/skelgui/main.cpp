...
glut_display(void)
{
	...
	if (g_draw_image && g_image_drawer)
		g_image_drawer->draw();
	if (g_draw_depth && g_depth_drawer)
		g_depth_drawer->draw();
...
glut_keyboard(unsigned char key, int x, int y)
{
	...
	case 'd':
		...
		if (g_draw_depth)
			g_draw_image = false;
		break;
	case 's': g_draw_skeleton = !g_draw_skeleton; break;
	case 'l': g_skeleton_drawer->toggle_print_state(); break;
	case 'L':
		if (g_depth_drawer)
			g_depth_drawer->toggle_show_labels();
		break;
...
main(int argc, char **argv)
{
	...
	if (argp.has_arg("n") || argp.has_arg("s")) {
		if (argp.has_arg("n")) {
			g_image_cam = new NetworkCamera(fvhost.c_str(), fvport, "openni-image", argp.has_arg("j"));
