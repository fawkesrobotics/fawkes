...
OpenNiUserTrackerThread::init()
{
	...
	st = user_gen_->RegisterUserCallbacks(cb_new_user, cb_lost_user, this, user_cb_handle_);
	if (st != XN_STATUS_OK) {
...
#if XN_VERSION_GE(1, 3, 2, 0)
	st = skelcap_->RegisterToCalibrationStart(cb_calibration_start, this, calib_start_cb_handle_);
	if (st != XN_STATUS_OK) {
		throw Exception("Failed to register calibration start event (%s)", xnGetStatusString(st));
	}
	st = skelcap_->RegisterToCalibrationComplete(cb_calibration_complete,
	                                             this,
	                                             calib_complete_cb_handle_);
#else
	...
	if (st != XN_STATUS_OK) {
		throw Exception("Failed to register calibration callback (%s)", xnGetStatusString(st));
	}
...
#if XN_VERSION_GE(1, 3, 2, 0)
		st = posecap.RegisterToPoseDetected(cb_pose_start, this, pose_start_cb_handle_);
		if (st != XN_STATUS_OK) {
			throw Exception("Failed to register pose detect event (%s)", xnGetStatusString(st));
		}
		st = posecap.RegisterToOutOfPose(cb_pose_end, this, pose_end_cb_handle_);
#else
		st = posecap.RegisterToPoseCallbacks(cb_pose_start, cb_pose_end, this, pose_cb_handle_);
#endif
		if (st != XN_STATUS_OK) {
	...
	label_buf_ =
	  new SharedMemoryImageBuffer("openni-labels", RAW16, scene_md_->XRes(), scene_md_->YRes());
	label_bufsize_ = colorspace_buffer_size(RAW16, scene_md_->XRes(), scene_md_->YRes());
		...
		if (new_state == HumanSkeletonInterface::STATE_TRACKING) {
			...
			try {
				...
				logger->log_warn(name(),
				                 "Failed to update skeleton data for %u, "
				                 "exception follows",
				                 i->first);
...
OpenNiUserTrackerThread::new_user(XnUserID id)
{
	if (users_.find(id) != users_.end()) {
		...
		if (asprintf(&ifid, "OpenNI Human %u", id) == -1) {
			logger->log_warn(name(),
			                 "New user ID %u, but cannot generate "
			                 "interface ID",
			                 id);
			return;
		}
...
OpenNiUserTrackerThread::lost_user(XnUserID id)
{
	...
	logger->log_error(name(),
	                  "Lost user ID %u, setting interface '%s' to invalid",
	                  id,
	                  users_[id].skel_if->uid());
...
OpenNiUserTrackerThread::pose_start(XnUserID id, const char *pose_name)
{
	if (users_.find(id) == users_.end()) {
		logger->log_error(name(),
		                  "Pose start for user ID %u, "
		                  "but interface does not exist",
		                  id);
		return;
	}
...
OpenNiUserTrackerThread::calibration_end(XnUserID id, bool success)
{
	...
	if (success) {
		logger->log_info(name(),
		                 "Calibration successful for user %u, "
		                 "starting tracking",
		                 id);
