...
{
	pb_peer_ =
	  std::shared_ptr<protobuf_comm::ProtobufBroadcastPeer>(new protobuf_comm::ProtobufBroadcastPeer(
	    broadcast_address, broadcast_port, crypto_key, crypto_cipher));

	service_ = std::shared_ptr<NetworkService>(
	  new NetworkService(peer_name.c_str(), GOSSIP_MDNSSD_SERVICE_NAME, broadcast_port));
...
: name_(group_name), service_publisher_(service_publisher)
{
	pb_peer_ =
	  std::shared_ptr<protobuf_comm::ProtobufBroadcastPeer>(new protobuf_comm::ProtobufBroadcastPeer(
	    broadcast_address, send_port, recv_port, crypto_key, crypto_cipher));

	service_ = std::shared_ptr<NetworkService>(
	  new NetworkService(peer_name.c_str(), GOSSIP_MDNSSD_SERVICE_NAME, recv_port));
 ...
 */
void
GossipGroup::send(std::string &peer, google::protobuf::Message &m)
{
	pb_peer_->send(m);
}
