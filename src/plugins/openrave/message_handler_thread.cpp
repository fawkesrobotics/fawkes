	...
	while (!if_openrave_->msgq_empty()) {
		...
		if (if_openrave_->msgq_first_is<OpenRaveInterface::StartViewerMessage>()) {
			...
			logger->log_debug(name(),
			                  "AddObject message received: name=%s, path=%s",
			                  msg->name(),
			                  msg->path());
			...
			logger->log_debug(name(),
			                  "AttachObjectMessage received: name=%s, manip_name=%s",
			                  msg->name(),
			                  msg->manip_name());
			bool success = false;
			...
			logger->log_debug(name(),
			                  "MoveObjectMessage received: name=%s, x=%f, y=%f, z=%f",
			                  msg->name(),
			                  msg->x(),
			                  msg->y(),
			                  msg->z());
			...
			logger->log_debug(name(),
			                  "RotateObjectMessage received: name=%s, x=%f, y=%f, z=%f",
			                  msg->name(),
			                  msg->x(),
			                  msg->y(),
			                  msg->z());
			...
			logger->log_debug(name(),
			                  "RenameObjectMessage received: name=%s, new_name=%s",
			                  msg->name(),
			                  msg->newName());
