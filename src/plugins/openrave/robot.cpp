...
: logger_(logger), name_(""), manip_(0), find_best_ik_(1)
{
	init();
}
/** Constructor
	...
	try {
		...
		if (logger_)
			logger_->log_warn(name(),
			                  "Could not unload robot properly from environment. Ex:%s",
			                  e.what());
	}
}
...
OpenRaveRobot::load(const std::string &filename, fawkes::OpenRaveEnvironmentPtr &env)
{
	...
	if (!robot_)
		throw fawkes::IllegalArgumentException(
		  "%s: Robot could not be loaded. Check xml file/path '%s'.", name(), filename.c_str());
...
OpenRaveRobot::set_ready()
{
	...
	if (robot_->GetActiveDOF() == 0)
		throw fawkes::Exception(
		  "%s: Robot not added to environment yet. Need to do that first, otherwise planner will fail.",
		  name());
	...
	try {
		...
		planner_params_->SetRobotActiveJoints(
		  robot_); // set planning configuration space to current active dofs
 ...
 */
bool
OpenRaveRobot::set_target_quat(float           trans_x,
                               float           trans_y,
                               float           trans_z,
                               float           quat_w,
                               float           quat_x,
 ...
 */
bool
OpenRaveRobot::set_target_axis_angle(float           trans_x,
                                     float           trans_y,
                                     float           trans_z,
                                     float           angle,
                                     float           axisX,
                                ...
                                bool                      no_offset)
{
	...
	case (EULER_ZXZ):
		if (logger_)
			logger_->log_debug(
			  name(), "Target EULER_ZXZ: %f %f %f %f %f %f", trans_x, trans_y, trans_z, phi, theta, psi);
 ...
 */
bool
OpenRaveRobot::set_target_object_position(float           trans_x,
                                          float           trans_y,
                                          float           trans_z,
                                          float           rot_x,
                                          IkFilterOptions filter)
{
	...
	float alpha =
	  atan2(trans_y - trans_offset_y_,
	        trans_x - trans_offset_x_); //angle to rotate left/right when manipulator points to +x
	Vector quat_y =
	  quatFromAxisAngle(Vector(0.f, M_PI / 2, 0.f)); //1st, rotate down -> manipulator points to +x
	Vector quat_x = quatFromAxisAngle(
	  Vector(-alpha, 0.f, 0.f)); //2nd, rotate left/right -> manipulator points to object
	Vector quat_z =
	  quatFromAxisAngle(Vector(0.f, 0.f, rot_x)); //last, rotate wrist -> manipulator ready to grab
	...
	try {
		if (manip_name) {
			...
			if (!manip) {
				if (logger_)
					logger_->log_warn(name(),
					                  "Could not attach Object, could not get manipulator '%s'",
					                  manip_name);
				return false;
	...
	if (rotations.size() != 9) {
		...
		if (logger_)
			logger_->log_error(name(),
			                   "Bad size of rotations vector. Is %i, expected 9",
			                   rotations.size());
		return false;
	}
