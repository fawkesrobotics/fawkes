...
/** Constructor. */
PluginRestApi::PluginRestApi() : Thread("PluginRestApi", Thread::OPMODE_WAITFORWAKEUP)
{
}
...
PluginRestApi::init()
{
	...
	rest_api_->add_handler<WebviewRestArray<::Plugin>>(
	  WebRequest::METHOD_GET, "/?", std::bind(&PluginRestApi::cb_list_plugins, this));
	rest_api_->add_handler<PluginOpResponse, PluginOpRequest>(
	  WebRequest::METHOD_PUT,
	  "/{name}",
	  std::bind(
	    &PluginRestApi::cb_set_plugin_state, this, std::placeholders::_1, std::placeholders::_2));
...
PluginRestApi::cb_list_plugins()
{
	...
	for (const auto &i : available_plugins) {
		...
		if (is_meta) {
			...
			std::vector<std::string> v{std::make_move_iterator(std::begin(plugin_list)),
