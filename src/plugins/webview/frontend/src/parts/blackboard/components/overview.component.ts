...
export class BlackboardOverviewComponent implements OnInit, OnDestroy {
  ...
  dotgraph_zero_message = 'No blackboard graph available';
  dotgraph_loading = false;
  ...
  ifcd(hash_id: string[], field: string) {
    const iface = this.ifc(hash_id);
    const idx = iface.info.fields.findIndex(f => f.name === field);
    if (idx < 0) { // Ooops
      ...
      if (iface.info.fields[idx].is_array) {
        const rv = [];
        for (const v of iface.data.data[field]) {
          const t = iface.info.fields[idx].type;
          if (t === 'double' || t === 'float') {
            rv.push(v.toFixed(4));
          } else {
            rv.push(v);
  ...
  select_interface(hash: string, id: string) {
    this.dotgraph = null;
    const index = this.indexof_selected_interface(hash, id);
    if (index === -1) {
  ...
  refresh_data(hash: string, id: string) {
    ...
    const type = this.interfaces[hash].type;
  ...
  refresh(refresh_data = false) {
    ...
    this.api_service.list_interfaces().subscribe(
      (interfaces) => {
        ...
        for (const hash of Object.keys(ifs)) {
          for (const id of Object.keys(ifs[hash].instances)) {
            const index = updated.findIndex(
              (element) => (element[0] === hash) && (element[1] === id));
            const enabled = (index >= 0);
            const index_selected = this.indexof_selected_interface(hash, id);
        ...
        if (refresh_data) {
          for (const hash_id of this.selected_interfaces) {
