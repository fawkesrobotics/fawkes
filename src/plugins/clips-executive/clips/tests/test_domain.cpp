...
TEST_F(DomainTest, DisjunctivePreconditions)
{
	...
	env.run();
	EXPECT_TRUE(has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:executable FALSE))"));
	...
	env.run();
	EXPECT_TRUE(has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:executable TRUE))"));
...
TEST_F(DomainTest, IsPrecondNegativeFunction)
{
	...
	env.assert_fact("(domain-precondition (name p11) (part-of p1)"
	                "(type negation)"
	                ")");
	EXPECT_EQ("TRUE", env.evaluate("(domain-is-precond-negative p11)")[0].as_string());
	env.assert_fact("(domain-precondition (name p111) (part-of p11)"
	                "(type negation)"
	                ")");
	EXPECT_EQ("FALSE", env.evaluate("(domain-is-precond-negative p111)")[0].as_string());
	env.assert_fact("(domain-precondition (name p1111) (part-of p111)"
	                "(type disjunction)"
	                ")");
	EXPECT_EQ("FALSE", env.evaluate("(domain-is-precond-negative p1111)")[0].as_string());
	env.assert_fact("(domain-precondition (name p11111) (part-of p1111)"
	                "(type negation)"
	                ")");
	EXPECT_EQ("TRUE", env.evaluate("(domain-is-precond-negative p11111)")[0].as_string());
}
...
TEST_F(DomainTest, ErrorIfPreconditionHasNoOperator)
{
	...
	env.run();
	EXPECT_TRUE(has_fact("((?e domain-error))", "(eq ?e:error-type precondition-without-parent)"));
}
...
TEST_F(DomainTest, ErrorIfObjTypeDoesNotExist)
{
	...
	env.run();
	EXPECT_TRUE(has_fact("((?e domain-error))", "(eq ?e:error-type type-of-object-does-not-exist)"));
}
...
TEST_F(DomainTest, ErrorIfSuperTypeDoesNotExist)
{
	...
	env.run();
	EXPECT_TRUE(has_fact("((?e domain-error))", "(eq ?e:error-type super-type-does-not-exist)"));
}
...
TEST_F(BlocksworldDomainTest, ActionIsExecutableIfPreconditionIsSatisfied)
{
	...
	env.run();
	EXPECT_TRUE(
	  has_fact("((?a plan-action))", "(and (eq ?a:action-name pick-up) (eq ?a:executable TRUE))"));
	EXPECT_TRUE(
	  has_fact("((?a plan-action))", "(and (eq ?a:action-name unstack) (eq ?a:executable TRUE))"));
}
...
TEST_F(BlocksworldDomainTest, ApplyEffects)
{
	...
	env.run();
	EXPECT_FALSE(
	  has_fact("((?p domain-fact))", "(and (eq ?p:name holding) (eq ?p:param-values (create$ b1)))"));
	EXPECT_TRUE(has_fact("((?p domain-fact))", "(eq ?p:name handempty)"));
	EXPECT_TRUE(
	  has_fact("((?p domain-fact))", "(and (eq ?p:name clear) (eq ?p:param-values (create$ b1)))"));
...
TEST_F(BlocksworldDomainTest, ApplyContradictingEffectsWithDifferentParams)
{
	...
	EXPECT_TRUE(
	  has_fact("((?p domain-fact))", "(and (eq ?p:name clear) (eq ?p:param-values (create$ b1)))"));
	EXPECT_FALSE(
	  has_fact("((?p domain-fact))", "(and (eq ?p:name clear) (eq ?p:param-values (create$ b2)))"));
...
TEST_F(DomainTest, WaitForSensedEffects)
{
	...
	env.assert_fact("(plan-action"
	                ...
	                " (param-values obj1))");
	env.run();
	EXPECT_FALSE(has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:state FINAL))"));
	EXPECT_FALSE(
	  has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:state SENSED-EFFECTS-HOLD))"));
	EXPECT_TRUE(
	  has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:state SENSED-EFFECTS-WAIT))"));
	...
	EXPECT_TRUE(has_fact("((?f domain-fact))",
	                     "(and (eq ?f:name on-ground) (eq ?f:param-values (create$ obj1)))"));
	EXPECT_TRUE(has_fact("((?a plan-action))", "(and (eq ?a:id 1) (eq ?a:state FINAL))"));
}
...
TEST_F(DomainTest, ApplyCondtradictingEffectsWithSameParams)
{
	...
	env.run();
	EXPECT_TRUE(
	  has_fact("((?f domain-fact))", "(and (eq ?f:name p) (eq ?f:param-values (create$ a)))"));
}
...
TEST_F(DomainTest, PreconditionWithConstant)
{
	...
	EXPECT_FALSE(
	  has_fact("((?p domain-precondition))", "(and (eq ?p:name p1) (eq ?p:is-satisfied TRUE))"));
	EXPECT_FALSE(has_fact("((?a plan-action))", "(and (eq a:?id 1) (eq ?a:executable TRUE))"));
...
TEST_F(DomainTest, Equality)
{
	...
	env.assert_fact("(plan-action"
	                ...
	                " (param-values b c))");
	env.run();
	EXPECT_TRUE(has_fact("((?a plan-action))", "(and (eq ?a:id 2) (eq ?a:executable FALSE))"));
}
...
TEST_F(DomainTest, EqualityErrors)
{
	...
	env.run();
	EXPECT_TRUE(
	  has_fact("((?e domain-error))", "(eq ?e:error-type precondition-with-equality-and-predicate)"));
	env.reset();
	env.assert_fact("(domain-atomic-precondition"
	                " (name ap1)"
	                " (part-of p1)"
...
TEST_F(DomainTest, PreconditionWithUnknownParameter)
{
	...
	env.assert_fact("(domain-atomic-precondition"
	                ...
	                " (param-constants nil a))");
	env.run();
	EXPECT_TRUE(has_fact("((?e domain-error))", "(eq ?e:error-type unknown-parameter)"));
}
...
TEST_F(DomainTest, ActionHasADomainOperator)
{
	...
	env.run();
	EXPECT_TRUE(
	  has_fact("((?e domain-error))", "(eq ?e:error-type operator-of-action-does-not-exist)"));
}
...
TEST_F(DomainTest, ValuePredicatesHaveUniqueValues)
{
	...
	env.run();
	EXPECT_FALSE(
	  has_fact("((?e domain-error))", "(eq ?e:error-type value-predicate-with-multiple-values)"));
...
TEST_F(BlocksworldDomainTest, EffectsOnValuePredicatesMustOccurInPairs)
{
	...
	env.run();
	EXPECT_FALSE(
	  has_fact("((?e domain-error))", "(eq ?e:error-type value-predicate-without-paired-effect)"));
...
TEST_F(ConditionalSayDomainTest, ApplyCondEffectIfCondHolds)
{
	...
	env.assert_fact("(plan-action"
	                ...
	                ")");
	env.assert_fact("(apply-action g0 p0 1)");
	env.assert_fact("(domain-fact (name speaker-ready) (param-values front_speaker))");
	env.run();
