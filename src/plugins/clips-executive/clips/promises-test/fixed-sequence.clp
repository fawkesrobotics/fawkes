...
(defrule goal-expander-get-container-and-fill
  ...
  (assert
    ...
    (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                 (action-name move-plaza) (param-values ?r ?start CONTAINER-DEPOT) (type SEQUENTIAL))
    (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                 (action-name pick-container) (param-values ?r ?c) (type SEQUENTIAL))
    (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                 (action-name move) (param-values ?r CONTAINER-DEPOT ?mine) (type SEQUENTIAL))
    (plan-action (id 4) (plan-id ?plan-id) (goal-id ?goal-id)
                 (action-name collect-regolith) (param-values ?r ?mine ?c) (type SEQUENTIAL))
  )
...
(defrule goal-expander-deliver-to-machine
  ...
  (if (eq ?mat REGOLITH) then
    (assert
      (plan (id ?plan-id) (goal-id ?goal-id))
      (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move) (param-values ?r ?start ?side) (type SEQUENTIAL))
      (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name put-regolith) (param-values ?r ?side ?machine ?c) (type SEQUENTIAL))
      (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move-plaza) (param-values ?r ?side BASE) (type SEQUENTIAL))
    )
    else
    (assert
      (plan (id ?plan-id) (goal-id ?goal-id))
      (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move) (param-values ?r ?start ?side) (type SEQUENTIAL))
      (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name put-processite) (param-values ?r ?side ?machine ?c) (type SEQUENTIAL))
      (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move-plaza) (param-values ?r ?side BASE) (type SEQUENTIAL))
    )
  )
...
(defrule goal-expander-start-machine
  ...
  (assert
    (plan (id ?plan-id) (goal-id ?goal-id))
    (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                 (action-name start-machine) (param-values ?m) (type SEQUENTIAL))
  )
  (modify ?g (mode EXPANDED))
)
...
(defrule goal-expander-clean-machine
  ...
  (if (eq ?mat PROCESSITE) then
    (assert
      (plan (id ?plan-id) (goal-id ?goal-id))
      (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move) (param-values ?r ?start ?s) (type SEQUENTIAL))
      (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name collect-processite) (param-values ?r ?m ?s ?c) (type SEQUENTIAL))
      (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move-plaza) (param-values ?r ?s BASE) (type SEQUENTIAL))
    )
    else
    (assert
      (plan (id ?plan-id) (goal-id ?goal-id))
      (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move) (param-values ?r ?start ?s) (type SEQUENTIAL))
      (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name collect-xenonite) (param-values ?r ?m ?s ?c) (type SEQUENTIAL))
      (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                  (action-name move-plaza) (param-values ?r ?s BASE) (type SEQUENTIAL))
    )
  )
...
´
  (assert
    (plan (id ?plan-id) (goal-id ?goal-id))
    (plan-action (id 1) (plan-id ?plan-id) (goal-id ?goal-id)
                (action-name move) (param-values ?r ?start STORAGE-INPUT) (type SEQUENTIAL))
    (plan-action (id 2) (plan-id ?plan-id) (goal-id ?goal-id)
                (action-name put-container) (param-values ?r ?c STORAGE-INPUT) (type SEQUENTIAL))
    (plan-action (id 3) (plan-id ?plan-id) (goal-id ?goal-id)
                (action-name move-plaza) (param-values ?r STORAGE-INPUT BASE) (type SEQUENTIAL))
  )
