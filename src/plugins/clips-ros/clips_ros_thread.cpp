...
ClipsROSThread::clips_context_init(const std::string &env_name, LockPtr<CLIPS::Environment> &clips)
{
	...
	clips->add_function("ros-get-nodes",
	                    sigc::slot<void>(
	                      sigc::bind<0>(sigc::mem_fun(*this, &ClipsROSThread::clips_ros_get_nodes),
	                                    env_name)));

	clips->add_function("ros-get-topics",
	                    sigc::slot<void>(
	                      sigc::bind<0>(sigc::mem_fun(*this, &ClipsROSThread::clips_ros_get_topics),
	                                    env_name)));
...
ClipsROSThread::clips_ros_get_nodes(std::string env_name)
{
	...
	if (temp) {
		for (auto n : nodes) {
			...
			for (auto t : n.second.published)
				published.push_back(t);
			for (auto t : n.second.subscribed)
			...
			if (!new_fact) {
				logger->log_warn(("CLIPS-ROS|" + env_name).c_str(),
				                 "Failed to assert ros-node fact for %s",
				                 n.first.c_str());
			}
		}
	} else {
		logger->log_warn(("CLIPS-ROS|" + env_name).c_str(), "Could not get deftemplate 'ros-node'");
	}
}
...
ClipsROSThread::clips_ros_get_topics(std::string env_name)
{
	...
	if (!ros::master::getTopics(topics)) {
		logger->log_warn(("CLIPS-ROS|" + env_name).c_str(), "Failed to retrieve topics ROS master");
		return;
	}
...
ClipsROSThread::clips_ros_get_topic_connections(std::string env_name)
{
	...
	if (!ros::master::getNodes(nodes)) {
		logger->log_warn(("CLIPS-ROS|" + env_name).c_str(), "Failed to get nodes from ROS master");
		return;
	}
	...
	for (auto n : xmlrpc_clients) {
		...
		if (n.second->execute("getBusInfo", args, result)) {
			if (!xm->validateXmlrpcResponse("getBusInfo", result, payload)) {
				logger->log_warn(("CLIPS-ROS|" + env_name).c_str(),
				                 "%s returned no valid response on getBusInfo",
				                 n.first.c_str());
				continue;
			}
			...
			for (int i = 0; i < payload.size(); ++i) {
				...
				bool connected = (payload[i].size() >= 6) ? (bool)payload[i][5] : true;
