...
NavGraphVisualizationThread::regenerate()
{
	...
	std::map<std::pair<std::string, std::string>, std::string> bl_edges = crepo_->blocks(edges);
	...
	for (size_t i = 0; i < nodes.size(); ++i) {
		...
		bool is_last =
		  traversal_ && (traversal_.path().size() >= 1) && (traversal_.path().goal() == nodes[i]);
	...
	if (traversal_ && !traversal_.path().empty()
	    && traversal_.path().goal().name() == "free-target") {
		...
		if (target_tolerance > 0.) {
			add_circle_markers(m,
			                   id_num,
			                   traversal_.path().goal().x(),
			                   traversal_.path().goal().y(),
			                   target_tolerance,
			                   10,
			                   sphere.color.r,
			                   sphere.color.g,
			...
			if (edge.is_directed()) {
				...
				if (plan_from_ == from.name() && plan_to_ == to.name()) {
					...
					if (in_plan) {
						...
						if (cost_factor >= 1.00001) {
							arrow.color.g = std::min(1.0, (cost_factor - 1.0) / cfg_cost_scale_max_) * 0.66666;
						} else {
							arrow.color.g = 0.f;
						}
						arrow.color.b = 0.f;
				...
				if ((plan_to_ == from.name() && plan_from_ == to.name())
					...
					if (in_plan) {
						...
						if (cost_factor >= 1.00001) {
							...
							line.color.g = std::min(1.0, (cost_factor - 1.0) / cfg_cost_scale_max_) * 0.66666;
							line.color.b = 0.f;
							line.scale.x = 0.02;
							m.markers.push_back(line);
						} else {
	...
	crepo_.lock();
	const NavGraphConstraintRepo::NodeConstraintList &node_constraints = crepo_->node_constraints();
	const NavGraphConstraintRepo::EdgeConstraintList &edge_constraints = crepo_->edge_constraints();
	std::list<const NavGraphPolygonConstraint *>      poly_constraints;

	std::for_each(node_constraints.begin(),
	...
	std::for_each(edge_constraints.begin(),
	              edge_constraints.end(),
