  ...
  : NavGraphNodeConstraint(name)
{
  logger_ = logger;
  clock_ = clock;
  logger_->log_info("Timed Node Constraint", "Created %s constraint", name.c_str() );
}
...
NavGraphTimedReservationListNodeConstraint::~NavGraphTimedReservationListNodeConstraint()
{
  logger_->log_info("Timed Node Constraint", "Deleted %s constraint", constraint_name_.c_str() );
}
...
NavGraphTimedReservationListNodeConstraint::compute(void) throw()
{
  ...
  std::vector<std::pair<TopologicalMapNode, fawkes::Time>> erase_list;
  for (const std::pair<TopologicalMapNode, fawkes::Time> &ec : node_time_list_) {
	  if(now > ec.second){
	    erase_list.push_back(ec);
	  }
  }
  for (const std::pair<TopologicalMapNode, fawkes::Time> &ec : erase_list) {
	  node_time_list_.erase(std::remove(node_time_list_.begin(), node_time_list_.end(), ec), node_time_list_.end());
      modified_ = true;
      logger_->log_info("Timed Node Constraint", "Deleted node '%s' from '%s' because its validity duration ran out", ec.first.name().c_str(), name_.c_str() );
  }
 ...
 */
void
NavGraphTimedReservationListNodeConstraint::add_node(const fawkes::TopologicalMapNode &node, fawkes::Time valid_time)
{
  fawkes::Time now(clock_);

  if (valid_time < now) {
    logger_->log_warn("Timed Node Constraint", "Constraint '%s' received node with old reservation time='%f' - now='%f'", name_.c_str(), valid_time.in_sec(), now.in_sec());
  }
  if (! has_node(node)) {
    modified_ = true;
    node_time_list_.push_back(std::make_pair(node, valid_time));
    std::string txt = node.name();
    //logger_->log_info("Timed Node Constraint", "Adding Node '%s' to constraint '%s'", txt.c_str(), name_.c_str() );
  }
}
...
NavGraphTimedReservationListNodeConstraint::add_nodes(
  const std::vector<std::pair<fawkes::TopologicalMapNode, fawkes::Time>> &timed_nodes)
{
  std::string txt = "{";
  for (const std::pair<TopologicalMapNode, fawkes::Time> &ec : timed_nodes) {
    add_node(ec.first, ec.second);
    txt += ec.first.name(); txt += ",";
  }
  txt.erase(txt.length()-1,1); txt += "}";
  //logger_->log_info("Timed Node Constraint", "Adding Nodes '%s' to constraint '%s'", txt.c_str(), name_.c_str() );
}
 ...
 */
void
NavGraphTimedReservationListNodeConstraint::remove_node(const fawkes::TopologicalMapNode &node)
{
  std::vector<std::pair<TopologicalMapNode, fawkes::Time>>::iterator ec
    = std::find_if(node_time_list_.begin(), node_time_list_.end(),
		   [&node](const std::pair<fawkes::TopologicalMapNode, fawkes::Time> &p) {
		     return p.first == node;
		   });
 ...
 */
bool
NavGraphTimedReservationListNodeConstraint::has_node(const fawkes::TopologicalMapNode &node)
{
 ...
 */
bool
NavGraphTimedReservationListNodeConstraint::blocks(const fawkes::TopologicalMapNode &node) throw()
{
  for (const std::pair<fawkes::TopologicalMapNode, fawkes::Time> &te : node_time_list_){
 ...
 */
const std::vector<std::pair<fawkes::TopologicalMapNode, fawkes::Time>> &
