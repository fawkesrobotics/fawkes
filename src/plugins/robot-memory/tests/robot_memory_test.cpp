...
TEST_F(RobotMemoryTest, StoreUpdateQuery)
{
	...
	ASSERT_TRUE(qres->more());
	ASSERT_TRUE(contains_pairs(qres->next(), fromjson("{'updated':'something',as:3.0,extra:true}")));
}
...
{
	ASSERT_TRUE(
	  robot_memory->update("{upsert:'not existing'}", "{upsert:'should not exist'}", "", false));
...
{
	ASSERT_THROW(robot_memory->update("{'testkey':'good'}", "{'bad':1.2.3}"), mongo::DBException);
...
TEST_F(RobotMemoryTest, JavaScriptQuery)
{
	...
	QResCursor qres =
	  robot_memory->query("{'testname':'js-query', $where: \"return obj.a * 2 == obj.b\"}");
	ASSERT_TRUE(qres->more());
	qres->next();
	ASSERT_TRUE(qres->more());
	qres->next();
...
TEST_F(RobotMemoryTest, EventTriggerReplica)
{
	...
	robot_memory->update(fromjson("{updateid:42}"),
	                     fromjson("{test:2, updateid:42}"),
	                     "syncedrobmem.test");
...
RobotMemoryTest::contains_pairs(BSONObj obj, BSONObj exp)
{
	for (BSONObjIterator it = exp.begin(); it.more();) {
		BSONElement kvpair = it.next();
		//printf("checking %s\n", kvpair.toString().c_str());
		if (!obj.hasElement(kvpair.fieldName()) || obj.getField(kvpair.fieldName()) != kvpair) {
			return ::testing::AssertionFailure()
			       << obj.toString() << " does not include {" << kvpair.toString() << "}";
		}
	}
...
TEST_F(RobotMemoryTest, MapReduceQuery)
{
	...
	BSONObj res = robot_memory->mapreduce(fromjson("{'testname':'mapreduce'}"),
	                                      "robmem.test",
	                                      "function() { emit( this.product, this.amount);}",
	                                      "function(key, values) { return Array.sum( values )}");
	ASSERT_TRUE(
	  contains_pairs(res,
	                 fromjson("{ok: 1.0, results:[{_id:1.0, value:3.0}, {_id:2.0, value: 7.0}]}")));
}
...
TEST_F(RobotMemoryTest, ComputableRegisterRemove)
{
	...
	Computable *    comp = robot_memory->register_computable(fromjson("{somekey:'value'}"),
                                                       "robmem.test",
...
{
	TestComputable *tc   = new TestComputable();
	Computable *    comp = robot_memory->register_computable(fromjson("{computed:true}"),
                                                       "robmem.test",
                                                       &TestComputable::compute,
                                                       tc);
...
TEST_F(RobotMemoryTest, ComputableCallAddition)
{
	TestComputable *tc = new TestComputable();
	Computable *    comp =
	  robot_memory->register_computable(fromjson("{compute:'sum',x:{$exists:true},y:{$exists:true}}"),
	                                    "robmem.test",
...
{
	TestComputable *tc     = new TestComputable();
	Computable *    comp   = robot_memory->register_computable(fromjson("{compute:'multiple'}"),
                                                       "robmem.test",
...
TEST_F(RobotMemoryTest, BlackboardComputable)
{
	...
	if3d->write();
	QResCursor qres = robot_memory->query(fromjson("{interface:'Position3DInterface',id:'test1'}"),
	                                      "robmem.blackboard");
	ASSERT_TRUE(qres->more());
	ASSERT_TRUE(contains_pairs(qres->next(),
	                           fromjson("{interface:'Position3DInterface',id:'test1',frame:'test_"
...
TEST_F(RobotMemoryTest, BlackboardComputableMultiple)
{
	...
	if3d_2->write();
	QResCursor qres = robot_memory->query(fromjson("{interface:'Position3DInterface',id:'test'}"),
	                                      "robmem.blackboard");
	ASSERT_TRUE(qres->more());
...
{
	robot_memory->insert("{name:'test pos', frame:'cam_tag', translation:[0.0, 0.0, 0.0], "
	                     "rotation:[0.0, 0.0, 0.0, 1.0]}",
	                     "robmem.test");
	QResCursor qres =
	  robot_memory->query(fromjson("{name:'test pos', frame:'base_link', allow_tf:true}"),
	                      "robmem.test");
	ASSERT_TRUE(qres->more());
