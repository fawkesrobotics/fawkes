 ...
 */
bool
ComputablesManager::check_and_compute(mongo::Query query, std::string collection)
{
...
ComputablesManager::cleanup_computed_docs()
{
	...
	for (std::map<std::tuple<std::string, std::string>, long long>::iterator it =
		...
		if (current_time_ms > it->second) {
			robot_memory_->remove(BSON("_robmem_info.computed" << true << "_robmem_info.cached_until"
			                                                   << BSON("$lt" << current_time_ms)),
			                      std::get<0>(it->first));
