                                           ...
                                           fawkes::Configuration *config)
{
	...
	computable = robot_memory_->register_computable(query,
	                                                "robmem.blackboard",
...
BlackboardComputable::compute_interfaces(const mongo::BSONObj &query, const std::string &collection)
{
	...
	for (Interface *interface : blackboard_->open_multiple_for_reading(type.c_str(), id.c_str())) {
		...
		//build document
		BSONObjBuilder b;
		b << "interface" << interface->type() << "id" << interface->id();
...
mongo::BSONArray
BlackboardComputable::get_interface_fields(fawkes::InterfaceFieldIterator it)
{
	...
	for (unsigned int i = 0; i < it.get_length(); i++) {
		...
		case IFT_STRING: arr_b.append(it.get_string()); return arr_b.arr();
