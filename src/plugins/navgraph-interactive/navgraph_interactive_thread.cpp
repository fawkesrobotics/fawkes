  ...
  visualization_msgs::InteractiveMarker &                      int_marker)
{
	...
	if (handler->getCheckState(entry_handle, check_state)) {
		if (check_state == MenuHandler::UNCHECKED) {
			...
			ori_control.interaction_mode = visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS;
			int_marker.controls.push_back(ori_control);
			handler->setCheckState(entry_handle, MenuHandler::CHECKED);

			auto control =
			  std::find_if(int_marker.controls.begin(),
			...
			int_marker.controls.erase(
			  std::remove_if(int_marker.controls.begin(),
			                 int_marker.controls.end(),
			                 [](const visualization_msgs::InteractiveMarkerControl &c) -> bool {
				                 return c.name == "rot_z";
			                 }),
...
void
NavGraphInteractiveThread::process_node_feedback(
  const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
		                  ...
		                  feedback->menu_entry_id);
		{
			...
			if (server_->get(feedback->marker_name, int_marker)) {
				if (node_menus_.find(int_marker.name) != node_menus_.end()) {
					...
					if (feedback->menu_entry_id == menu.ori_handle) {
						...
						if (nav_if_->has_writer()) {
							...
							logger->log_error(name(),
							                  "Cannot goto %s, no writer for interface",
						...
						try {
							...
							navgraph.unlock();
							logger->log_error(name(),
							                  "Failed to insert edge %s--%s as requested, exception follows",
							                  int_marker.name.c_str(),
							                  to_node.c_str());
						...
						server_->applyChanges();
					           ...
					           != menu.dir_connect_nodes.end()) {
						NavGraphEdge edge(int_marker.name, menu.dir_connect_nodes[feedback->menu_entry_id]);
						...
						try {
							...
							navgraph.unlock();
							logger->log_error(name(),
							                  "Failed to insert edge %s->%s as requested, "
							                  "exception follows",
							                  int_marker.name.c_str(),
						...
						logger->log_warn(name(),
						                 "Got menu feedback for %s/%s, but marker not known",
						                 feedback->marker_name.c_str(),
						                 feedback->control_name.c_str());
					}
					...
					logger->log_warn(name(),
					                 "Got feedback for %s/%s, but marker not known",
					                 feedback->marker_name.c_str(),
					                 feedback->control_name.c_str());
				}
			}
		...
		if (feedback->header.frame_id == cfg_global_frame_) {
			...
			if (node) {
				if (feedback->control_name == "rot_z") {
					// orientation update
					tf::Quaternion q(feedback->pose.orientation.x,
				...
				logger->log_warn(name(),
				                 "Position update for %s, but not unknown",
				                 feedback->marker_name.c_str());
			}
			navgraph.unlock();
	...
	case visualization_msgs::InteractiveMarkerFeedback::MOUSE_UP: break;
	}
...
void
NavGraphInteractiveThread::process_graph_feedback(
  const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
...
NavGraphInteractiveThread::add_node(const NavGraphNode &node, NavGraph *navgraph)
{
	...
	const tf::Quaternion ori_q =
	  has_ori ? tf::create_quaternion_from_yaw(node.property_as_float(navgraph::PROP_ORIENTATION))
	...
	if (has_ori) {
		...
		int_marker.pose.orientation.x = int_marker.pose.orientation.y = int_marker.pose.orientation.z =
		  0.;
	...
	menu.ori_handle =
	  menu.handler->insert("Orientation",
	                       boost::bind(&NavGraphInteractiveThread::process_node_feedback, this, _1));
	menu.goto_handle =
	  menu.handler->insert("Go to",
	                       boost::bind(&NavGraphInteractiveThread::process_node_feedback, this, _1));
	...
	std::for_each(nodes.begin(), nodes.end(), [&, this](const NavGraphNode &n) -> void {
		if (n.name() != node.name()) {
			auto edge =
			  std::find_if(edges.begin(), edges.end(), [&n, &node](const NavGraphEdge &e) -> bool {
				  return (e.from() == node.name() && e.to() == n.name())
				         || (!e.is_directed() && e.from() == n.name() && e.to() == node.name());
			  });
			if (edge == edges.end()) {
				MenuHandler::EntryHandle undir_handle = menu.handler->insert(
				  connect_undir_menu_handle,
				  n.name(),
				  boost::bind(&NavGraphInteractiveThread::process_node_feedback, this, _1));
				menu.undir_connect_nodes[undir_handle] = n.name();

				MenuHandler::EntryHandle dir_handle = menu.handler->insert(
				  connect_dir_menu_handle,
				  n.name(),
				  boost::bind(&NavGraphInteractiveThread::process_node_feedback, this, _1));
				menu.dir_connect_nodes[dir_handle] = n.name();
			} else {
				MenuHandler::EntryHandle handle = menu.handler->insert(
				  disconnect_menu_handle,
				  n.name(),
				  boost::bind(&NavGraphInteractiveThread::process_node_feedback, this, _1));
	...
	pos_control.orientation_mode = visualization_msgs::InteractiveMarkerControl::FIXED;
	pos_control.interaction_mode = visualization_msgs::InteractiveMarkerControl::MOVE_AXIS;
