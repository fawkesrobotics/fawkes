...
pred_time_eq(TermList terms)
{
	...
	if (t1_sec->type != LONG_LONG || t1_usec->type != LONG_LONG || t2_sec->type != LONG_LONG
	    || t2_usec->type != LONG_LONG) {
		fprintf(stderr,
...
pred_timeout(TermList terms)
{
	...
	return (fawkes::time_diff_sec(
	          t1_sec->u.llintval, t1_usec->u.llintval, t2_sec->u.llintval, t2_usec->u.llintval)
	        > compare_val);
}
...
action_set_idle_looptime(TermList terms)
{
	...
	if ((t_sec->type != INTEGER && t_sec->type != LONG_LONG)
		...
		fprintf(stderr,
		        "time-set-looptime: time values not (all) of type "
		        "integer (types %i %i)\n",
		        t_sec->type,
		        t_usec->type);
		ACTION_FAIL();
