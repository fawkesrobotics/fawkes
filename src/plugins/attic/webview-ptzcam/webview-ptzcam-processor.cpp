...
WebviewPtzCamRequestProcessor::process_overview()
{
	...
	r->set_html_header(
	  "  <link type=\"text/css\" href=\"/static/css/jqtheme/jquery-ui.custom.css\" "
	  "rel=\"stylesheet\" />\n"
	...
	*r += "<script>\n"
	...
	if (!presets_.empty()) {
		*r += "<br/>\n";
		for (auto p : presets_) {
			r->append_body("<button id=\"preset-%s\" title=\"Look at %s\">%s</button>\n"
			               "<script>\n"
			               "$(function() {\n"
			               "  $( \"#preset-%s\" ).button()\n"
			               "  .click(function() {\n"
...
WebviewPtzCamRequestProcessor::process_move(const fawkes::WebRequest *request)
{
	...
	if (tilt_str != "" || pan_str != "") {
		PanTiltInterface::GotoMessage *gotomsg = new PanTiltInterface::GotoMessage(pan_val, tilt_val);
		ptu_if_->msgq_enqueue(gotomsg);
	}

	if (zoom_str != "") {
		if (zoom_str == "out") {
			zoom_val = std::max((long int)camctrl_if_->zoom_min(),
			                    (long int)camctrl_if_->zoom() - zoom_increment_);
		} else if (zoom_str == "in") {
			zoom_val = std::min((long int)camctrl_if_->zoom_max(),
			                    (long int)camctrl_if_->zoom() + zoom_increment_);
		} else {
			try {
...
WebviewPtzCamRequestProcessor::process_effect(const fawkes::WebRequest *request)
{
	...
	CameraControlInterface::SetEffectMessage *setmsg = new CameraControlInterface::SetEffectMessage();
