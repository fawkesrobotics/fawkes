...
WebviewPtzCamThread::init()
{
	...
	web_proc_ = new WebviewPtzCamRequestProcessor(image_id,
	                                              pantilt_id,
	                                              camctrl_id,
	                                              power_id,
	                                              camera_id,
	...
	webview_url_manager->add_handler(WebRequest::METHOD_GET,
	                                 PTZCAM_URL_PREFIX "ping",
	                                 std::bind(&WebviewPtzCamRequestProcessor::process_ping,
	                                           web_proc_));
	webview_url_manager->add_handler(WebRequest::METHOD_GET,
	                                 PTZCAM_URL_PREFIX "move",
	                                 std::bind(&WebviewPtzCamRequestProcessor::process_move,
	                                           web_proc_,
	                                           std::placeholders::_1));
	webview_url_manager->add_handler(WebRequest::METHOD_GET,
	                                 PTZCAM_URL_PREFIX "effect",
	                                 std::bind(&WebviewPtzCamRequestProcessor::process_move,
	                                           web_proc_,
	                                           std::placeholders::_1));
	webview_url_manager->add_handler(WebRequest::METHOD_GET,
	                                 PTZCAM_URL_PREFIX "?",
	                                 std::bind(&WebviewPtzCamRequestProcessor::process_overview,
	                                           web_proc_));
...
WebviewPtzCamThread::loop()
{
	...
	if (webview_request_manager->num_active_requests() == 0) {
		...
		try {
			Time last_completion = webview_request_manager->last_request_completion_time();
