...
ClipsAgentThread::init()
{
	...
	try {
		cfg_skill_sim_time_ = config->get_float("/clips-agent/skill-sim-time");
	} catch (Exception &e) {
	} // ignore, use default
	try {
	...
	clips->add_function("skill-call-ext",
	                    sigc::slot<void, std::string, std::string>(
	                      sigc::mem_fun(*this, &ClipsAgentThread::clips_skill_call_ext)));

	clips->evaluate("(ff-feature-request \"config\")");

	bool cfg_req_redefwarn_feature = true;
	try {
		cfg_req_redefwarn_feature = config->get_bool("/clips-agent/request-redefine-warning-feature");
	} catch (Exception &e) {
	} // ignored, use default
		...
		for (as = active_skills_.begin(); as != active_skills_.end(); ++as) {
			...
			if (cfg_skill_sim_) {
				if ((now - as_info.start_time) >= cfg_skill_sim_time_) {
					...
					clips->assert_fact_f("(skill-update (name \"%s\") (status FINAL))", as_name.c_str());
					finished_skills.push_back(as_name);
				} else {
					clips->assert_fact_f("(skill-update (name \"%s\") (status RUNNING))", as_name.c_str());
				}
			} else {
				if (as_info.skill_string == skiller_if_->skill_string()) {
					clips->assert_fact_f("(skill-update (name \"%s\") (status %s))",
					                     as_name.c_str(),
...
ClipsAgentThread::clips_skill_call_ext(std::string skill_name, std::string skill_string)
{
	if (active_skills_.find(skill_name) != active_skills_.end()) {
		logger->log_warn(name(), "Skill %s called again while already active", skill_name.c_str());
	}
