...
/** @class RosImagesThread "image_thread.h"
 * Thread to export Fawkes images to ROS.
 * @author Tim Niemueller
 */

/** Constructor. */
RosImagesThread::RosImagesThread()
  : Thread("RosImagesThread", Thread::OPMODE_WAITFORWAKEUP),
...
void
RosImagesThread::init()
{
  it_ = new image_transport::ImageTransport(**rosnode);
...
RosImagesThread::loop()
{
  ...
  for (p = pubs_.begin(); p != pubs_.end(); ++p) {
    ...
    if ((pubinfo.last_sent != cap_time) && (pubinfo.pub.getNumSubscribers() > 0)) {
      ...
      pubinfo.msg.header.stamp =
        ros::Time(cap_time.get_sec(), cap_time.get_usec() * 1000);
      convert(pubinfo.img->colorspace(), RGB,
              pubinfo.img->buffer(), &pubinfo.msg.data[0],
              pubinfo.msg.width, pubinfo.msg.height);
...
RosImagesThread::update_images()
{
  ...
  if (! unbacked_images.empty()) {
    ...
    for (i = unbacked_images.begin(); i != unbacked_images.end(); ++i) {
      logger->log_info(name(), "Shutting down publisher for no longer available image %s",
                       i->c_str());
      PublisherInfo &pubinfo = pubs_[*i];
      pubinfo.pub.shutdown();
      delete pubinfo.img;
      pubs_.erase(*i);
    }
  }
  ...
  if (! missing_images.empty()) {
    ...
    for (i = missing_images.begin(); i != missing_images.end(); ++i) {
      logger->log_info(name(), "Creating publisher for new image %s",
                       i->c_str());
...
void
RosImagesThread::get_sets(std::set<std::string> &missing_images,
                          std::set<std::string> &unbacked_images)
{
