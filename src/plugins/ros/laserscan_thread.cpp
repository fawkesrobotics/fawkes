...
RosLaserScanThread::init()
{
	...
	sub_ls_ = rosnode->subscribe("scan", 100, &RosLaserScanThread::laser_scan_message_cb, this);
...
RosLaserScanThread::loop()
{
	...
	while (!ls_msg_queues_[queue].empty()) {
		const ros::MessageEvent<sensor_msgs::LaserScan const> &msg_evt = ls_msg_queues_[queue].front();
		...
		if (callerid.empty()) {
			logger->log_warn(name(),
			                 "Received laser scan from ROS without caller ID,"
			                 "ignoring");
		} else {
