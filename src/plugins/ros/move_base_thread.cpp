...
/** @class RosMoveBaseThread "move_base_thread.h"
...
RosMoveBaseThread::RosMoveBaseThread()
  : Thread("RosMoveBaseThread", Thread::OPMODE_WAITFORWAKEUP),
...
RosMoveBaseThread::init()
{
  ...
  as_colli_ = new MoveBaseServer(**rosnode, "move_base_colli",
				   boost::bind(&RosMoveBaseThread::action_goal_cb, this, _1, COLLI),
				   boost::bind(&RosMoveBaseThread::action_cancel_cb, this, _1),
				   /* auto_start */ false);

  as_navgraph_ = new MoveBaseServer(**rosnode, "move_base",
				    boost::bind(&RosMoveBaseThread::action_goal_cb, this, _1, NAVGRAPH),
				    boost::bind(&RosMoveBaseThread::action_cancel_cb, this, _1),
				    /* auto_start */ false);
...
RosMoveBaseThread::stop()
{
  ...
  if (exec_type_ == NAVGRAPH) {
    if (nav_navgraph_if_->has_writer()) nav_navgraph_if_->msgq_enqueue(msg);
  } else {
    if (nav_colli_if_->has_writer()) nav_colli_if_->msgq_enqueue(msg);
  }
  if (exec_as_) {
...
void
RosMoveBaseThread::message_cb(geometry_msgs::PoseStampedConstPtr goal_pose, ExecType ext)
{
  MutexLocker lock(loop_mutex);
...
RosMoveBaseThread::create_feedback()
{
  ...
  try {
    ...
    feedback.base_position.header.stamp = ros::Time(transform_pose_out.stamp.get_sec(),
						    transform_pose_out.stamp.get_nsec());
    ...
    logger->log_warn(name(),
		     "Failed to determine fixed frame pose");
  }
...
RosMoveBaseThread::loop()
{
  if (exec_request_) {
    ...
    tf::Stamped<tf::Pose>
      target_pose_in(tf::Transform(tf::Quaternion(goal_pose_.pose.orientation.x,
		     ...
		     goal_time, goal_pose_.header.frame_id);
    ...
    try {
      ...
      logger->log_warn(name(),
		       ...
		       target_pose_in.frame_id.c_str(), target_frame.c_str());
    ...
    NavigatorInterface::CartesianGotoMessage *msg =
      new NavigatorInterface::CartesianGotoMessage(/* target_frame.c_str(), */ pos.x(), pos.y(),
						   tf::get_yaw(target_pose_out.getRotation()));

    logger->log_info(name(), "Goto (%f,%f,%f) in %s",
		     msg->x(), msg->y(), msg->orientation(), target_frame.c_str());
