...
void
RemoteBlackBoard::open_interface(const char *type,
                                 ...
                                 Interface * iface)
{
	...
	mutex_->lock();
	if (inbound_thread_ != NULL && Thread::current_thread()
	...
	while (
	  is_alive()
	  && (!m_ || ((m_->msgid() != MSG_BB_OPEN_SUCCESS) && (m_->msgid() != MSG_BB_OPEN_FAILURE)))) {
		if (m_) {
			m_->unref();
			m_ = NULL;
		}
	...
	if (!is_alive()) {
		throw Exception("Connection died while trying to open %s::%s", type, identifier);
	}

	if (m_->msgid() == MSG_BB_OPEN_SUCCESS) {
		// We got the interface, create internal storage and prepare instance for return
		BlackBoardInterfaceProxy *proxy =
		  new BlackBoardInterfaceProxy(fnc_, m_, notifier_, iface, writer);
...
{
	mutex_->lock();
	if (inbound_thread_ != NULL && strcmp(Thread::current_thread()->name(), inbound_thread_) == 0) {
	...
	FawkesNetworkMessage *omsg = new FawkesNetworkMessage(FAWKES_CID_BLACKBOARD, MSG_BB_LIST_ALL);
	wait_mutex_->lock();
	fnc_->enqueue(omsg);
	while (!m_ || (m_->msgid() != MSG_BB_INTERFACE_LIST)) {
		if (m_) {
			m_->unref();
			m_ = NULL;
		}
...
RemoteBlackBoard::list(const char *type_pattern, const char *id_pattern)
{
	...
	bb_ilistreq_msg_t *om = (bb_ilistreq_msg_t *)calloc(1, sizeof(bb_ilistreq_msg_t));
	strncpy(om->type_pattern, type_pattern, INTERFACE_TYPE_SIZE_ - 1);
	strncpy(om->id_pattern, id_pattern, INTERFACE_ID_SIZE_ - 1);

	FawkesNetworkMessage *omsg =
	  new FawkesNetworkMessage(FAWKES_CID_BLACKBOARD, MSG_BB_LIST, om, sizeof(bb_ilistreq_msg_t));
