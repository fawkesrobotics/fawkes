 ...
 */
BlackBoardNetworkHandler::BlackBoardNetworkHandler(BlackBoard *blackboard, FawkesNetworkHub *hub)
	...
	while (!inbound_queue_.empty()) {
		...
		case MSG_BB_LIST_ALL: {
			...
			try {
				...
				LibLogger::log_error("BlackBoardNetworkHandler",
				                     "Failed to send interface "
				                     "list to %u, exception follows",
				                     clid);
		...
		case MSG_BB_LIST: {
			...
			try {
				...
				LibLogger::log_error("BlackBoardNetworkHandler",
				                     "Failed to send "
				                     "interface list to %u, exception follows",
				                     clid);
				LibLogger::log_error("BlackBoardNetworkHandler", e);
			}
			delete infl;
		...
		case MSG_BB_OPEN_FOR_WRITING: {
			...
			LibLogger::log_debug("BlackBoardNetworkHandler", "Remote opens interface %s::%s", type, id);
			try {
				Interface *iface;
				...
				if (memcmp(iface->hash(), om->hash, INTERFACE_HASH_SIZE_) != 0) {
					...
					listeners_[iface->serial()] =
					  new BlackBoardNetHandlerInterfaceListener(bb_, iface, nhub_, clid);
		...
		case MSG_BB_CLOSE: {
			...
			if (interfaces_.find(sm_serial) != interfaces_.end()) {
				...
				if (client_interfaces_.find(clid) != client_interfaces_.end()) {
					...
					for (ciit_ = client_interfaces_[clid].begin(); ciit_ != client_interfaces_[clid].end();
					     ++ciit_) {
				...
				if (close) {
					...
					LibLogger::log_debug("BlackBoardNetworkHandler",
					                     "Remote %u closing interface %s",
					                     clid,
					                     interfaces_[sm_serial]->uid());
...
BlackBoardNetworkHandler::send_opensuccess(unsigned int clid, Interface *interface)
{
	...
	try {
		...
		LibLogger::log_error("BlackBoardNetworkHandler",
		                     "Failed to send interface "
		                     "open success to %u, exception follows",
		                     clid);
		LibLogger::log_error("BlackBoardNetworkHandler", e);
	}
}
...
BlackBoardNetworkHandler::send_openfailure(unsigned int clid, unsigned int error_code)
{
	...
	FawkesNetworkMessage *omsg = new FawkesNetworkMessage(
	  clid, FAWKES_CID_BLACKBOARD, MSG_BB_OPEN_FAILURE, ofm, sizeof(bb_iopenfail_msg_t));
	try {
		nhub_->send(omsg);
	} catch (Exception &e) {
		LibLogger::log_error("BlackBoardNetworkHandler",
		                     "Failed to send interface "
		                     "open failure to %u, exception follows",
		                     clid);
		LibLogger::log_error("BlackBoardNetworkHandler", e);
	}
}
...
BlackBoardNetworkHandler::client_disconnected(unsigned int clid)
{
	...
	if (client_interfaces_.find(clid) != client_interfaces_.end()) {
		...
		for (ciit_ = client_interfaces_[clid].begin(); ciit_ != client_interfaces_[clid].end();
		     ++ciit_) {
			LibLogger::log_debug("BlackBoardNetworkHandler",
			                     "Closing interface %s::%s of remote "
