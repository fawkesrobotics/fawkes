	...
	while (!quit) {
		...
		if (ti_writer->msgq_first() != NULL) {
			...
			if (ti_writer->msgq_first_is<TestInterface::SetTestIntMessage>()) {
				TestInterface::SetTestIntMessage *m2 =
				  ti_writer->msgq_first<TestInterface::SetTestIntMessage>();
			...
			if (val == expval) {
				...
				cout << " failure, value is " << ti_reader->test_int() << ", expected " << expval << endl;
			}
		} else {
...
main(int argc, char **argv)
{
	...
	cout << "Writing initial value (" << TestInterface::TEST_CONSTANT << ") into interface as TestInt"
	     << endl;
