 ...
 */
SyslogLogger::SyslogLogger(LogLevel log_level) : Logger(log_level)
{
 ...
 * @param log_level minimum level to log
 */
SyslogLogger::SyslogLogger(const char *ident, LogLevel log_level) : Logger(log_level)
{
	now_s = (struct ::tm *)malloc(sizeof(struct ::tm));
	mutex = new Mutex();
	if (ident == NULL) {
...
void
SyslogLogger::vtlog_debug(struct timeval *t, const char *component, const char *format, va_list va)
{
	if (log_level <= LL_DEBUG) {
		mutex->lock();
		localtime_r(&t->tv_sec, now_s);
