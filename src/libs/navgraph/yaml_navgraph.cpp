...
operator>>(const YAML::Node &n, NavGraphNode &node)
{
	...
	try {
		if (n["pos"].size() != 2) {
			throw Exception("Invalid position for node %s, "
			                "must be list of [x,y] coordinates",
			                name.c_str());
		}
		float x, y;
#ifdef HAVE_YAMLCPP_0_5
	...
	if (has_properties) {
		try {
			...
			throw Exception("Failed to read propery of %s: %s", name.c_str(), e.what());
		} // ignored
	}
...
save_yaml_navgraph(std::string filename, NavGraph *graph)
{
	...
	YAML::Emitter out;
	out << YAML::TrueFalseBool << YAML::BeginMap << YAML::Key << "graph-name" << YAML::Value
	    << graph->name();

	const std::map<std::string, std::string> &def_props = graph->default_properties();
	if (!def_props.empty()) {
		out << YAML::Key << "default-properties" << YAML::Value << YAML::BeginSeq;
		for (auto &p : def_props) {
			out << YAML::BeginMap << YAML::Key << p.first << YAML::Value << p.second << YAML::EndMap;
		}
		out << YAML::EndSeq;
	}
	...
	for (const NavGraphNode &node : nodes) {
		if (node.unconnected())
			out << YAML::LocalTag("unconnected");
		out << YAML::BeginMap << YAML::Key << "name" << YAML::Value << node.name() << YAML::Key << "pos"
