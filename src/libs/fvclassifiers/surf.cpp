...
void
saveIpoints(std::string                      sFileName,
 ...
 */
void
loadIpoints(std::string sFileName, std::vector<surf::Ipoint> &ipts, bool bVerbose, int &vlen_)
{
...
: Classifier("SurfClassifier")
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		std::cout << "SurfClassifier: reading the following descriptor file" << object_file
		          << std::endl;
...
: Classifier("SurfClassifier")
{
	...
	while ((ent = readdir(dir)) != NULL) {
		...
		std::cout << "SurfClassifier(classify): opening object image file '" << object_file << "'"
		          << std::endl;
		...
		std::cout << "SurfClassifier(classify): computed '" << obj_num_features_
...
SurfClassifier::classify()
{
  ...
  */
	...
	for (unsigned j = 0; j < num_obj_; j++) {
		...
		for (unsigned i = 0; i < obj_features_[j].size(); i++) {
			...
			if (match != -1) {
				...
				/// adding feature-ROI
				ROI r((int)(img_features_[matches[i]].x) - 5,
		...
		//#endif
		if (c == 0)
			std::cout << "SurfClassifier(classify) matched '" << c << fawkes::cnormal << "' of '"
			          << obj_features_[j].size() << "' features in scene. (for supplied object = " << j
			          << std::endl;
		else
			std::cout << "SurfClassifier(classify) matched '" << fawkes::cblue << c << fawkes::cnormal
		...
		std::cout << "SurfClassifier(classify): match_ratio is '" << match_ratio
		...
		if ((c != 0) && ((unsigned)c > min_match_) && (match_ratio > min_match_ratio_)
		    && (x_max - x_min != 0) && (y_max - y_min != 0)) {
			std::cout << "SurfClassifier(classify): c='" << c << "' min_match_='" << min_match_ << "'."
			          << std::endl;
	...
	std::cout << "SurfClassifier(classify): done,  ... returning '" << rv->size()
	          << "' ROIs. The object class is " << min_ratio_index << "and object name is "
	          << fawkes::cgreen << obj_name_tmp << fawkes::cnormal << std::endl;
	return final_rv;
}
