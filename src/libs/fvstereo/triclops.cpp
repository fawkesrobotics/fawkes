...
TriclopsStereoProcessor::setup_triclops()
{
  ...
  */
	...
	triclopsSetResolutionAndPrepare(data->triclops, _height, _width, _height, _width);
...
TriclopsStereoProcessor::~TriclopsStereoProcessor()
{
	...
	if (buffer_green != NULL)
		free(buffer_green);
	if (buffer_yuv_right != NULL)
		free(buffer_yuv_right);
	if (buffer_yuv_left != NULL)
		free(buffer_yuv_left);
	...
	if (!bb2) {
		if (buffer_rgb)
			free(buffer_rgb);
		if (buffer_deinterlaced)
			free(buffer_deinterlaced);
	}
...
TriclopsStereoProcessor::preprocess_stereo()
{
	if (bb2) {
		...
		Bumblebee2Camera::deinterlace_stereo(buffer_raw16, buffer_deinterlaced, _width, _height);
		Bumblebee2Camera::decode_bayer(
		  buffer_deinterlaced, buffer_rgb, _width, _height, BAYER_PATTERN_BGGR);
	}
}
...
TriclopsStereoProcessor::calculate_disparity(ROI *roi)
{
	...
	if (data->enable_subpixel_interpolation) {
		triclopsGetImage16(data->triclops,
		                   TriImg16_DISPARITY,
		                   TriCam_REFERENCE,
		                   &(data->disparity_image_hires));
	} else {
		triclopsGetImage(data->triclops,
		                 TriImg_DISPARITY,
		                 TriCam_REFERENCE,
		                 &(data->disparity_image_lores));
	}
	/*
  if ( data->enable_subpixel_interpolation ) {
...
TriclopsStereoProcessor::generate_rectification_lut(const char *lut_file)
{
	...
	RectificationLutInfoBlock *lib_left =
	  new RectificationLutInfoBlock(_width, _height, FIREVISION_RECTINFO_CAMERA_LEFT);
	RectificationLutInfoBlock *lib_right =
	  new RectificationLutInfoBlock(_width, _height, FIREVISION_RECTINFO_CAMERA_RIGHT);

	register float row, col;
	for (unsigned int h = 0; h < _height; ++h) {
		for (unsigned int w = 0; w < _width; ++w) {
			if (triclopsUnrectifyPixel(data->triclops, TriCam_LEFT, h, w, &row, &col)
			    != TriclopsErrorOk) {
			...
			if (triclopsUnrectifyPixel(data->triclops, TriCam_RIGHT, h, w, &row, &col)
			    != TriclopsErrorOk) {
				throw Exception("Failed to get unrectified position from Triclops SDK");
			}
 ...
 */
void
TriclopsStereoProcessor::getall_world_xyz(float ***buffer,
                                          int      hoff,
                                          int      voff,
                                          int      width,
                                          int      height,
		...
		for (py = 0; py < height; py++) {
			...
			for (px = 0; px < width; px++) {
				if (disp[displine + px + hoff] >= 0xFF00) {
					...
					triclopsRCD16ToXYZ(
					  data->triclops, py + voff, px + hoff, disp[displine + px + hoff], &fx, &fy, &fz);
