 ...
 */
PNMWriter::PNMWriter(PNMFormat format) : Writer("pnm")
{
	this->format = format;
...
PNMWriter::set_buffer(colorspace_t cspace, unsigned char *yuv422_planar_buf)
{
	...
	if ((format == PNM_PBM) || (format == PNM_PBM_ASCII)) {
...
PNMWriter::format2string(PNMFormat format)
{
	...
	default: throw Exception("Unknown PNMFormat");
	}
}
...
PNMWriter::calc_buffer_size()
{
	...
	case PNM_PGM: num_row_bytes = width; break;

	case PNM_PGM_ASCII: num_row_bytes = width * 4; break;

	case PNM_PPM: num_row_bytes = 3 * width; break;
