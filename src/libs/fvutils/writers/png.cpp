...
/** Constructor. */
PNGWriter::PNGWriter() : Writer("png")
{
}
 ...
 * @param height height
 */
PNGWriter::PNGWriter(const char *filename, unsigned int width, unsigned int height) : Writer("png")
{
...
PNGWriter::write()
{
	...
	for (unsigned int i = 0; i < height; ++i) {
		if (colorspace_ == YUV422_PLANAR) {
			...
			convert_line_bgr_rgb((buffer + width * 3 * i), row, width, height);
