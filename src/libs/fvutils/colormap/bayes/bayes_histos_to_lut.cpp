 ...
 */
float
BayesHistosToLut::getAPrioriProb(unsigned int u, unsigned int v, hint_t object)
{
	unsigned int sum = 0;
 ...
 */
float
BayesHistosToLut::getAPrioriProb(unsigned int y, unsigned int u, unsigned int v, hint_t object)
{
 ...
 */
float
BayesHistosToLut::getAPosterioriProb(hint_t object, unsigned int u, unsigned int v)
{
	...
	for (hit = histograms.begin(); hit != histograms.end(); hit++) {
		sumOfProbabilities +=
		  (getAPrioriProb(u, v, (hint_t)hit->first) * getObjectProb((hint_t)hit->first));
	}
 ...
 */
float
BayesHistosToLut::getAPosterioriProb(hint_t object, unsigned int y, unsigned int u, unsigned int v)
{
	/* calculate "nenner" for bayes-formula,
     i.e. sum up the probabilities P(u, v| object) * P(object)
     over all objects */
	...
	for (hit = histograms.begin(); hit != histograms.end(); hit++) {
		sumOfProbabilities +=
		  (getAPrioriProb(y, u, v, (hint_t)hit->first) * getObjectProb((hint_t)hit->first));
	}
 ...
 */
hint_t
BayesHistosToLut::getMostLikelyObject(unsigned int u, unsigned int v)
{
 ...
 */
hint_t
BayesHistosToLut::getMostLikelyObject(unsigned int y, unsigned int u, unsigned int v)
{
	hint_t                             mostLikelyObject       = H_UNKNOWN;
	float                              probOfMostLikelyObject = 0.0;
	map<hint_t, Histogram *>::iterator hit;
	...
	for (unsigned int y = 0; y < depth; ++y) {
		...
		for (unsigned int v = 0; v < height; ++v) {
			for (unsigned int u = 0; u < width; ++u) {
				...
				highest_prob = 0.0;
				color_with_highest_prob =
				  H_UNKNOWN; // ...maybe it is better to have default = H_BACKGROUND...
				map<hint_t, Histogram *>::iterator hit;
				for (hit = histograms.begin(); hit != histograms.end(); hit++) {
					// if current histogram is not empty...
					if (numberOfOccurrences[(hint_t)hit->first] > 0) {
						current_prob =
						  float(hit->second->get_value(u, v, y)) / float(numberOfOccurrences[hit->first]);
						// if current histogram has higher probability for color (u, v),
						// _and_ is above min_prob-threshold...
						if (current_prob > highest_prob && current_prob > min_probability) {
...
BayesHistosToLut::calculateLutValues(bool penalty)
{
	...
	if (penalty) {
		...
		if (histo_bg->get_num_undos() < 2) {
			...
			cout << "Histogram::calculateLutValues: There are not enough undos possible for background "
			        "histogram, not penalizing"
			     << endl;
		} else {
	...
	for (unsigned int y = 0; y < depth; ++y) {
		...
		for (unsigned int u = 0; u < width; ++u) {
			...
			for (unsigned int v = 0; v < height; ++v) {
				...
				case H_BALL: count_ball++; break;
				case H_BACKGROUND: count_background++; break;
				case H_ROBOT:
				case H_ROBOT_OPP: count_robot++; break;
				case H_FIELD: count_field++; break;
				case H_LINE: count_line++; break;
				case H_GOAL_YELLOW:
				case H_GOAL_BLUE: count_goal++; break;
				case H_UNKNOWN: count_unknown++; break;
				default:
	...
	printf("d/w/h: %u/%u/%u  ball: %d  field: %d  line: %d  robot: %d  goal: %d  background: %d  "
	       "unknown: %d\n",
	       depth,
	       width,
	       height,
...
{
	switch (hint) {
	case H_BALL: min_prob_ball = min_prob; break;
	case H_FIELD: min_prob_green = min_prob; break;
	case H_GOAL_YELLOW: min_prob_yellow = min_prob; break;
	case H_GOAL_BLUE: min_prob_blue = min_prob; break;
	case H_LINE: min_prob_white = min_prob; break;
	case H_ROBOT: min_prob_black = min_prob; break;
	default:
		/**/
		break;
	}
