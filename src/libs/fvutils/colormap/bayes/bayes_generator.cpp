 ...
 */
BayesColormapGenerator::BayesColormapGenerator(unsigned int lut_depth,
                                               hint_t       fg_object,
 ...
 */
void
BayesColormapGenerator::set_buffer(unsigned char *buffer, unsigned int width, unsigned int height)
{
	this->buffer = buffer;
	image_width  = width;
	image_height = height;
...
BayesColormapGenerator::load_histograms(const char *filename)
{
	...
	for (histo_it = bg_histos.begin(); histo_it != bg_histos.end(); ++histo_it) {
		hint_t cur_object = histo_it->first;
		float  factor =
		  (norm_size - fg_histos[cur_object]->get_sum()) / float(histo_it->second->get_sum());
