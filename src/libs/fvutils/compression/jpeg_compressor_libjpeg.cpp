...
fv_jpeg_memory_destination_setup(j_compress_ptr cinfo, JOCTET *buffer, int bufsize)
{
	...
	if (cinfo->dest == NULL) {
		cinfo->dest = (struct jpeg_destination_mgr *)(*cinfo->mem->alloc_small)(
		  (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(fv_jpeg_memory_destination_mgr_t));
	}
	...
	if (jpeg_cs == JpegImageCompressor::JPEG_CS_RGB) {
		if (vflip) {
			while (cinfo.next_scanline < cinfo.image_height) {
				convert_line_yuv422planar_to_rgb(buffer,
				                                 row_buffer,
				                                 cinfo.image_width,
				                                 cinfo.image_height,
			...
			while (cinfo.next_scanline < cinfo.image_height) {
				convert_line_yuv422planar_to_rgb(
				  buffer, row_buffer, cinfo.image_width, cinfo.image_height, cinfo.next_scanline, 0);
				jpeg_write_scanlines(&cinfo, &row_buffer, 1);
			}
		}
	} else {
		if (vflip) {
			while (cinfo.next_scanline < cinfo.image_height) {
				convert_line_yuv422planar_to_yuv444packed(buffer,
				                                          row_buffer,
				                                          cinfo.image_width,
				                                          cinfo.image_height,
				                                          cinfo.image_height - cinfo.next_scanline - 1,
...
bool
JpegImageCompressorLibJpeg::supports_compression_destination(
