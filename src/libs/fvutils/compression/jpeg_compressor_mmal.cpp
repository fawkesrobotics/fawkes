...
encoder_output_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
	...
	JpegImageCompressorMMAL::State *state = (JpegImageCompressorMMAL::State *)port->userdata;
...
static void
encoder_input_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
...
JpegImageCompressorMMAL::compress()
{
	...
	if ((buffer = mmal_queue_get(state_->encoder_pool_in->queue)) != NULL) {
		...
		if (vflip_) {
			...
			for (h = 0; h < encoder_input->format->es->video.height; ++h) {
				memcpy(data,
				       imgb + (width_ * height_) + ((width_ / 2) * height_)
				         + ((height_ - h - 1) * (width_ / 2)),
				       width_ / 2);
...
bool
JpegImageCompressorMMAL::supports_compression_destination(
  ImageCompressor::CompressionDestination cd)
{
	return true;
}
...
JpegImageCompressorMMAL::create_encoder_component()
{
	...
	status = mmal_port_parameter_set_boolean(encoder_output, MMAL_PARAMETER_EXIF_DISABLE, 1);
	if (status != MMAL_SUCCESS) {
		mmal_component_destroy(encoder);
	...
	pool =
	  mmal_port_pool_create(encoder_output, encoder_output->buffer_num, encoder_output->buffer_size);

	if (!pool) {
		mmal_component_destroy(encoder);
		throw Exception("Failed to create buffer header pool for encoder output port %s",
		                encoder_output->name);
	}
	...
	pool =
	  mmal_port_pool_create(encoder_input, encoder_input->buffer_num, encoder_input->buffer_size);

	if (!pool) {
		mmal_component_destroy(encoder);
		throw Exception("Failed to create buffer header pool for encoder input port %s",
		                encoder_input->name);
	}
