...
JpegImageDecompressor::decompress()
{
	...
	buffer = (unsigned char *)malloc((size_t)cinfo.output_width * (size_t)cinfo.output_height
	                                 * (size_t)cinfo.num_components);

	row_pointer[0] =
	  (unsigned char *)malloc((size_t)cinfo.output_width * (size_t)cinfo.num_components);
	...
	convert(
	  RGB, YUV422_PLANAR, buffer, _decompressed_buffer, cinfo.output_width, cinfo.output_height);
