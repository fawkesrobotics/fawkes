 ...
 */
void
Zauberstab::setBuffer(unsigned char *b, unsigned int w, unsigned int h)
{
	this->buffer = b;
	this->width  = w;
	this->height = h;
...
Zauberstab::deleteRegion(ZRegion *region2)
{
	...
	while (region2->slices->size()) {
		...
		while (i < size) //for all existing slices (but not the newly added)
		{
			...
			{
				if ((oSlice->leftX >= nSlice->leftX) && (oSlice->leftX < nSlice->rightX)) {
				...
				if ((nSlice->leftX >= oSlice->leftX) && (nSlice->leftX < oSlice->rightX)) {
					//The slice to be deleted starts before the part that should be deleted
					if (oSlice->rightX <= nSlice->rightX) { //just truncate the old slices
...
Zauberstab::privFindRegion(unsigned int seedX, unsigned int seedY)
{
	...
	for (std::vector<ZSlice *>::iterator it = region2->slices->begin(); it != region2->slices->end();
	     ++it) {
		cout << "start x: " << ((*it)->leftX) << " end x: " << ((*it)->rightX) << " y: " << ((*it)->y)
		     << endl;
	}
	cout << endl;
	return region2;
 ...
 */
ZSlice *
Zauberstab::findSlice(unsigned int x, unsigned int y, unsigned int vSeed, int uSeed)
{
	...
	while (region2->slices->size()) {
       ...
       at a height different from all slices at region */
		...
		while (i < region->slices->size()) //for all existing slices
		{
			...
			{
				if (((oSlice->leftX >= nSlice->leftX) && (oSlice->leftX <= nSlice->rightX))
				    || ((nSlice->leftX >= oSlice->leftX) && (nSlice->leftX <= oSlice->rightX))) {
 ...
 */
bool
Zauberstab::isSimilarV(unsigned int v1, unsigned int v2)
{
 ...
 */
bool
Zauberstab::isSimilarU(unsigned int u1, unsigned int u2)
{
 ...
 */
bool
Zauberstab::isSimilarUV(unsigned int u1, unsigned int u2, unsigned int v1, unsigned int v2)
{
