...
PNMReader::PNMReader(const char *filename)
{
	...
	token = strtok(line, " ");
	if (atoi(token) >= 0) {
		m_img_width = (unsigned int)atoi(token);
	} else {
		throw Exception("PNMReader::ctor: could not read out image width");
	};
	token = strtok(NULL, " ");
	if (atoi(token) >= 0) {
		m_img_height = (unsigned int)atoi(token);
	} else {
		throw Exception("PNMReader::ctor: could not read out image height");
	};
	free(tmp);
	...
	if (max >= 0) {
		switch (max) {
		case 1: m_img_depth = 1; break;

		case 15: m_img_depth = 2; break;

		case 255: m_img_depth = 3; break;
