...
SyncPoint::handle_default(string component, WakeupType type)
{
	...
	if (bad_components_.size() > 1) {
		...
		logger_->log_debug(component.c_str(), "bad components:%s", bad_components_string.c_str());
	} else if (bad_components_.size() == 1) {
		logger_->log_debug(component.c_str(), "bad component: %s", bad_components_.begin()->c_str());
