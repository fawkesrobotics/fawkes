...
/** Constructor. */
FilterOr::FilterOr() : Filter("FilterOr", 2)
{
}
...
FilterOr::apply()
{
	...
	if ((dst == NULL) || (dst == src[1])) {
		// In-place
		status = ippiOr_8u_C1IR(src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
		                          + (src_roi[0]->start.x * src_roi[0]->pixel_step),
		                        src_roi[0]->line_step,
		                        src[1] + (src_roi[1]->start.y * src_roi[1]->line_step)
		                          + (src_roi[1]->start.x * src_roi[1]->pixel_step),
		                        src_roi[1]->line_step,
		                        size);

	} else {
		status = ippiOr_8u_C1R(src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
		                         + (src_roi[0]->start.x * src_roi[0]->pixel_step),
		                       src_roi[0]->line_step,
		                       src[1] + (src_roi[1]->start.y * src_roi[1]->line_step)
		                         + (src_roi[1]->start.x * src_roi[1]->pixel_step),
	...
	cv::Mat srcm_0(src_roi[0]->height,
	               src_roi[0]->width,
	               CV_8UC1,
	...
	cv::Mat srcm_1(src_roi[1]->height,
	               src_roi[1]->width,
	               CV_8UC1,
	               src[1] + (src_roi[1]->start.y * src_roi[1]->line_step)
	                 + (src_roi[1]->start.x * src_roi[1]->pixel_step),
