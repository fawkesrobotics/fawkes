 ...
 */
FilterSobel::FilterSobel(orientation_t ori) : Filter("FilterSobel")
{
}
  ...
  orientation_t ori)
{
	...
	case ORI_DEG_315:
		...
		k[8] = 0;
		break;
	default: throw fawkes::Exception("Cannot generate Sobel kernel for the given orientation"); break;
	}
}
...
FilterSobel::apply()
{
	...
	if (ori[0] == ORI_HORIZONTAL) {
		...
		status = ippiFilterSobelHoriz_8u_C1R(src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
	...
	} else if ((ori[0] == ORI_DEG_0) || (ori[0] == ORI_DEG_45) || (ori[0] == ORI_DEG_90)
	           || (ori[0] == ORI_DEG_135) || (ori[0] == ORI_DEG_180) || (ori[0] == ORI_DEG_225)
	           || (ori[0] == ORI_DEG_270) || (ori[0] == ORI_DEG_315) || (ori[0] == ORI_DEG_360)) {
		Ipp32s kernel[9];
		...
		status = ippiFilter_8u_C1R(src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
		                             + (src_roi[0]->start.x * src_roi[0]->pixel_step),
		                           src_roi[0]->line_step,
		                           dst + (dst_roi->start.y * dst_roi->line_step)
		                             + (dst_roi->start.x * dst_roi->pixel_step),
