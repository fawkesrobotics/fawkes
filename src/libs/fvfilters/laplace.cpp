...
/** Constructor. */
FilterLaplace::FilterLaplace() : Filter("FilterLaplace")
{
	kernel       = NULL;
	kernel_float = NULL;
}
 ...
 */
FilterLaplace::FilterLaplace(float sigma, unsigned int size, float scale) : Filter("FilterLaplace")
{
	kernel_size = size;
...
FilterLaplace::apply()
{
	...
	if (kernel == NULL) {
		...
		status = ippiFilterLaplace_8u_C1R(src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
    ...
    */
		...
		status =
		  ippiFilter_8u_C1R(src[0] + ((src_roi[0]->start.y + kernel_size / 2) * src_roi[0]->line_step)
		                      + ((src_roi[0]->start.x + kernel_size / 2) * src_roi[0]->pixel_step),
		                    src_roi[0]->line_step,
		                    dst + ((dst_roi->start.y + kernel_size / 2) * dst_roi->line_step)
		                      + ((dst_roi->start.x + kernel_size / 2) * dst_roi->pixel_step),
		                    dst_roi->line_step,
		                    size,
		                    kernel,
		                    ksize,
