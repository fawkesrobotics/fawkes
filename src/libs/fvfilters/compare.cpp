...
/** Constructor. */
FilterCompare::FilterCompare() : Filter("FilterCompare", 2)
{
}
...
FilterCompare::apply()
{
	...
	if (src[FOREGROUND] == NULL)
		return;
	if (src_roi[BACKGROUND] == NULL)
		return;
	if (src_roi[FOREGROUND] == NULL)
		return;
	...
	// y-plane
	unsigned char *byp = src[BACKGROUND]
	                     + (src_roi[BACKGROUND]->start.y * src_roi[BACKGROUND]->line_step)
	                     + (src_roi[BACKGROUND]->start.x * src_roi[BACKGROUND]->pixel_step);

	// y-plane
	unsigned char *fyp = src[FOREGROUND]
	                     + (src_roi[FOREGROUND]->start.y * src_roi[FOREGROUND]->line_step)
	                     + (src_roi[FOREGROUND]->start.x * src_roi[FOREGROUND]->pixel_step);
	// u-plane
	unsigned char *fup = YUV422_PLANAR_U_PLANE(src[FOREGROUND],
	                                           src_roi[FOREGROUND]->image_width,
	                                           src_roi[FOREGROUND]->image_height)
	                     + ((src_roi[FOREGROUND]->start.y * src_roi[FOREGROUND]->line_step) / 2
	                        + (src_roi[FOREGROUND]->start.x * src_roi[FOREGROUND]->pixel_step) / 2);
	// v-plane
	unsigned char *fvp = YUV422_PLANAR_V_PLANE(src[FOREGROUND],
	                                           src_roi[FOREGROUND]->image_width,
	                                           src_roi[FOREGROUND]->image_height)
	                     + ((src_roi[FOREGROUND]->start.y * src_roi[FOREGROUND]->line_step) / 2
	                        + (src_roi[FOREGROUND]->start.x * src_roi[FOREGROUND]->pixel_step) / 2);
