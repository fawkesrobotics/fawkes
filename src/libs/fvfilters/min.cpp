...
/** Constructor. */
FilterMin::FilterMin() : Filter("FilterMin", 2)
{
}
...
{
	if (src[0] == NULL)
		throw NullPointerException("FilterInvert: src buffer 0 is NULL");
	if (src[1] == NULL)
		throw NullPointerException("FilterInvert: src buffer 1 is NULL");
	if (src_roi[0] == NULL)
		throw NullPointerException("FilterInvert: src ROI 0 is NULL");
	if (src_roi[1] == NULL)
		throw NullPointerException("FilterInvert: src ROI 1 is NULL");

	unsigned int h = 0;
	unsigned int w = 0;

	// y-plane
	unsigned char *byp = src[0] + (src_roi[0]->start.y * src_roi[0]->line_step)
	...
	// y-plane
	unsigned char *fyp = src[1] + (src_roi[1]->start.y * src_roi[1]->line_step)
	                     + (src_roi[1]->start.x * src_roi[1]->pixel_step);
	// u-plane
	unsigned char *fup =
	  YUV422_PLANAR_U_PLANE(src[1], src_roi[1]->image_width, src_roi[1]->image_height)
	  + ((src_roi[1]->start.y * src_roi[1]->line_step) / 2
	     + (src_roi[1]->start.x * src_roi[1]->pixel_step) / 2);
	// v-plane
	unsigned char *fvp =
	  YUV422_PLANAR_V_PLANE(src[1], src_roi[1]->image_width, src_roi[1]->image_height)
	  + ((src_roi[1]->start.y * src_roi[1]->line_step) / 2
	     + (src_roi[1]->start.x * src_roi[1]->pixel_step) / 2);
