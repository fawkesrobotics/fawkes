...
InterfaceParser::getFields(xmlpp::Node *node, const std::set<std::string> &reserved_names)
{
	...
	for (NodeSet::iterator i = set.begin(); i != set.end(); ++i) {
		...
		if (el) {
			...
			for (Element::AttributeList::const_iterator iter = attrs.begin(); iter != attrs.end();
			     ++iter) {
		...
		if (nameset.size() == 0) {
			throw InterfaceGeneratorInvalidContentException("no comment for field %s",
			                                                f.getName().c_str());
		}
...
InterfaceParser::getPseudoMaps(xmlpp::Node *node, std::vector<InterfaceField> &fields)
{
	...
	for (NodeSet::iterator i = set.begin(); i != set.end(); ++i) {
		...
		if (el) {
			...
			if (!attr)
				throw InterfaceGeneratorInvalidContentException("no name for pseudo map");
			pm_name = attr->get_value();

			attr = el->get_attribute("type");
			if (!attr)
				throw InterfaceGeneratorInvalidContentException("no type for pseudo map");
			pm_type = attr->get_value();

			attr = el->get_attribute("keytype");
			if (!attr)
				throw InterfaceGeneratorInvalidContentException("no key type for pseudo map");
					...
					if ((*j).getName() == text_node->get_content()) {
						...
						if (j->getLengthValue() > 0) {
							throw InterfaceGeneratorInvalidContentException(
							  "pseudomap references may only point to non-map types");
						}
...
InterfaceParser::printPseudoMaps(vector<InterfacePseudoMap> &pseudo_maps)
{
	for (vector<InterfacePseudoMap>::iterator i = pseudo_maps.begin(); i != pseudo_maps.end(); ++i) {
		cout << "  PseudoMap: name=" << i->getName() << "  type=" << i->getType()
		...
		for (j = reflist.begin(); j != reflist.end(); ++j) {
			cout << "    Ref:  field=" << j->first << "  key=" << j->second << endl;
		}
     ...
     */
		...
		for (NodeSet::iterator i = set.begin(); i != set.end(); ++i) {
			...
			for (NodeSet::iterator j = items.begin(); j != items.end(); ++j) {
				...
				if (comment_node) {
					if (item_value != "") {
						enum_constant.add_item(item_name,
						                       comment_node->get_content(),
   ...
   */
	...
	for (NodeSet::iterator i = set.begin(); i != set.end(); ++i) {
		...
		if (el) {
			...
			if (msg_name.length() > INTERFACE_MESSAGE_TYPE_SIZE_) {
				throw InterfaceGeneratorInvalidContentException(
				  "Interface message name too long, max length is %u", INTERFACE_MESSAGE_TYPE_SIZE_);
			}
		} else {
		...
		for (NodeSet::iterator r = ref_nodes.begin(); r != ref_nodes.end(); ++r) {
			...
			if (text_node) {
				// find field in data fields
				bool found = false;
				for (vector<InterfaceField>::iterator j = data_fields.begin(); j != data_fields.end();
				     ++j) {
					if ((*j).getName() == text_node->get_content()) {
						// field found
