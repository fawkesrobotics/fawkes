	...
	for (vector<InterfaceEnumConstant>::iterator i = enum_constants.begin();
		...
		while (j != items.end()) {
			if (j->has_custom_value) {
				fprintf(f, "    %s = %i /**< %s */", j->name.c_str(), j->custom_value, j->comment.c_str());
			} else {
...
CppInterfaceGenerator::write_copy_value_method_cpp(FILE *f)
{
	fprintf(f,
	        ...
	        "  if (oi == NULL) {\n"
	        "    throw TypeMismatchException(\"Can only copy values from interface of same type (%%s "
	        "vs. %%s)\",\n"
	        "                                type(), other->type());\n"
	        "  }\n"
                                                 ...
                                                 std::vector<InterfaceField>    fields)
{
	...
	if (fields.size() > 0) {
		...
		i = fields.begin();
		while (i != fields.end()) {
			fprintf(f, "const %s ini_%s", (*i).getAccessType().c_str(), (*i).getName().c_str());
			++i;
			if (i != fields.end()) {
				fprintf(f, ", ");
			}
                                                   ...
                                                   std::vector<InterfaceField> fields)
{
	...
	if (fields.size() > 0) {
		fprintf(f, "/** Constructor with initial values.\n");
                                         ...
                                         bool                        write_data_changed)
{
	...
	for (vector<InterfaceField>::iterator i = fields.begin(); i != fields.end(); ++i) {
		fprintf(f,
		        ...
		        (*i).getName().c_str(),
		        (*i).isEnumType()
		          ? (std::string("(") + interface_classname + "::" + i->getAccessType() + ")").c_str()
		          : "",
		        (*i).getName().c_str());
		...
		if ((i->getLengthValue() > 0) && (i->getType() != "string")) {
			fprintf(
			  ...
			  i->getMaxIdx().c_str(),
			  (*i).isEnumType()
			    ? (std::string("(") + interface_classname + "::" + i->getPlainAccessType() + ")").c_str()
			    : "",
			  (*i).getName().c_str());
		}
