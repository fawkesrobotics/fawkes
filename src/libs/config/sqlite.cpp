...
#define SQL_ATTACH_DEFAULTS "ATTACH DATABASE '%s' AS defaults"

#define SQL_ATTACH_MODIFIED "ATTACH DATABASE ':memory:' AS modified"

#define SQL_ATTACH_DUMPED "ATTACH DATABASE '%s' AS dumped"

#define SQL_DETACH_DUMPED "DETACH DATABASE dumped"
...
#define SQL_SELECT_COMMENT "SELECT comment, 0 AS is_default FROM config WHERE path=?"

#define SQL_SELECT_DEFAULT_COMMENT                              \
	"SELECT comment, 1 AS is_default FROM defaults.config AS dc " \
	"WHERE dc.path=?"

#define SQL_UPDATE_VALUE "UPDATE config SET value=? WHERE path=?"

#define SQL_UPDATE_DEFAULT_VALUE "UPDATE defaults.config SET value=? WHERE path=?"

#define SQL_UPDATE_COMMENT "UPDATE config SET comment=? WHERE path=?"

#define SQL_UPDATE_DEFAULT_COMMENT "UPDATE defaults.config SET comment=? WHERE path=?"

#define SQL_INSERT_VALUE "INSERT INTO config (path, type, value) VALUES (?, ?, ?)"

#define SQL_INSERT_DEFAULT_VALUE "INSERT INTO defaults.config (path, type, value) VALUES (?, ?, ?)"
...
#define SQL_SELECT_ALL_DEFAULT "SELECT *, 1 AS is_default FROM defaults.config"

#define SQL_SELECT_ALL_HOSTSPECIFIC "SELECT *, 0 AS is_default FROM config"

#define SQL_DELETE_VALUE "DELETE FROM config WHERE path=?"

#define SQL_DELETE_DEFAULT_VALUE "DELETE FROM defaults.config WHERE path=?"
...
#define SQL_SELECT_MODIFIED_ALL "SELECT * FROM modified.config"
 ...
 */
SQLiteConfiguration::SQLiteConfiguration(const char *sysconfdir, const char *userconfdir)
{
	opened = false;
	mutex  = new Mutex();
...
SQLiteConfiguration::import_default(const char *default_sql)
{
	...
	if ((sqlite3_exec(db, SQL_ATTACH_MODIFIED, NULL, NULL, &errmsg) != SQLITE_OK)
		...
		CouldNotOpenConfigException ce("Could not create or attach modified memory database: %s",
		                               errmsg);
		sqlite3_free(errmsg);
		throw ce;
	}
...
void
SQLiteConfiguration::set_string(const char *path, const char *s)
{
	sqlite3_stmt *stmt = NULL;
...
void
SQLiteConfiguration::set_default_string(const char *path, const char *s)
{
	sqlite3_stmt *stmt = NULL;
