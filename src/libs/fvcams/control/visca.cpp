 ...
 */
ViscaControlException::ViscaControlException(const char *msg) : Exception(msg)
{
}
...
{
	if (!opened)
		throw ViscaControlException("Serial port not open");
...
ViscaControl::recv(unsigned int max_wait_ms)
{
	...
	default: throw ViscaControlException("Receiving failed, unexpected packet type received");
	}
}
...
ViscaControl::cancel_command(unsigned int socket)
{
	...
	if (((ibuffer[1] & 0xF0) == VISCA_RESPONSE_ERROR) && ((ibuffer[1] & 0x0F) == cancel_socket)
...
{
	if (inquire)
		throw ViscaControlInquiryRunningException();
