...
void
SonyEviD100PControl::set_pan_tilt_limit(int pan_left, int pan_right, int tilt_up, int tilt_down)
{
...
SonyEviD100PControl::supports_effect(unsigned int effect_)
{
	...
	case EFFECT_MOSAIC:
	case EFFECT_SLIM:
	case EFFECT_STRETCH: return true; break;
...
SonyEviD100PControl::set_effect(unsigned int effect_)
{
	...
	switch (effect_) {
	case EFFECT_PASTEL: visca->applyEffectPastel(); break;
	case EFFECT_NEGATIVE: visca->applyEffectNegArt(); break;
	case EFFECT_SEPIA: visca->applyEffectSepia(); break;
	case EFFECT_BW: visca->applyEffectBnW(); break;
	case EFFECT_SOLARIZE: visca->applyEffectSolarize(); break;
	case EFFECT_MOSAIC: visca->applyEffectMosaic(); break;
	case EFFECT_SLIM: visca->applyEffectSlim(); break;
	case EFFECT_STRETCH: visca->applyEffectStretch(); break;
	default: break;
	}
}
