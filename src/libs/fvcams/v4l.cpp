...
V4LCamera::V4LCamera(const CameraArgumentParser *cap)
{
	...
	if (cap->has("device"))
		...
		throw fawkes::MissingParameterException("Missing device for V4lCamera");
}
...
V4LCamera::open()
{
	...
	if (dev < 0)
		throw fawkes::Exception("V4LCam: Could not open device");
...
{
	if (!_v4l_cam)
		throw fawkes::Exception("V4LCam: Trying to start closed cam!");
...
{
	if (!_v4l_cam)
		throw fawkes::Exception("V4LCam: Trying to stop closed cam!");
...
{
	if (!_v4l_cam)
		throw fawkes::Exception("V4LCam::pixel_width(): Camera not opened");
...
{
	if (!_v4l_cam)
		throw fawkes::Exception("V4LCam::pixel_height(): Camera not opened");
