...
typedef struct
{
	...
	uint32_t
	  camera_type_id : 16; /**< See Table 122: Camera type ID list on page 267 in the technical manual (v 4.3.0). */
} avt_version_info3_t;

/** Constructor.
 * @param cap Camera argument parser.
 */
PikeCamera::PikeCamera(const CameraArgumentParser *cap) : FirewireCamera(cap)
{
	aoi_left_           = 0;
	aoi_top_            = 0;
	aoi_width_          = 0;
...
PikeCamera::open()
{
	...
	if (!set_autofunction_aoi(aoi_left_, aoi_top_, aoi_width_, aoi_height_, aoi_show_work_area_)) {
...
PikeCamera::print_info()
{
	...
	uint32_t      value;
	dc1394error_t err = dc1394_get_register(_camera, AVT_VERSION_INFO1_REGISTER, &value);
	...
	err = dc1394_get_register(_camera, AVT_VERSION_INFO3_REGISTER, &value);
	...
	printf("uC type ID: %d  uC version: %x  camera type id: %d   FPGA version: %x\n",
	       version1.uc_type_id,
	       version1.uc_version,
	       version3.camera_type_id,
	       version3.fpga_version);
}
