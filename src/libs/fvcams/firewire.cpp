...
FirewireCamera::open()
{
	...
	if (_format7_mode_enabled) {
		...
		if (((err = dc1394_format7_set_image_size(_camera, _mode, _format7_width, _format7_height))
		     != DC1394_SUCCESS)
		    || ((err =
		           dc1394_format7_set_image_position(_camera, _mode, _format7_startx, _format7_starty))
		        != DC1394_SUCCESS)
		    || ((err = dc1394_format7_set_color_coding(_camera, _mode, _format7_coding))
		        != DC1394_SUCCESS)
		    || ((err = dc1394_format7_set_packet_size(_camera, _mode, _format7_bpp))
		        != DC1394_SUCCESS)) {
	...
	if (!_auto_white_balance && (_white_balance_ub != 0xFFFFFFFF) && (_white_balance_vr != 0xFFFFFFFF)
...
FirewireCamera::start()
{
	...
	dc1394error_t err;
	if ((err = dc1394_capture_setup(_camera, _num_buffers, DC1394_CAPTURE_FLAGS_DEFAULT))
	    != DC1394_SUCCESS) {
	...
	if ((err = dc1394_video_set_transmission(_camera, DC1394_ON)) != DC1394_SUCCESS) {
		...
		dc1394_capture_stop(_camera);
		throw Exception("FirewireCamera: Could not start ISO transmission (%s)",
...
FirewireCamera::capture()
{
	...
	dc1394error_t err;
	if (DC1394_SUCCESS
	    != (err = dc1394_capture_dequeue(_camera, DC1394_CAPTURE_POLICY_WAIT, &_frame))) {
		_valid_frame_received = false;
...
FirewireCamera::pixel_width()
{
	if (_opened) {
		if (_valid_frame_received) {
			...
			dc1394error_t err;
			if ((err = dc1394_get_image_size_from_video_mode(_camera, _mode, &width, &height))
			    != DC1394_SUCCESS) {
				throw Exception("FirewireCamera(%s): cannot get width (%s)",
				                _model,
				                dc1394_error_get_string(err));
			}
			return width;
...
FirewireCamera::colorspace()
{
	...
	case DC1394_VIDEO_MODE_1600x1200_YUV422: return YUV422_PACKED;

	case DC1394_VIDEO_MODE_640x480_YUV411: return YUV411_PACKED;

	case DC1394_VIDEO_MODE_640x480_RGB8:
	case DC1394_VIDEO_MODE_800x600_RGB8:
	case DC1394_VIDEO_MODE_1024x768_RGB8:
	case DC1394_VIDEO_MODE_1280x960_RGB8:
	case DC1394_VIDEO_MODE_1600x1200_RGB8: return RGB;

	case DC1394_VIDEO_MODE_640x480_MONO8:
	case DC1394_VIDEO_MODE_800x600_MONO8:
	case DC1394_VIDEO_MODE_1024x768_MONO8:
	case DC1394_VIDEO_MODE_1280x960_MONO8:
	case DC1394_VIDEO_MODE_1600x1200_MONO8: return MONO8;

	case DC1394_VIDEO_MODE_640x480_MONO16:
	case DC1394_VIDEO_MODE_800x600_MONO16:
	case DC1394_VIDEO_MODE_1024x768_MONO16:
	case DC1394_VIDEO_MODE_1280x960_MONO16:
	case DC1394_VIDEO_MODE_1600x1200_MONO16: return MONO16;
	...
	case DC1394_VIDEO_MODE_FORMAT7_7:
		...
		case DC1394_COLOR_CODING_YUV411: return YUV411_PACKED;
		case DC1394_COLOR_CODING_YUV422: return YUV422_PACKED;
		case DC1394_COLOR_CODING_RGB8: return RGB;
...
FirewireCamera::set_auto_focus(bool enabled)
{
	dc1394error_t err;
	if ((err =
	       dc1394_feature_set_mode(_camera,
	                               DC1394_FEATURE_FOCUS,
	                               enabled ? DC1394_FEATURE_MODE_AUTO : DC1394_FEATURE_MODE_MANUAL))
	    == DC1394_SUCCESS) {
		_auto_focus = enabled;
	} else {
		throw Exception("FirewireCamera(%s): Setting auto focus failed (%s)",
		                _model,
		                dc1394_error_get_string(err));
	}
}
...
{
	if (dc1394_feature_set_mode(_camera,
...
{
	if (dc1394_feature_whitebalance_get_value(_camera, &_white_balance_ub, &_white_balance_vr)
	    != DC1394_SUCCESS) {
...
FirewireCamera::set_gain(unsigned int gain)
{
	...
	if (dc1394_feature_set_mode(_camera, DC1394_FEATURE_GAIN, DC1394_FEATURE_MODE_MANUAL)
	    != DC1394_SUCCESS) {
