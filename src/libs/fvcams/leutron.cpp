...
LeutronCamera::open()
{
	...
	if (DsyDetectCamera(&conn_info_handle) > 0) {
		...
		if (info) {
			...
			camera_handle = grabber->ConnectCamera(camera_id, info[0].hConn, info[0].SyncNr);
      ...
      */
			...
			if ((scaled_width != width) || (scaled_height != height)) {
				// scaled image
				scaled_buffer = (unsigned char *)malloc(
				  colorspace_buffer_size(YUV422_PACKED, scaled_width, scaled_height));
			}
