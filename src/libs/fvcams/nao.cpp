...
static inline s32_
i2c_smbus_access(int file, char read_write, u8_ command, int size, union i2c_smbus_data *data)
{
...
static inline s32_
i2c_smbus_write_block_data(int file, u8_ command, u8_ length, u8_ *values)
{
	union i2c_smbus_data data;
	int                  i;
	if (length > 32)
 ...
 */
NaoCamera::NaoCamera(const CameraArgumentParser *cap) : V4L2Camera(cap)
{
	if (cap->has("i2c_device"))
		i2c_device_name_ = strdup(cap->get("i2c_device").c_str());
	else
		throw MissingParameterException("NaoCamera: Missing I2C device");

	can_switch_cam_ = false;
	cam_id_         = 2;

	if (cap->has("cam")) {
		if (strcasecmp(cap->get("cam").c_str(), "brow") == 0)
			cam_id_ = 1;
	}
	...
	if (val == -1)
		close_dev(dev, "NaoCamera: Error reading dsPic version from I2C");
	if (val < 2) {
 ...
 */
int
NaoCamera::open_dev(const char *i2c)
{
	// Connect to dsPIC through I2C
	int dev = ::open(i2c, O_RDWR);
	if (dev < 0)
		throw Exception("NaoCamera: Error opening I2C for connection to dsPIC");
	if (ioctl(dev, I2C_SLAVE, DSPIC_I2C_ADDR) < 0)
		close_dev(dev, "NaoCamera: Can't connect I2C to dsPIC");
	return dev;
}
 ...
 */
void
NaoCamera::close_dev(int dev, const char *error)
{
	if (::close(dev) < 0)
		throw fawkes::Exception("NaoCamera: Error closing device");
	if (error)
		throw fawkes::Exception(error);
}
 ...
 */
int
NaoCamera::get_open_cam_id(int dev)
{
	// Ask dsPIC which camera is active
	int cid = i2c_smbus_read_byte_data(dev, 220);
	if (cid == -1)
		close_dev(dev, "Error reading active cam from I2C");
	return cid;
}
 ...
 */
void
NaoCamera::switch_to_cam_id(int dev, int cam_id)
{
	unsigned char cmd[2];
	cmd[0]   = cam_id;
	cmd[1]   = 0;
	...
	if (size == -1)
		close_dev(dev, "NaoCamera: Error switching to other camera");
}
 ...
 */
void
NaoCamera::init_cam(const char *cam)
{
	int dev = ::open(cam, O_RDWR);
	if (dev < 0)
		throw Exception("NaoCamera: Error opening Camera");
	...
	if (ioctl(dev, VIDIOC_S_CTRL, &control))
		close_dev(dev, "Error setting other camera to default parameters");
 ...
 */
void
NaoCamera::set_source(unsigned char source)
{
	if (source == cam_id_) {
		LibLogger::log_debug("NaoCamera", "Correct camera already chosen");
		return;
 ...
 */
void
NaoCamera::set_exposure_auto(bool enabled)
{
