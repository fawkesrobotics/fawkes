...
/** Constructor. */
PluginTreeView::PluginTreeView() : m_dispatcher(FAWKES_CID_PLUGINMANAGER)
{
	ctor();
}
 ...
 */
PluginTreeView::PluginTreeView(BaseObjectType *cobject, const Glib::RefPtr<Gtk::Builder> builder)
: Gtk::TreeView(cobject), m_dispatcher(FAWKES_CID_PLUGINMANAGER)
{
	ctor();
}
...
PluginTreeView::ctor()
{
	...
	m_dispatcher.signal_disconnected().connect(
	  sigc::mem_fun(*this, &PluginTreeView::on_disconnected));
	m_dispatcher.signal_message_received().connect(
	  sigc::mem_fun(*this, &PluginTreeView::on_message_received));
}
...
PluginTreeView::~PluginTreeView()
{
	if (m_dispatcher) {
		...
		FawkesNetworkMessage *msg =
		  new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_UNSUBSCRIBE_WATCH);
...
PluginTreeView::set_gconf_prefix(Glib::ustring gconf_prefix)
{
	...
	gconf_connection_ = gconf_->signal_value_changed().connect(
	  sigc::hide(sigc::hide(sigc::mem_fun(*this, &PluginTreeView::on_config_changed))));
...
PluginTreeView::on_connected()
{
	try {
		...
		FawkesNetworkMessage *msg =
		  new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_SUBSCRIBE_WATCH);
		client->enqueue(msg);

		// request list of available plugins
		msg = new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_LIST_AVAIL);
		client->enqueue(msg);

		// request list of loaded plugins
		msg = new FawkesNetworkMessage(FAWKES_CID_PLUGINMANAGER, MSG_PLUGIN_LIST_LOADED);
		client->enqueue(msg);
	} catch (Exception &e) {
		e.print_trace();
	}
...
PluginTreeView::on_message_received(fawkes::FawkesNetworkMessage *msg)
{
	...
	if ((msgid == MSG_PLUGIN_LOADED) || (msgid == MSG_PLUGIN_LOAD_FAILED)
	    || (msgid == MSG_PLUGIN_UNLOADED) || (msgid == MSG_PLUGIN_UNLOAD_FAILED)) {
		Glib::ustring name   = "";
		bool          loaded = false;
		...
		Gtk::TreeIter iter;
		for (iter = m_plugin_list->children().begin(); iter != m_plugin_list->children().end();
		     ++iter) {
...
#	endif
	...
	Gtk::TreeViewColumn *plugin_col = get_column(2);
	if (plugin_col)
		plugin_col->signal_clicked().connect(sigc::mem_fun(*this, &PluginTreeView::on_name_clicked));
}
