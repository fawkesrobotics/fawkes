...
FuseImageListWidget::FuseImageListWidget()
{
	...
	menulist.push_back(
	  Gtk::Menu_Helpers::MenuElem("Update now",
	                              sigc::mem_fun(*this, &FuseImageListWidget::update_image_list)));
	menulist.push_back(Gtk::Menu_Helpers::SeparatorElem());
	menulist.push_back(
	  Gtk::Menu_Helpers::MenuElem("Add host manually",
	                              sigc::mem_fun(*this, &FuseImageListWidget::on_add_host_manually)));
#endif
 ...
 */
void
FuseImageListWidget::add_fountain_service(const char *name, const char *host_name, uint32_t port)
{
	...
	m_new_clients.lock();
	for (LockList<ClientData>::iterator iter = m_new_clients.begin(); iter != m_new_clients.end();
	     ++iter) {
...
FuseImageListWidget::set_image_list_trv(Gtk::TreeView *trv)
{
	...
	m_trv_image_list->signal_event().connect(
	  sigc::mem_fun(*this, &FuseImageListWidget::on_image_event));
	m_trv_image_list->signal_cursor_changed().connect(
	  sigc::mem_fun(*this, &FuseImageListWidget::on_image_selected));
	m_img_list_mutex.unlock();
}
...
FuseImageListWidget::set_toggle_compression_chk(Gtk::CheckButton *chk)
{
	...
	m_chk_compression->signal_toggled().connect(
	  sigc::mem_fun(*this, &FuseImageListWidget::on_compression_toggled));
}
...
FuseImageListWidget::set_auto_update_chk(Gtk::CheckButton *chk)
{
	...
	m_chk_auto_update->signal_toggled().connect(
	  sigc::mem_fun(*this, &FuseImageListWidget::on_auto_update_toggled));
}
...
FuseImageListWidget::set_auto_update(bool active, unsigned int interval_sec)
{
	...
	if (m_auto_update) {
		...
		m_timeout_conn = Glib::signal_timeout().connect_seconds(
		  sigc::mem_fun(*this, &FuseImageListWidget::on_update_timeout), m_interval_sec);
#else
		m_timeout_conn =
		  Glib::signal_timeout().connect(sigc::mem_fun(*this, &FuseImageListWidget::on_update_timeout),
		                                 m_interval_sec);
#endif
	} else
...
FuseImageListWidget::get_image_list()
{
	...
	try {
		m_cur_client.client = new FuseClient(m_cur_client.host_name.c_str(), m_cur_client.port, this);
...
FuseImageListWidget::fuse_inbound_received(FuseNetworkMessage *m) throw()
{
	...
	default: printf("Unhandled message type\n");
	}
}
...
{
	Gtk::Dialog *add_host = new Gtk::Dialog("Add host manually", true);
	...
	if (fawkes_ip)
		hent->set_text(std::string(fawkes_ip).append(":2208"));
	else
		hent->set_text("localhost:2208");
