...
: progname_(progname),
  ...
  logger_(NULL),
  sd_stdin_(io_service_),
  sd_stdout_(io_service_),
  sd_stderr_(io_service_),
  exit_status_(-1)
{
...
{
	pid_ = run_proc(file, argv, envp, pipe_stdin_w_, pipe_stdout_r_, pipe_stderr_r_);
                      ...
                      boost::asio::streambuf &               buf)
{
	boost::asio::async_read_until(sd,
	                              buf,
	                              '\n',
	                              boost::bind(&SubProcess::handle_log_line,
	                                          this,
	                                          logname,
	                                          log_level,
	                                          boost::ref(sd),
	...
	if (pid_ > 0) {
		...
		if (waitpid(pid_, &status, WUNTRACED | WCONTINUED | WNOHANG) > 0) {
			if (WIFEXITED(status)) {
				...
				if (exit_status_ != 0) {
					logger_->log_error(progname_.c_str(),
				...
				logger_->log_warn(progname_.c_str(),
