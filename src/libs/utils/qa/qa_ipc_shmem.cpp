...
public:
	...
	virtual bool
	operator==(const SharedMemoryHeader &s) const
	{
...
main(int argc, char **argv)
{
	...
	try {
		...
		s1 = new SharedMemory(MAGIC_TOKEN,
		                      h1,
		                      /* read only */ false,
		                      /* create    */ true,
		                      /* destroy   */ true);
		...
		// use ipcs to check
		s2 = new SharedMemory(MAGIC_TOKEN,
		                      h1,
		                      /* read only */ true,
		                      /* create    */ false,
		                      /* destroy   */ false);
	...
	while (!quit) {
		*m1 = i;
		std::cout << "Wrote " << *m1 << " (should be " << i << ") to b1, afterwards b2 reads: " << *m2
		          << std::endl;
		usleep(500000);
		++i;
	};
