 ...
 */
PositionToPixel::PositionToPixel(fawkes::tf::Transformer *tf,
                                              ...
                                              const fawkes::Time &     time)
{
	...
	fawkes::cart2polar3d(coord_in_transformed.getX(),
	                     coord_in_transformed.getY(),
	                     coord_in_transformed.getZ(),
	                     (polar_in.phi),
	                     (polar_in.theta),
	                     (polar_in.r));
                                    ...
                                    const fawkes::Time &     time)
{
	...
	if (pixel_unchecked.x < 0 || pixel_unchecked.x > (long int)cam_resolution_x_) {
		throw fawkes::OutOfBoundsException("horizontal position outside cam viewport.",
		                                   pixel_unchecked.x,
		                                   0,
		                                   cam_resolution_x_);
	} else if (pixel_unchecked.y < 0 || pixel_unchecked.y > (long int)cam_resolution_y_) {
		throw fawkes::OutOfBoundsException("vertical position outside cam viewport.",
		                                   pixel_unchecked.y,
		                                   0,
		                                   cam_resolution_y_);
	}
