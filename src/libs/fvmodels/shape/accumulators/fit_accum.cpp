...
FitAccum::getCircle(void) const
{
	...
	float delta = +A00 * A11 * A22 + A01 * A12 * A20 + A02 * A10 * A21 - A00 * A12 * A21
	...
	if (delta > -TOO_SMALL_DELTA && delta < TOO_SMALL_DELTA) {
		...
		c.center.x = (float)((+b0 * A11 * A22 + A01 * A12 * b2 + A02 * b1 * A21 - b0 * A12 * A21
		                      - A01 * b1 * A22 - A02 * A11 * b2)
		                     / delta);
		c.center.y = (float)((+A00 * b1 * A22 + b0 * A12 * A20 + A02 * A10 * b2 - A00 * A12 * b2
		                      - b0 * A10 * A22 - A02 * b1 * A20)
		                     / delta);
		c.radius   = (float)sqrt((+A00 * A11 * b2 + A01 * b1 * A20 + b0 * A10 * A21 - A00 * b1 * A21
