...
FittedCircle::fitCircle(circle_matrix *p) const
{
	...
	if (delta > -TOO_SMALL_DELTA && delta < TOO_SMALL_DELTA) {
		...
		c.center.x =
		  (float)((+p->b0 * p->A11 * p->A22 + p->A01 * p->A12 * p->b2 + p->A02 * p->b1 * p->A21
		           - p->b0 * p->A12 * p->A21 - p->A01 * p->b1 * p->A22 - p->A02 * p->A11 * p->b2)
		          / delta);
		c.center.y =
		  (float)((+p->A00 * p->b1 * p->A22 + p->b0 * p->A12 * p->A20 + p->A02 * p->A10 * p->b2
		           - p->A00 * p->A12 * p->b2 - p->b0 * p->A10 * p->A22 - p->A02 * p->b1 * p->A20)
		          / delta);
		c.radius =
		  (float)sqrt((+p->A00 * p->A11 * p->b2 + p->A01 * p->b1 * p->A20 + p->b0 * p->A10 * p->A21
		               - p->A00 * p->b1 * p->A21 - p->A01 * p->A10 * p->b2 - p->b0 * p->A11 * p->A20)
		                / delta
