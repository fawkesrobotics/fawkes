...
int
RcdCircleModel::parseImage(unsigned char *buf, ROI *roi)
{
	unsigned char *buffer     = roi->get_roi_buffer_start(buf);
	unsigned char *line_start = buffer;

	unsigned int     x, y;
	vector<upoint_t> pixels, remove_list;
	...
	do {
		...
		if ((float)count
			...
			if (radius > TBY_CIRCLE_RADIUS_MIN && radius < TBY_CIRCLE_RADIUS_MAX) {
...
int
RcdCircleModel::getShapeCount(void) const
{
	return m_Circles.size();
}

Circle *
RcdCircleModel::getShape(int id) const
{
...
Circle *
RcdCircleModel::getMostLikelyShape(void) const
{
	int cur = 0;
	switch (m_Circles.size()) {
	case 0: return NULL;
	case 1: return const_cast<Circle *>(&m_Circles[0]); // or use const Shape* def?!...
	default:
...
void
RcdCircleModel::calcCircle(const upoint_t & p1,
                           const upoint_t & p2,
                           const upoint_t & p3,
                           center_in_roi_t &center,
                           float &          radius)
