...
ScanlineRadial::operator++()
{
	...
	do {
		...
		if ((tmp_x < -(int)center_x) || (tmp_x > (int)(width - center_x)) || (tmp_y < -(int)center_y)
...
ScanlineRadial::reset()
{
	...
	if (auto_max_radius) {
		...
		dists[1] = (unsigned int)sqrt(float((width - center_x) * (width - center_x))
		                              + float(center_y * center_y));
		dists[2] = (unsigned int)sqrt(float((width - center_x) * (width - center_x))
		                              + float((height - center_y) * (height - center_y)));
		dists[3] = (unsigned int)sqrt(float(center_x * center_x)
		                              + float((height - center_y) * (height - center_y)));
