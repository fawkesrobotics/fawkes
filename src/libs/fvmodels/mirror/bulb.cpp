 ...
 */
Bulb::Bulb(const char *filename, const char *lut_id, bool destroy_on_delete)
{
	init();
 ...
 * @see SharedMemoryLookupTable
 */
Bulb::Bulb(unsigned int width, unsigned int height, const char *lut_id, bool destroy_on_delete)
{
	init();
...
Bulb::create()
{
	...
	if (lut_id != NULL) {
		shm_lut = new SharedMemoryLookupTable(lut_id, width, height, /* depth */ 1, bytes_per_sample);
...
void
Bulb::warp2unwarp(unsigned int  warp_x,
                  unsigned int  warp_y,
...
void
Bulb::unwarp2warp(unsigned int  unwarp_x,
                  unsigned int  unwarp_y,
                  unsigned int *warp_x,
                  unsigned int *warp_y)
{
...
polar_coord_2d_t
Bulb::getWorldPointRelative(unsigned int image_x, unsigned int image_y) const
{
                          ...
                          float        pose_ori) const
{
	...
	// (depends on "robOri")
	float globalPhi;
	if (pose_ori >= 0.0 && pointRelative.phi >= 0.0 && pointRelative.phi + pose_ori > M_PI) {
		globalPhi = -(2 * M_PI - (pointRelative.phi + pose_ori));
	} else if (pose_ori < 0.0 && pointRelative.phi < 0.0 && pointRelative.phi + pose_ori < -M_PI) {
 ...
 */
void
Bulb::setWorldPoint(unsigned int image_x, unsigned int image_y, float world_r, float world_phi)
{
	if (image_x > width) {
	...
	// update distances
	float dist_new = getDistanceInImage(image_x, image_y, image_center_x, image_center_y);
...
void
Bulb::setCenter(unsigned int image_x, unsigned int image_y)
{
	if (image_x > width) {
 ...
 */
bool
Bulb::isNonZero(unsigned int image_x, unsigned int image_y) const
{
	if (image_x > width)
		return false;
	if (image_y > height)
 ...
 */
float
Bulb::getAngle(unsigned int image_x, unsigned int image_y) const
{
	return atan2f((float(image_y) - float(image_center_y)), (float(image_x) - float(image_center_x)));
}
 ...
 */
float
Bulb::getDistanceInImage(unsigned int image_p1_x,
...
Bulb::convertAngleI2W(float angle_in_image) const
{
	...
	if (angle_in_image - orientation >= -M_PI && angle_in_image - orientation <= M_PI) {
	...
	if (angle_in_image + M_PI >= -M_PI && angle_in_image + M_PI <= M_PI) {
	...
	if (angle_in_image >= 0.0 && angle_in_image <= M_PI) {
		angle_in_image = (-angle_in_image + M_PI);
		...
		cout << "Bulb::convertAngleI2W: ERROR! An invalid angle occurred (angle=" << angle_in_image
		     << ")." << endl;
		return 0.0;
	}
