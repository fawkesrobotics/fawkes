...
format-branch-clang: check-parallel
	...
	if type -p clang-format >/dev/null; then \
		MERGE_BASE=$$MERGE_BASE git filter-branch -f --tree-filter 'PREV=$$(map $$(git rev-parse $$GIT_COMMIT^)); $(FAWKES_BASEDIR)/etc/format-scripts/format-tree-filter.sh $$MERGE_BASE $$GIT_COMMIT $$PREV' -- $$MERGE_BASE..HEAD; \
	else \
		echo -e "$(INDENT_PRINT)$(TRED)--- Cannot format code$(TNORMAL) (clang-format not found)"; \
    exit 1; \
  fi
