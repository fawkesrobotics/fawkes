...
.PHONY: config
config:
	$(SILENT)if [ ! -f $(CONFIG_DIR)/config.tmpl ]; then \
                echo "$(INDENT_PRINT)$(TRED)--- No $(CONFIG_DIR)/config.tmpl file found. ---$(TNORMAL)"; \
	else \
		echo "$(INDENT_PRINT)--> Generating configuration classes from template"; \
		rm -rf $(CONFIG_DIR)/config.cpp $(CONFIG_DIR)/config.h; \
		$(GENCONFIG) $(GENCONFIG_CONFIG) $(GENCONFIG_TEMPLATE_H) $(GENCONFIG_TEMPLATE_CPP) $(CONFIG_DIR)/config.h $(CONFIG_DIR)/config.cpp; \
	fi
